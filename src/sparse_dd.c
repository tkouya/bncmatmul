/********************************************************************************/
/*                                                                              */
/* sparse_dd.c : Sparse Matrix and Vector Library (DD Precision)                */
/* Copyright (c) 2024 Tomonori Kouya, All rights reserved.                      */
/*                                                                              */
/* Version 0.2 2024-08-01 : Append Ozaki scheme for SpMV                        */
/* Version 0.1 2024-04-19 : Create sparse_dd.c from sparse_mpf.c                */
/*                                                                              */
...
DDRSMatrix init_ddrsmatrix(long int row_dim, long int *nzero_col_dim, long int nzero_total_num)
{
	...
	ret = (DDRSMatrix)malloc(sizeof(ddrsmatrix));
	//ret = (DDRSMatrix)BNC_MALLOC(sizeof(ddrsmatrix));
	if(ret == NULL)
	{
		fprintf(stderr, "Cannot allocate DDRSMatrix\n");
		return ret;
	}
	...
	//ret->real_nzero_total_num = (long int)ceil((double)(nzero_total_num) / (double)_BNC_D_WIDTH) * _BNC_D_WIDTH;
	...
	//printf("nzero initialized!\n");
	if(ret->nzero_index == NULL)
	{
		fprintf(stderr, "Cannot allocate DDRSMatrix(nzero_index!)\n");
		return NULL;
	}
	...
	for(i = 0; i < row_dim; i++)
	{
		...
		{
			fprintf(stderr, "Cannot allocate DDRSMatrix(nzero_index[%ld]!)\n", i);
			return NULL;
		}
		// location of nonzero elements have not been set!
...
/*
	if((ret->element[0] == NULL) || (ret->element[1] == NULL))
	{
		fprintf(stderr, "Cannot allocate DDRSMatrix(element!)\n");
		return NULL;
	}
*/
	// element := 0
...
void normf_ddrsmatrix(double ret[DDSIZE], DDRSMatrix mat)
{
...
#if defined(__AVX2__) && !defined(__AVX512F__) // __AVX2__
	__m256d mat4[DDSIZE], tmp4[DDSIZE], mul4[DDSIZE];
	...
	{
		mat4[0] = _mm256_load_pd(&(mat->element[0][index]));
		mat4[1] = _mm256_load_pd(&(mat->element[1][index]));

		// tmp4 += a4 * a4
		_bncavx2_rdd_mul(mul4, mat4, mat4);
		//tmp4 = _mm256_fmadd_pd(mat4, mat4, tmp4);
		_bncavx2_rdd_add(tmp4, tmp4, mul4);
	}
...
#elif defined(__AVX512F__) // __AVX512F__
	__m512d mat8[DDSIZE], tmp8[DDSIZE], mul8[DDSIZE];
	...
	for(index = 0; index < mat->real_nzero_total_num; index += _BNC_D_WIDTH)
	{
		mat8[0] = _mm512_load_pd(&(mat->element[0][index]));
		mat8[1] = _mm512_load_pd(&(mat->element[1][index]));

		// tmp8 += a8 * a8
		_bncavx512_rdd_mul(mul8, mat8, mat8);
		//tmp8= _mm512_fmadd_pd(mat8, mat8, tmp8);
		_bncavx512_rdd_add(tmp8, tmp8, mul8);
	}
...
#elif defined(__ARM_SVE2) && defined(BNC_ENABLE_SVE2) // Arm SVE2
	{
		...
		for(index = 0; index < mat->real_nzero_total_num; index += _vl)
		{
			...
			svfloat64_t a0 = svld1_f64(pg, &(mat->element[0][index]));
			svfloat64_t a1 = svld1_f64(pg, &(mat->element[1][index]));
			svfloat64_t m0, m1;
			_bncsve2_rdd_mul(pg, &m0, &m1, a0, a1, a0, a1);
			_bncsve2_rdd_add(pg, &tmp0, &tmp1, tmp0, tmp1, m0, m1);
		}
		{
			long _L, _vl = (long)svcntd();
			double _la0[64], _la1[64];
			svst1_f64(svptrue_b64(), _la0, tmp0);
			svst1_f64(svptrue_b64(), _la1, tmp1);
...
#elif defined(__ARM_NEON) && defined(BNC_ENABLE_NEON) // ARM Neon
	float64x2_t mat2[DDSIZE], tmp2[DDSIZE], mul2[DDSIZE];
	...
	{
		mat2[0] = vld1q_f64(&(mat->element[0][index]));
		mat2[1] = vld1q_f64(&(mat->element[1][index]));

		// tmp2 += a2 * a2
		_bncneon_rdd_mul(mul2, mat2, mat2);
		//tmp2 = vfmaq_f64(tmp2, mat2, mat2);
		_bncneon_rdd_add(tmp2, tmp2, mul2);
	}
	...
	for(index = 0; index < mat->real_nzero_total_num; index++)
	{
		...
		mat_element[0] = mat->element[0][index];
		mat_element[1] = mat->element[1][index];
...
DDRSMatrix init_set_ddrsmatrix_ddmatrix(DDMatrix org_mat)
{
	...
	for(i = 0; i < org_mat->row_dim; i++)
	{
		*(ptr_nzero_col_dim + i) = 0;
		for(j = 0; j < org_mat->col_dim; j++)
		{
			...
			if(rdd_cmp_ui(get_ddmatrix_ij(org_mat, i, j), 0UL) != 0)
			{
	...
	// initialize
	ret = init_ddrsmatrix(org_mat->row_dim, ptr_nzero_col_dim, nzero_total_num); // , org_mat->prec);
	...
	for(i = 0; i < ret->row_dim; i++)
	{
		...
		for(j = 0; j < ret->col_dim; j++)
		{
			...
			if(rdd_cmp_ui(get_ddmatrix_ij(org_mat, i, j), 0UL) != 0)
			{
				...
				rdd_set(tmp.val, get_ddmatrix_ij(org_mat, i, j));
				//ret->element[0][total_index] = tmp.val[0];
				//ret->element[1][total_index] = tmp.val[1];
...
DDRSMatrix init_set_ddrsmatrix(DDRSMatrix org_sp)
{
    ...
    ret = init_ddrsmatrix(org_sp->row_dim, org_sp->nzero_col_dim, org_sp->nzero_total_num);
...
sparse_status_t convert_ddrsmatrix_mkl_csrmat(sparse_matrix_t *ret[DDSIZE], MKL_INT *ret_i_csr_start, MKL_INT *ret_i_csr_end, MKL_INT *ret_j_csr, DDRSMatrix mat)
{
    ...
    //int *i_mat_csr, *j_mat_csr, row_dim = (int)mat->row_dim;
    //int row_dim = (int)mat->row_dim;
    sparse_status_t ret_mkl[DDSIZE];
...
#endif // 0
	convert_indeces_ddrsmatrix_mkl_csrmat(ret_i_csr_start, ret_i_csr_end, ret_j_csr, mat);
...
/*		switch(jres)
		{
			case 1:
				...
				rdd_add(mat_vec_i, mat_vec_i, tmp);
				total_index++;
		...
		printf("%ld %ld mat_vec_i = %25.17e\n", i, total_index, mat_vec_i[0]);
*/
...
*/
...
#elif defined(__ARM_SVE2) && defined(BNC_ENABLE_SVE2) // Arm SVE2
	/* SVE2 DD SpMV: gathers vec->element[0..1][nzero_index[i][j+k]] using
	 * hardware gather instructions, accumulates DD product via _bncsve2_rdd_mul/add. */
	{
		long _vl = (long)svcntd();
		long int jmax, jres;
		double mat_vec_i[DDSIZE];
		for(i = 0; i < mat->row_dim; i++)
		{
			...
			}
			{
				...
				svst1_f64(svptrue_b64(), _la0, tmp0);
				svst1_f64(svptrue_b64(), _la1, tmp1);
				rdd_set_ui(mat_vec_i, 0UL);
				for(_L = 0; _L < _vl; _L++)
				{
					rdd_add_d(mat_vec_i, mat_vec_i, _la0[_L]);
					rdd_add_d(mat_vec_i, mat_vec_i, _la1[_L]);
				}
			}
			/* scalar tail using rdd_mul/add */
			{
				...
				for(j = 0; j < jres; j++)
				{
					...
					rdd_mul(pr, mij, vj);
					rdd_add(mat_vec_i, mat_vec_i, pr);
					total_index++;
				}
			}
	...
	long int jmax, jres, col_dim;
	double mat_vec_i[DDSIZE], vset[DDSIZE];
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jmax; j += 2)
		{
			...
			// vb2[1] = _mm256_set_pd(
			// 	vset[1], // vec->element[1][mat->nzero_index[i][j + 3]],
			// 	vset[0], // vec->element[1][mat->nzero_index[i][j + 2]],
			// );
			//printf("mul_add ");
		...
		_bncneon_rdd_sum128d(mat_vec_i, tmp2);
...
int mul_ddrsmatrixt_ddvec(DDVector ret, DDRSMatrix mat, DDVector vec)
{
	...
	double tmp[DDSIZE], mat_ji[DDSIZE], vec_i[DDSIZE], ret_j[DDSIZE];
...
#if defined(__AVX2__) && !defined(__AVX512F__) // __AVX2__
	__m256d a4[DDSIZE], vb4[DDSIZE], tmp4[DDSIZE], ret4[DDSIZE];
	long int jmax, jres, col_dim;
...
/*
		...
		switch(jres)
		{
			case 1: 
				...
				rdd_mul(tmp, mat_ji, vec_i);
				rdd_add(ret_j, ret_j, tmp);
...
*/
...
#elif defined(__AVX512F__) // __AVX512F__
	__m512d a8[DDSIZE], vb8[DDSIZE], tmp8[DDSIZE], ret8[DDSIZE];
	long int jmax, jres, col_dim;
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jfull; j += _BNC_D_WIDTH)
		{
			...
			_bncavx512_rdd_mul(tmp8, a8, vb8);
...
*/
...
#elif defined(__ARM_SVE2) && defined(BNC_ENABLE_SVE2) // Arm SVE2
	/* SVE2 DD A^T*x: vb is a constant DD vector built from vec->element[0..1][i];
	 * for each j-batch we gather ret[0..1][nzero_index[i][j+k]], FMA in DD, scatter back. */
	{
		long _vl = (long)svcntd();
		long int jmax, jres;
		for(i = 0; i < mat->row_dim; i++)
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < mat->nzero_col_dim[i]; j++)
		{
			...
			rdd_add(ret_j, ret_j, tmp);
			set_ddvector_i(ret, mat->nzero_index[i][j], ret_j);
			total_index++;
		}
...
int mul_drsmatrix_ddvec(DDVector ret, DRSMatrix mat, DDVector vec)
{
	long int i, j, total_index;
	//ddfloat tmp, mat_ij, vec_j, ret_i;
	double tmp[DDSIZE], mat_ij, vec_j[DDSIZE], ret_i[DDSIZE];
...
#if defined(__AVX2__) && !defined(__AVX512F__) // __AVX2__
	__m256d a4, vb4[DDSIZE], tmp4[DDSIZE], tmp4_mul[DDSIZE];
	long int jmax, jres, col_dim;
	double mat_vec_i[DDSIZE], vset[_BNC_D_WIDTH];
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jmax; j += _BNC_D_WIDTH)
		{
			...
			//tmp4 = _mm256_fmadd_pd(a4, vb4, tmp4);
			//_bncavx2_rdd_mul(tmp4_mul, a4, vb4);
			_bncavx2_rdd_mul_d(tmp4_mul, vb4, a4); //, vb4);
			_bncavx2_rdd_add(tmp4, tmp4, tmp4_mul);
...
#elif defined(__AVX512F__) // __AVX512F__
	__m512d a8, vb8[DDSIZE], tmp8[DDSIZE], tmp8_mul[DDSIZE];
	long int jmax, jres, col_dim;
	double mat_vec_i[DDSIZE];
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jmax; j += _BNC_D_WIDTH)
		{
			...
			//_bncavx512_rdd_mul(tmp8_mul, a8, vb8);
			_bncavx512_rdd_mul_d(tmp8_mul, vb8, a8); //, vb8);
			_bncavx512_rdd_add(tmp8, tmp8, tmp8_mul);
...
#elif defined(__ARM_SVE2) && defined(BNC_ENABLE_SVE2) // Arm SVE2
	/* SVE2 D*DD SpMV: mat->element is plain double; vec is DD.
	 * Gather DD limbs of vec and multiply with D using _bncsve2_rdd_mul_d. */
	{
		long _vl = (long)svcntd();
		long int jmax, jres;
		double mat_vec_i[DDSIZE];
		for(i = 0; i < mat->row_dim; i++)
		{
			...
			/* scalar tail */
			{
				double vj[DDSIZE], pr[DDSIZE];
				double mij;
				long int col_dim = mat->nzero_col_dim[i];
				for(j = 0; j < jres; j++)
				{
					long int idx_t = mat->nzero_index[i][col_dim - jres + j];
					mij    = mat->element[total_index];
					vj[0]  = vec->element[0][idx_t];
					vj[1]  = vec->element[1][idx_t];
					rdd_mul_d(pr, vj, mij);
					rdd_add(mat_vec_i, mat_vec_i, pr);
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jmax; j += 2)
		{
			...
			//tmp2 = vfmaq_f64(tmp2, a2, vb2);
			//_bncneon_rdd_mul(tmp4_mul, a2, vb2);
			_bncneon_rdd_mul_d(tmp4_mul, vb2, a2); //, vb2);
			_bncneon_rdd_add(tmp2, tmp2, tmp4_mul);
...
int mul_drsmatrixt_ddvec(DDVector ret, DRSMatrix mat, DDVector vec)
{
	long int i, j, total_index;
	//ddfloat tmp, mat_ji, vec_j, ret_j;
	double tmp[DDSIZE], mat_ji, vec_i[DDSIZE], ret_j[DDSIZE];
...
#if defined(__AVX2__) && !defined(__AVX512F__) // __AVX2__
	__m256d a4, vb4[DDSIZE], tmp4[DDSIZE], ret4[DDSIZE];
	long int jmax, jres, col_dim;
	double vset[_BNC_D_WIDTH];
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jmax; j += _BNC_D_WIDTH)
		{
			...
			//ret4 = _mm256_fmadd_pd(a4, vb4, ret4);
			//_bncavx2_rdd_mul(tmp4, a4, vb4);
			_bncavx2_rdd_mul_d(tmp4, vb4, a4); 
			_bncavx2_rdd_add(ret4, ret4, tmp4);
...
#elif defined(__AVX512F__) // __AVX512F__
	__m512d a8, vb8[DDSIZE], tmp8[DDSIZE], ret8[DDSIZE];
	long int jmax, jres, col_dim;
	...
	for(i = 0; i < mat->row_dim; i++)
	{
		...
		for(j = 0; j < jmax; j += _BNC_D_WIDTH)
		{
			...
			_bncavx512_rdd_mul_d(tmp8, vb8, a8);
			_bncavx512_rdd_add(ret8, ret8, tmp8);
...
#elif defined(__ARM_SVE2) && defined(BNC_ENABLE_SVE2) // Arm SVE2
	/* SVE2 D*DD^T*x: mat is D, vec is DD; for each j-batch gather DD result
	 * limbs from ret[], FMA (D * DD), scatter back. */
	{
		long _vl = (long)svcntd();
		long int jmax, jres;
		for(i = 0; i < mat->row_dim; i++)
		{
			...
			/* scalar tail */
			{
				double vi[DDSIZE], pr[DDSIZE], cur[DDSIZE];
				double mij;
				long int col_dim = mat->nzero_col_dim[i];
				vi[0] = vec->element[0][i];
				vi[1] = vec->element[1][i];
...
void ddpower_sp(double max_eig[DDSIZE], DDVector evec, DDRSMatrix mat, double reps[DDSIZE], double aeps[DDSIZE], long int max_times)
{
	...
	ddfloat absmax_new_evec, old_max_eig, tmp, tmp2;
	DDVector new_evec;

	new_evec = init_ddvector(mat->row_dim);

	/* initialize evec */
	for(i = 0; i < evec->dim; i++)
		rdd_set_ui(get_ddvector_i(evec, i), 1UL);
		//evec->element[i] = 1.0;

	/* main loop */
	//old_max_eig = 0.0;
	rdd_set_ui(old_max_eig.val, 0UL);
	for(times = 0; times < max_times; times++)
	{
		/* w := A * x */
		mul_ddrsmatrix_ddvec(new_evec, mat, evec);
		absmax_index = absmax_index_ddvector(absmax_new_evec.val, new_evec);
//		max_eig = absmax_new_evec / evec->element[absmax_index];
		rdd_div(max_eig, absmax_new_evec.val, get_ddvector_i(evec, absmax_index));
//		smul_mpfvector(evec, 1.0 / absmax_new_evec, new_evec);
//		smul_mpfvector(evec, 1.0 / norm1_MPFVector(new_evec), new_evec); // Baba's example
		norm1_ddvector(tmp.val, new_evec);
		rdd_ui_div(tmp.val, 1UL, tmp.val);
		cmul_ddvector(evec, tmp.val, new_evec);

//		if((fabs(max_eig - old_max_eig) <= reps * fabs(old_max_eig) + aeps) && (times >= 2))
		rdd_sub(tmp.val, max_eig, old_max_eig.val);
		rdd_abs(tmp.val, tmp.val);
		
		rdd_abs(tmp2.val, old_max_eig.val);
		rdd_mul(tmp2.val, reps, tmp2.val);
		rdd_add(tmp2.val, tmp2.val, aeps);
		if((rdd_cmp(tmp.val, tmp2.val) <= 0) && (times >= 2))
		{
			fprintf(stderr, "Convergent!(Iterative Times = %ld)\n", times);
			break;
		}
		if(times % 10 == 0)
			fprintf(stderr, "%5ld %25.17e\n", times, rdd_get_d(max_eig));
		//old_max_eig = max_eig;
		rdd_set(old_max_eig.val, max_eig);
	}

	free_ddvector(new_evec);
...
long int absmax_index_ddvector(double ret[DDSIZE], DDVector vec)
{
	...
	for(i = 0; i < vec->dim; i++)
	{
		rdd_abs(abs_element.val, get_ddvector_i(vec, i));
		if(rdd_cmp(ret, abs_element.val) < 0)
		{
			absmax_index = i;
			//*ret = abs_element;
		}
	...
	rdd_set(ret, get_ddvector_i(vec, absmax_index));
...
void subst_ddrsmatrix_drsmat(DDRSMatrix c, DRSMatrix a)
{
	...
	{
		fprintf(stderr, "ERROR: subst_ddrsmatrix_drsmat\n");
		return;
	}
...
void subst_drsmatrix_ddrsmat(DRSMatrix c, DDRSMatrix a)
{
	...
	if((c->row_dim != a->row_dim) || (c->col_dim != a->col_dim))
	{
		fprintf(stderr, "ERROR: subst_dmatrix_ddrsmat\n");
		return;
	}
...
void absmax_row_ddrsmatrix(double mu[DDSIZE], long int *max_j, long int row_index, DDRSMatrix mat)
{
    ...
    double abs_aij[DDSIZE], aij[DDSIZE];

	//mu = fabs(mat[i * col_dim + 0]);
	get_ddrsmatrix_ij(aij, mat, row_index, 0);
    rdd_abs(mu, aij);

	//for(j = 1; j < mat->nzero_col_dim[row_index]; j++)
	for(j = 1; j < mat->real_nzero_col_dim[row_index]; j++)
	{
		get_ddrsmatrix_ij(aij, mat, row_index, j);
		rdd_abs(abs_aij, aij);
...
void absmax_col_ddrsmatrix(double mu[DDSIZE], long int *max_i, long int col_index, DDRSMatrix mat)
{
	...
	//mu = fabs(mat[i * col_dim + 0]);
	get_ddrsmatrix_ij(aij, mat, 0, col_index);
    rdd_abs(mu, aij);

	//for(j = 1; j < mat->nzero_col_dim[row_index]; j++)
	for(i = 1; i < mat->row_dim; i++) //mat->real_nzero_col_dim[row_index]; j++)
	{
		get_ddrsmatrix_ij(aij, mat, i, col_index);
		rdd_abs(abs_aij, aij);
		if(rdd_cmp(abs_aij, mu) > 0)
        {
			//mu = abs_aij;
...
void add_ddrsmatrix_drsmat(DDRSMatrix c, DDRSMatrix a, DRSMatrix b)
{
    ...
    double aij[DDSIZE], bij[DDSIZE], tmp[DDSIZE];

	/* check row_dim */
	if((a->row_dim != b->row_dim) || (b->row_dim != c->row_dim) || (c->row_dim != a->row_dim))
	{
		fprintf(stderr, "ERROR: add_ddrsmatrix_drsmat\n");
		return;
	}
	row_dim = c->row_dim;
	...
	{
		//rdd_add_d(tmp, get_ddrsmatrix_ij(a, i, j), get_drsmatrix_ij(b, i, j));
        bij[0] = b->element[index]; bij[1] = 0.0;
		aij[0] = a->element[0][index]; aij[1] = a->element[1][index];
        rdd_add(tmp, aij, bij);
...
void sub_ddrsmatrix_drsmat(DDRSMatrix c, DDRSMatrix a, DRSMatrix b)
{
	...
	if((a->row_dim != b->row_dim) || (b->row_dim != c->row_dim) || (c->row_dim != a->row_dim))
	{
		fprintf(stderr, "ERROR: sub_ddrsmatrix_drsmat\n");
		return;
	}
	row_dim = c->row_dim;
	//real_row_dim = c->real_row_dim;
...
int split_ddrsmatrix_drsmat_ex(DRSMatrix ret_mat[], long int row_shift[], int num_div, DDRSMatrix org_mat)
{
	...
	{
		fprintf(stderr, "ERROR: split_ddrsmatrix_drsmat: cannot allocate\n");
		free(power2);
		free(row_start);
		return 0;
	}
	...
	if(ret_mat == NULL)
	{
		own_ret_mat = init_set_drsmatrix_ddrsmat(org_mat);
	...
	tmp_org_mat = init_set_ddrsmatrix(org_mat);
...
{
	return split_ddrsmatrix_drsmat_ex(ret_mat, NULL, num_div, org_mat);
}
...
/* slice pair for it, accumulating in DD on the spot, so both the sparse      */
...
int split_ddrsmatrix_t_drsmat_ex(DRSMatrix ret_mat[], long int col_shift[], int num_div, DDRSMatrix org_mat)
{
	...
	{
		fprintf(stderr, "ERROR: split_ddrsmatrix_t_drsmat: cannot allocate\n");
		free(power2);
		free(row_start);
		free(mu_local);
		return 0;
...
{
	return split_ddrsmatrix_t_drsmat_ex(ret_mat, NULL, num_div, org_mat);
}
...
void mul_ddrsmatrixt_ddvec_oz(DDVector ret, DDRSMatrix a, int max_num_div_a, DDVector vb, int max_num_div_vb)
{
    ...
    {
        fprintf(stderr, "ERROR: mul_ddrsmatrixt_ddvec_oz: cannot allocate\n");
    ...
    real_num_div_a = split_ddrsmatrix_t_drsmat_ex(div_a, col_shift, max_num_div_a, a);
    real_num_div_vb = split_ddvector_dvec_ex(div_vb, vec_shift, max_num_div_vb, vb);
        ...
        for(i = 0; i < real_num_div_a; i++)
        {
            for(j = 0; j < real_num_div_vb; j++)
            {
                ...
                for(jj = 0; jj < col_dim; jj++)
                {
                    ...
                    double ret_j[DDSIZE], add_j[DDSIZE], sum_j[DDSIZE];
                    int comp, th;
...
// Incomplete LU decomposition; iLU0_ddrsmatrix
void iLU0_ddrsmatrix(DDRSMatrix mat)
{
	long int i, j, k, row_dim, col_dim;
	double aii[DDSIZE], aji[DDSIZE], ajk[DDSIZE], aik[DDSIZE], ctmp[DDSIZE];
	//double dtmp[DDSIZE];
	...
	for(i = 0; i < row_dim; i++)
	{
		get_ddrsmatrix_ij(aii, mat, i, i);
		if(rdd_cmp_ui(aii, 0UL) != 0)
		{
			for(j = i + 1; j < row_dim; j++)
			{
				get_ddrsmatrix_ij(aji, mat, j, i);
				if(rdd_cmp_ui(aji, 0UL) != 0)
				{
					//aji /= aii;
					rdd_div(aji, aji, aii);
					set_ddrsmatrix_ij(mat, j, i, aji);
					for(k = i + 1; k < col_dim; k++)
					{
						get_ddrsmatrix_ij(ajk, mat, j, k);
						get_ddrsmatrix_ij(aik, mat, i, k);
						if((rdd_cmp_ui(ajk, 0UL) != 0) && (rdd_cmp_ui(aik, 0UL) != 0))
						{
							//ajk = ajk - aji * aik;
							rdd_mul(ctmp, aji, aik);
							rdd_sub(ajk, ajk, ctmp);
							set_ddrsmatrix_ij(mat, j, k, ajk);
							//printf("%ld, %ld, %ld\n", i, j, k);
						}
					}
				}
...
void solve_iLU0_ddrsmatrix(DDVector ret, DDRSMatrix ilu, DDVector b)
{
	...
	double ret_i[DDSIZE], ret_j[DDSIZE], ilu_ii[DDSIZE], ilu_ij[DDSIZE], ctmp[DDSIZE];

	row_dim = ilu->row_dim;
	col_dim = ilu->col_dim;

	// ret := b
	subst_ddvector(ret, b);

	// Forward substitution
	for(i = 0; i < row_dim; i++)
	{
		get_ddrsmatrix_ij(ilu_ii, ilu, i, i);
		subst_ddvector_i(ret_i, ret, i);
		if(rdd_cmp_ui(ilu_ii, 0UL) != 0)
		{
			for(j = 0; j < i; j++)
			{
				get_ddrsmatrix_ij(ilu_ij, ilu, i, j);
				if(rdd_cmp_ui(ilu_ij, 0UL) != 0)
				{
					subst_ddvector_i(ret_j, ret, j);
					//ret_j = ret_j - ilu_ji * ret_i;
					rdd_mul(ctmp, ilu_ij, ret_j);
					rdd_sub(ret_i, ret_i, ctmp);
					//set_ddvector_i(ret, j, &ret_j);
					//printf("f: %ld, %ld\n", i, j);
				}
			}
			set_ddvector_i(ret, i, ret_i);
...
{
	long int i, j, row_dim, col_dim;
	double ret_i[DDSIZE], ret_j[DDSIZE], ctmp[DDSIZE], ilu_ii[DDSIZE], ilu_ji[DDSIZE];
	...
	for(i = 0; i < row_dim; i++)
	{
		...
		if(rdd_cmp_ui(ilu_ii, 0UL) != 0)
		{
			for(j = 0; j < i; j++)
			{
				get_ddrsmatrix_ij(ilu_ji, ilu, j, i);
				if(rdd_cmp_ui(ilu_ji, 0UL) != 0)
				{
					subst_ddvector_i(ret_j, ret, j);
					//ret_i = ret_i - ilu_ji * ret_j;
					rdd_mul(ctmp, ret_j, ilu_ji);
					rdd_sub(ret_i, ret_i, ctmp);
...
void solve_iLU0_drsmatrix_ddvec(DDVector ret, DRSMatrix ilu, DDVector b)
{
	...
	double ret_i[DDSIZE], ret_j[DDSIZE], ctmp[DDSIZE];
...
/* (SplitMat_B on the dense one), exactly as in mul_ddmatrix_oz().            */
/*                                                                               */
/* The rows of ret are cut into blocks and one thread takes a block at a time,   */
/* running every slice pair for it and accumulating in DD on the spot, so    */
...
void mul_ddrsmatrix_ddmat_oz(DDMatrix ret, DDRSMatrix a, int max_num_div_a, DDMatrix b, int max_num_div_b)
{
    ...
    {
        fprintf(stderr, "ERROR: mul_ddrsmatrix_ddmat_oz mid_dim(a, b) = (%ld, %ld)!\n", mid_dim, b->row_dim);
        return;
    }
    ...
    {
        fprintf(stderr, "ERROR: mul_ddrsmatrix_ddmat_oz: cannot allocate\n");
