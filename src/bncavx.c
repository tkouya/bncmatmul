...
void mul_dmatrix_block(DMatrix ret, DMatrix mat_a, DMatrix mat_b, long int min_dim)
{
	...
	// for SIMD
	row_dim = ret->real_row_dim;
	col_dim = ret->real_col_dim;
	...
	mat_a_index = (long int **)BNC_CALLOC(max_num_div, sizeof(long int *));
	mat_b_index = (long int **)BNC_CALLOC(max_num_div, sizeof(long int *));
	ret_index = (long int **)BNC_CALLOC(max_num_div, sizeof(long int *));
	...
	{
		mat_a_index[i] = (long int *)BNC_CALLOC(4, sizeof(long int));
		mat_b_index[i] = (long int *)BNC_CALLOC(4, sizeof(long int));
		ret_index[i] = (long int *)BNC_CALLOC(4, sizeof(long int));
	}

	small_ret = (DMatrix *)BNC_CALLOC(num_div_col, sizeof(DMatrix));
	small_mat_a = (DMatrix *)BNC_CALLOC(num_div_mid, sizeof(DMatrix));
	small_mat_b = (DMatrix *)BNC_CALLOC(num_div_mid, sizeof(DMatrix));
	small_tmp_mat = (DMatrix *)BNC_CALLOC(num_div_mid, sizeof(DMatrix));
...
//		printf("start AB\n");
		...
		for(i = 0; i < dim; i++)
		{
			for(j = 0; j < dim; j++)
			{
				set_dmatrix_ij(mat_a, i, j, sqrt(5.0) * (double)(i + j + 1));
				set_dmatrix_ij(mat_b, i, j, sqrt(3.0) * (double)(dim - (i + j)));
...
//		printf("||A||_F, ||B||_F = %25.17e, %25.17e\n", normf_dmatrix(mat_a), normf_dmatrix(mat_b));
