...
void _bncomp_mul_dmatrix_oz(DMatrix ret, DMatrix a, int max_num_div_a, DMatrix b, int max_num_div_b) //, int num_digits)
{
    int i, j;
...
*/
	...
	{
		#pragma omp section
		//real_num_div_a = split_dmatrix(div_a, max_num_div_a, a);
		split_dmatrix(div_a[0], div_a[1], a);

		//printf("split_dmatrix_dmat(%d, %d)  ->%d\n", div_a[0]->real_row_dim, div_a[0]->real_col_dim, real_num_div_a);

		#pragma omp section
		//real_num_div_b = split_dmatrix_t(div_b, max_num_div_b, b);
		split_dmatrix_t(div_b[0], div_b[1], b);

		//printf("split_dmatrix_t_dmat(%d, %d)->%d\n", div_b[0]->real_row_dim, div_b[0]->real_col_dim, real_num_div_b);
	}
...
#endif // USE_IMKL
            //add_dmatrix_dmat(ret, ret, div_ret[thread_index]);	 
            //add_dmatrix_dmat(ret, ret, div_ret[j]);
			#pragma omp critical
				add_dmatrix(ret, ret, div_ret[i]);
    	}
