...
//	right_scaling_ddmatrix(mat_b, diag_right, NULL);
	...
	if((ret->row_dim % 2 == 1) || (ret->col_dim % 2 == 1) || (mid_dim % 2 == 1))
	{
		...
		*/
		...
		{
			//printf("peeling\n");
			#pragma omp task depend(in: mat_a, mat_b) depend(out: ret)
			_bncomp_mul_ddmatrix_strassen_odd_peeling(ret, mat_a, mat_b, min_dim);
			//mul_ddmatrix_strassen_odd_peeling(ret, mat_a, mat_b, min_dim);
...
void _bncomp_mul_ddmatrix_strassen_odd_peeling(DDMatrix ret, DDMatrix mat_a, DDMatrix mat_b, long int min_dim)
{
	...
	//#pragma omp parallel for private(j)
	#pragma omp task depend(in: mat_a) depend(out: mat_a11) private(i, j)
	...
	//#pragma omp parallel for private(j)
	#pragma omp task depend(in: mat_b) depend(out: mat_b11) private(i, j)
	...
	if(mid_dim_h < mid_dim)
	{
		...
		///#pragma omp parallel for
		#pragma omp task depend(in: mat_a) depend(out: vec_a12) private(i)
		for(i = 0; i < row_dim_h; i++)
		...
		//#pragma omp parallel for
		#pragma omp task depend(in: mat_b) depend(out: vec_b21) private(i)
	...
	if(row_dim_h < row_dim)
	{
		...
		//#pragma omp parallel for
		#pragma omp task depend(in: mat_a) depend(out: vec_a21) private(i)
		for(i = 0; i < mid_dim_h; i++)
	...
	if(col_dim_h < col_dim)
	{
		...
		//#pragma omp parallel for
		#pragma omp task depend(in: mat_b) depend(out: vec_b12) private(i)
		for(i = 0; i < mid_dim_h; i++)
...
//	_bncomp_mul_ddmatrix_strassen_even(mat_c11, mat_a11, mat_b11, min_dim);
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		//#pragma omp parallel for private(j)
		#pragma omp task depend(in: vec_a12, vec_b21) depend(out: mat_tmp) private(i, j)
		for(i = 0; i < row_dim_h; i++)
		{
			thread_index = omp_get_thread_num();
			for(j = 0; j < col_dim_h; j++)
	...
	//#pragma omp parallel for private(j)
	#pragma omp task depend(in: mat_c11) depend(out: ret) private(i, j)
	for(i = 0; i < row_dim_h; i++)
	{
		for(j = 0; j < col_dim_h; j++) 
	...
	{
		//#pragma omp parallel for
		#pragma omp task depend(in: vec_c12) depend(out: ret) private(i)
	...
	{
		//#pragma omp parallel for
		#pragma omp task depend(in: vec_c21) depend(out: ret) private(i)
		for(i = 0; i < col_dim_h; i++)
