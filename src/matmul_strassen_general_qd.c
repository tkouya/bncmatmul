...
long int num_addsub_mul_qdmatrix_strassen;	// addition and subtraction
long int num_mul_mul_qdmatrix_strassen;		// multiplication
...
void add_qdmatrix_partial(QDMatrix ret, long int ret_index[4], QDMatrix mat_a, long int mat_a_index[4], QDMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			rqd_add(tmp_val, get_qdmatrix_ij(mat_a, a_i, a_j), get_qdmatrix_ij(mat_b, b_i, b_j));
			set_qdmatrix_ij(ret, ret_i, ret_j, tmp_val);
		}
	}
}
...
void sub_qdmatrix_partial(QDMatrix ret, long int ret_index[4], QDMatrix mat_a, long int mat_a_index[4], QDMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			//tmp_val = get_qdmatrix_ij(mat_a, a_i, a_j) - get_qdmatrix_ij(mat_b, b_i, b_j);
			rqd_sub(tmp_val, get_qdmatrix_ij(mat_a, a_i, a_j), get_qdmatrix_ij(mat_b, b_i, b_j));
			set_qdmatrix_ij(ret, ret_i, ret_j, tmp_val);
		}
	}
}
...
void subst_qdmatrix_partial(QDMatrix ret, long int ret_index[4], QDMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			set_qdmatrix_ij(ret, ret_i, ret_j, get_qdmatrix_ij(mat_a, a_i, a_j));
		}
	}
}
			...
			for(j = 0; j < jmax; j++)
			{
				...
				if((ret_j >= 0) && (ret_j < ret->col_dim))
				{
					if((a_i >= 0) && (a_i < mat_a->row_dim) && (a_j >= 0) && (a_j < mat_a->col_dim))
					{
						...
						set0_qdmatrix_ij(ret, ret_i, ret_j); // Padding
...
void neg_qdmatrix_partial(QDMatrix ret, long int ret_index[4], QDMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			set_qdmatrix_ij(ret, ret_i, ret_j, -get_qdmatrix_ij(mat_a, a_i, a_j));
		#else
			rqd_neg(tmp, get_qdmatrix_ij(mat_a, a_i, a_j));
			set_qdmatrix_ij(ret, ret_i, ret_j, tmp);
		#endif // __cplusplus
		}
	}
}
...
/* c = a * b */
/* void mul_qdmatrix_simple(QDMatrix c, QDMatrix a, QDMatrix b)
{
	mul_qdmatrix(c, a, b);
}
*/
...
void mul_qdmatrix_block(QDMatrix ret, QDMatrix mat_a, QDMatrix mat_b, long int min_dim)
{
	...
	{
		fprintf(stderr, "ERROR: mat_a's col_dim %ld does not just fit mat_b's row_dim %ld!(mul_qdmatrix_block)\n", mat_a->col_dim, mat_b->row_dim);
		return;
	}
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		for(j = 0; j < num_div_mid; j++)
		{
			...
			subst_qdmatrix_partial_checked(small_mat_a[j], small_mat_a_index, mat_a, mat_a_index);
		}
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			for(k = 0; k < num_div_mid; k++)
			{
				...
				subst_qdmatrix_partial_checked(small_mat_b[k], small_mat_b_index, mat_b, mat_b_index);

				// ret[j] += small_mat_a[i][k] * small_mat_b[k][j];
				mul_qdmatrix(small_tmp_mat, small_mat_a[k], small_mat_b[k]);
				add_qdmatrix(small_ret[j], small_ret[j], small_tmp_mat);
			}
			...
			subst_qdmatrix_partial_checked(ret, ret_index, small_ret[j], small_ret_index);
		}
	}

	// free
	free_qdmatrix(small_tmp_mat);
	for(i = 0; i < num_div_col; i++)
...
void mul_qdmatrix_strassen_odd_padding(QDMatrix ret, QDMatrix mat_a, QDMatrix mat_b, long int min_dim)
{
	...
	tmp_ret = init_static_padding_qdmatrix_strassen(ret);
	tmp_mat_a = init_static_padding_qdmatrix_strassen(mat_a);
	tmp_mat_b = init_static_padding_qdmatrix_strassen(mat_b);
#else
//	tmp_ret = init_dynamic_padding_qdmatrix_strassen(ret);
//	tmp_mat_a = init_dynamic_padding_qdmatrix_strassen(mat_a);
//	tmp_mat_b = init_dynamic_padding_qdmatrix_strassen(mat_b);
	tmp_ret = init_dynamic_padding_qdmatrix_strassen2(ret, min_dim);
	tmp_mat_a = init_dynamic_padding_qdmatrix_strassen2(mat_a, min_dim);
	tmp_mat_b = init_dynamic_padding_qdmatrix_strassen2(mat_b, min_dim);
#endif

	// strassen
#ifdef USE_WINOGRAD
	mul_qdmatrix_winograd_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#else
	mul_qdmatrix_strassen_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#endif
	...
	subst_qdmatrix_partial(ret, ret_index, tmp_ret, tmp_ret_index);
...
{
	num_addsub_mul_qdmatrix_strassen = 0;
	num_mul_mul_qdmatrix_strassen = 0;
}
...
void get_num_mul_qdmatrix_strassen(long int *num_addsub, long int *num_mul)
{
	...
	if(num_addsub != NULL)
		*num_addsub = num_addsub_mul_qdmatrix_strassen;
	if(num_mul != NULL)
		*num_mul = num_mul_mul_qdmatrix_strassen;
}
...
//	QDVector diag_left, diag_right;
	...
	if((ret->row_dim <= min_dim) && (ret->col_dim <= min_dim) && (mid_dim <= min_dim))
	{
		...
		num_addsub_mul_qdmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
		num_mul_mul_qdmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
...
//	right_scaling_qdmatrix(mat_b, diag_right, NULL);
	...
	{
#ifdef PEELING_ONLY
		mul_qdmatrix_strassen_odd_peeling(ret, mat_a, mat_b, min_dim);
#elif PADDING_ONLY
		mul_qdmatrix_strassen_odd_padding(ret, mat_a, mat_b, min_dim);
#else
...
#ifdef USE_WINOGRAD
		mul_qdmatrix_winograd_even(ret, mat_a, mat_b, min_dim);
#else
		mul_qdmatrix_strassen_even(ret, mat_a, mat_b, min_dim);
#endif
		//printf("end\n");
	}

//	mul_qdmatrix_ddiag_mat(ret, diag_left, 0, ret, diag_right, 0);
...
void mul_qdmatrix_strassen_odd_peeling(QDMatrix ret, QDMatrix mat_a, QDMatrix mat_b, long int min_dim)
{
	...
	// C11 = A11 * B11
#ifdef USE_WINOGRAD
	mul_qdmatrix_winograd_even(mat_c11, mat_a11, mat_b11, min_dim);
#else
	mul_qdmatrix_strassen_even(mat_c11, mat_a11, mat_b11, min_dim);
#endif
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += row_dim_h * col_dim_h;
		num_mul_mul_qdmatrix_strassen += row_dim_h * col_dim_h;
		...
		for(i = 0; i < row_dim_h; i++)
		{
			for(j = 0; j < col_dim_h; j++)
			{
				...
				rqd_mul(tmp, get_qdvector_i(vec_a12, i), get_qdvector_i(vec_b21, j));
				set_qdmatrix_ij(mat_tmp, i, j, tmp);
			}
		}
		add_qdmatrix(mat_c11, mat_c11, mat_tmp);
		//printf("C11 += a12 * b21...\n");
	}
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;
		num_mul_mul_qdmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;

		mul_qdmatrix_qdvec(vec_c12, mat_a11, vec_b12);
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += vec_c12->dim;
		num_mul_mul_qdmatrix_strassen += vec_tmp12->dim;

		cmul_qdvector(vec_tmp12, b22, vec_a12);
		add_qdvector(vec_c12, vec_c12, vec_tmp12);
	}
	//printf("c12 += b22 * a12\n");
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;
		num_mul_mul_qdmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;

		mul_qdmatrixt_qdvec(vec_c21, mat_b11, vec_a21);
	}
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += vec_c21->dim;
		num_mul_mul_qdmatrix_strassen += vec_tmp21->dim;

		cmul_qdvector(vec_tmp21, a22, vec_b21);
		add_qdvector(vec_c21, vec_c21, vec_tmp21);
	}
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += vec_a21->dim;
		num_mul_mul_qdmatrix_strassen += vec_tmp21->dim;

#ifdef __cplusplus
		ip_qdvector(&c22, vec_a21, vec_b12);
#else
		ip_qdvector(c22, vec_a21, vec_b12);
#endif // __cplusplus
	}
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_qdmatrix_strassen += 1;
		num_mul_mul_qdmatrix_strassen += 1;
...
//	printf("P1a:\n");
	...
	// counting the number of arithmetic
	num_addsub_mul_qdmatrix_strassen += row_dim_h * mid_dim_h;
...
//	printf("P1b:\n");
	...
	// counting the number of arithmetic
	num_addsub_mul_qdmatrix_strassen += mid_dim_h * col_dim_h;
...
//	add_qdmatrix_partial(mat_tmp_b[0], ret_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
	...
	// P1 = tmp_a * tmp_b
//	printf("P1:\n");
	mul_qdmatrix_strassen(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim);
...
//	subst_qdmatrix_partial(mat_tmp_b[1], ret_index, mat_b, mat_br_index[1]);
	...
	// P2 = tmp_a * tmp_b
	//printf("P2:\n");
	mul_qdmatrix_strassen(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim);
...
//	sub_qdmatrix_partial(mat_tmp_b[2], ret_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
	...
	// P3 = tmp_a * tmp_b
	//printf("P3:\n");
	mul_qdmatrix_strassen(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim);
...
//	sub_qdmatrix_partial(mat_tmp_b[3], ret_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
	...
	// P4 = tmp_a * tmp_b
	//printf("P4:\n");
	mul_qdmatrix_strassen(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim);
...
//	subst_qdmatrix_partial(mat_tmp_b[4], ret_index, mat_b, mat_br_index[4]);
	...
	// P5 = tmp_a * tmp_b
	//printf("P5:\n");
	mul_qdmatrix_strassen(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim);
...
//	add_qdmatrix_partial(mat_tmp_b[5], ret_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
	...
	// P6 = tmp_a * tmp_b
	//printf("P6:\n");
	mul_qdmatrix_strassen(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim);
...
//	add_qdmatrix_partial(mat_tmp_b[6], ret_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
	...
	// P7 = tmp_a * tmp_b
	//printf("P7:\n");
	mul_qdmatrix_strassen(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim);

	// -------------------------------
	// C11 := P1 + P4 - P5 + P7
	// -------------------------------
	// counting the number of arithmetic
	num_addsub_mul_qdmatrix_strassen += 3 * row_dim_h * col_dim_h;
...
//	printf("s...\n");
	...
	// M1 := S2 * S6
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[0], mat_s[1], mat_s[5], min_dim);

	// -------------------------------
	// M2 := A11 * B11
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[1], mat_tmp_a[0], mat_tmp_b[0], min_dim);

	// -------------------------------
	// M3 := A12 * B21
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[2], mat_tmp_a[1], mat_tmp_b[2], min_dim);

	// -------------------------------
	// M4 := S3 * S7
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[3], mat_s[2], mat_s[6], min_dim);

	// -------------------------------
	// M5 := S1 * S5
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[4], mat_s[0], mat_s[4], min_dim);

	// -------------------------------
	// M6 := S4 * B22
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[5], mat_s[3], mat_tmp_b[3], min_dim);

	// -------------------------------
	// M7 := A22 * S8
	// -------------------------------
	mul_qdmatrix_strassen(mat_m[6], mat_tmp_a[3], mat_s[7], min_dim);
...
//	printf("t...\n");
	...
	// -------------------------------
	// counting the number of arithmetic
	num_addsub_mul_qdmatrix_strassen += 2 * row_dim_h * col_dim_h;
...
{
	QDMatrix mat_p[6], mat_tmp_a[4], mat_p3p6, mat_p6p2, mat_p3p6p2;
	long int dim_h, dim;
	...
	{
		fprintf(stderr, "ERROR: Impossible to get inverse matrix for rectangle matrix!(inv_qdmatrix_strassen_even)\n");
		return;
	}
	...
	{
		subst_qdmatrix(ret, mat_a);
		//print_qdmatrix(mat_a);
		inv_qdmatrix(ret);
	...
	mat_p3p6 = init_qdmatrix(dim_h, dim_h);
	mat_p6p2 = init_qdmatrix(dim_h, dim_h);
	mat_p3p6p2 = init_qdmatrix(dim_h, dim_h);

	for(i = 0; i < 6; i++)
		mat_p[i] = init_qdmatrix(dim_h, dim_h);

	for(i = 0; i < 4; i++)
		mat_tmp_a[i] = init_qdmatrix(dim_h, dim_h);
	...
	subst_qdmatrix_partial(mat_tmp_a[0], ret_index, mat_a, mat_a_index[0]);

	// P1 = tmp_a * tmp_b
	//printf("P1: %ld -> %ld start \n", dim, dim_h);
	inv_qdmatrix_strassen_even(mat_p[0], mat_tmp_a[0], min_dim);
	//inv_qdmatrix_strassen(mat_p[0], mat_tmp_a[0], min_dim);
	//printf("P1: %ld -> %ld end \n", dim, dim_h);
	...
	subst_qdmatrix_partial(mat_tmp_a[2], ret_index, mat_a, mat_a_index[2]);

	// P2 := A21 * P1
	mul_qdmatrix_strassen(mat_p[1], mat_tmp_a[2], mat_p[0], min_dim);
	...
	subst_qdmatrix_partial(mat_tmp_a[1], ret_index, mat_a, mat_a_index[1]);

	// P3 = P1 * A12
	mul_qdmatrix_strassen(mat_p[2], mat_p[0], mat_tmp_a[1], min_dim);

	// -------------------------------
	// P4 := A21 * P3
	// -------------------------------
	mul_qdmatrix_strassen(mat_p[3], mat_tmp_a[2], mat_p[2], min_dim);
	...
	mat_a_index[3][3] = dim;
	subst_qdmatrix_partial(mat_tmp_a[3], ret_index, mat_a, mat_a_index[3]);

	// P5 = P4 - A22
	sub_qdmatrix(mat_p[4], mat_p[3], mat_tmp_a[3]);
	...
	inv_qdmatrix_strassen_even(mat_p[5], mat_p[4], min_dim);
	//inv_qdmatrix_strassen(mat_p[5], mat_p[5], min_dim);
	//printf("P6: %ld -> %ld end\n", dim, dim_h);

	// -------------------------------
	// P3P6 := P3 * P6
	// -------------------------------
	mul_qdmatrix_strassen(mat_p3p6, mat_p[2], mat_p[5], min_dim);

	// -------------------------------
	// P6P2 := P6 * P2
	// -------------------------------
	mul_qdmatrix_strassen(mat_p6p2, mat_p[5], mat_p[1], min_dim);

	// -------------------------------
	// P3P6P2 := P6 * P2
	// -------------------------------
	mul_qdmatrix_strassen(mat_p3p6p2, mat_p3p6, mat_p[1], min_dim);

	// -------------------------------
	// RET11 := P1 - P3 * P6 * P2
	// -------------------------------
	sub_qdmatrix_partial(ret, mat_a_index[0], mat_p[0], ret_index, mat_p3p6p2, ret_index);

	// -------------------------------
	// RET12 := P3 * P6
	// -------------------------------
	subst_qdmatrix_partial(ret, mat_a_index[1], mat_p3p6, ret_index);

	// -------------------------------
	// RET21 := P6 * P2
	// -------------------------------
	subst_qdmatrix_partial(ret, mat_a_index[2], mat_p6p2, ret_index);

	// -------------------------------
	// RET22 := -P6
	// -------------------------------
	neg_qdmatrix_partial(ret, mat_a_index[3], mat_p[5], ret_index);

	// free
	free_qdmatrix(mat_p3p6);
	free_qdmatrix(mat_p6p2);
	free_qdmatrix(mat_p3p6p2);
...
void left_scaling_qdmatrix(QDMatrix ret, QDMatrix org_mat, QDVector scaling_diag_mat, long int *ret_col_index)
{
...
#ifdef __cplusplus
	qd_real absmax_val, tmp_abs;
#else // __cplusplus
	double absmax_val[QDSIZE], tmp_abs[QDSIZE];
#endif // __cplusplus
	row_dim = ret->row_dim;
	col_dim = ret->col_dim;
	...
	for(i = 0; i < row_dim; i++)
	{
		// search absmax value
		absmax_col_index = 0;
		//absmax_val = fabs(get_qdmatrix_ij(org_mat, i, 0));
		rqd_abs(absmax_val, get_qdmatrix_ij(org_mat, i, 0));
		for(j = 1; j < col_dim; j++)
		{
			//tmp_abs = fabs(get_qdmatrix_ij(org_mat, i, j));
			rqd_abs(tmp_abs, get_qdmatrix_ij(org_mat, i, j));
			if(rqd_cmp(absmax_val, tmp_abs) < 0)
			{
				absmax_col_index = j;
				//absmax_val = tmp_abs;
				rqd_set(absmax_val, tmp_abs);
			}
		}
		
		if(scaling_diag_mat != NULL)
			set_qdvector_i(scaling_diag_mat, i, absmax_val);
		if(ret_col_index != NULL)
			ret_col_index[i] = absmax_col_index;
		
		// scaling
		if(rqd_cmp_ui(absmax_val, 0UL) == 0)
		{
			for(j = 0; j < col_dim; j++)
				set_qdmatrix_ij(ret, i, j, get_qdmatrix_ij(org_mat, i, j));
		}
		else
		{
			for(j = 0; j < col_dim; j++)
			{
				//tmp_abs = get_qdmatrix_ij(org_mat, i, j) / absmax_val;
				rqd_div(tmp_abs, get_qdmatrix_ij(org_mat, i, j), absmax_val);
				set_qdmatrix_ij(ret, i, j, tmp_abs);
			}
		}
	}
...
void right_scaling_qdmatrix(QDMatrix ret, QDMatrix org_mat, QDVector scaling_diag_mat, long int *ret_row_index)
{
	...
	for(i = 0; i < col_dim; i++)
	{
		// search absmax value
		absmax_row_index = 0;
		//absmax_val = fabs(get_qdmatrix_ij(org_mat, 0, i));
		rqd_abs(absmax_val, get_qdmatrix_ij(org_mat, 0, i));
		for(j = 1; j < row_dim; j++)
		{
			//tmp_abs = fabs(get_qdmatrix_ij(org_mat, j, i));
			rqd_abs(tmp_abs, get_qdmatrix_ij(org_mat, j, i));
			if(rqd_cmp(absmax_val, tmp_abs) < 0)
			{
				absmax_row_index = j;
				rqd_set(absmax_val, tmp_abs);
		...
		{
			for(j = 0; j < row_dim; j++)
				set_qdmatrix_ij(ret, j, i, get_qdmatrix_ij(org_mat, j, i));
		}
		else
		{
			for(j = 0; j < row_dim; j++)
			{
				//tmp_abs = get_qdmatrix_ij(org_mat, j, i) / absmax_val;
				rqd_div(tmp_abs, get_qdmatrix_ij(org_mat, j, i), absmax_val);
				set_qdmatrix_ij(ret, j, i, tmp_abs);
			}
		}
	}
	...
	if(left_diag_mat != NULL)
	{
		...
		for(i = 0; i < row_dim; i++)
		{
			...
			if(left_inv_flag == 0)
			{
				for(j = 0; j < col_dim; j++)
				{
					//tmp = get_qdvector_i(left_diag_mat, i) * get_qdmatrix_ij(org_mat, i, j);
					rqd_mul(tmp, get_qdvector_i(left_diag_mat, i), get_qdmatrix_ij(org_mat, i, j));
					set_qdmatrix_ij(ret, i, j, tmp);
				}
			}
			...
			else
			{
				for(j = 0; j < col_dim; j++)
				{
					//tmp = get_qdmatrix_ij(org_mat, i, j) / get_dvector_i(left_diag_mat, i);
					rqd_div(tmp, get_qdmatrix_ij(org_mat, i, j), get_qdvector_i(left_diag_mat, i));
					set_qdmatrix_ij(ret, i, j, tmp);
				}
			}
		}
	...
	if(right_diag_mat != NULL)
	{
		...
		for(i = 0; i < row_dim; i++)
		{
			...
			if(right_inv_flag == 0)
			{
				for(j = 0; j < col_dim; j++)
				{
					//tmp = get_qdmatrix_ij(ret, i, j) * get_dvector_i(right_diag_mat, j);
					rqd_mul(tmp, get_qdmatrix_ij(ret, i, j), get_qdvector_i(right_diag_mat, j));
					set_qdmatrix_ij(ret, i, j, tmp);
				}
			}
			...
			else
			{
				...
				{
					//tmp = get_qdmatrix_ij(ret, i, j) / get_dvector_i(right_diag_mat, j);
					rqd_div(tmp, get_qdmatrix_ij(ret, i, j), get_qdvector_i(right_diag_mat, j));
					set_qdmatrix_ij(ret, i, j, tmp);
				}
			}
		}
...
int main(int argc, char *argv[])
{
	...
	double qdtmp[3][QDSIZE];
	QDMatrix qdc, qda, qdb, qdc_normal, qdc_block;
...
/* double-double precision */
	...
	qdc = init_qdmatrix(dim, dim);
	qdc_normal = init_qdmatrix(dim, dim);
	qdc_block = init_qdmatrix(dim, dim);
	qda = init_qdmatrix(dim, dim);
	qdb = init_qdmatrix(dim, dim);

	mpfdiag_left = init_qdvector(dim);
	mpfdiag_right = init_qdvector(dim);

	for(i = 0; i < dim; i++)
	{
		for(j = 0; j < dim; j++)
		{
			rqd_set_d(qdtmp[0], (double)rand());
			if(rand() % 2 != 0)
				rqd_neg(qdtmp[0], qdtmp[0]);

			rqd_set_d(qdtmp[1], (double)rand());
			rqd_ui_div(qdtmp[1], 1UL, qdtmp[1]);
			if(rand() % 2 != 0)
				rqd_neg(qdtmp[1], qdtmp[1]);

			set_qdmatrix_ij(qda, i, j, qdtmp[0]);
			set_qdmatrix_ij(qdb, i, j, qdtmp[1]);
		}
	}

	// normal matrix mul
	stime = get_secv();
	mul_qdmatrix(qdc_normal, qda, qdb);
	etime[1] = get_secv() - stime;
	normi_qdmatrix(qdtmp[0], qdc_normal);
	printf("||qdc_normal  || = "); rqd_out_str(qdtmp[0]); printf("\n");

	//left_scaling_qdmatrix(qda, qda, mpfdiag_left, NULL);
	//right_scaling_qdmatrix(qdb, qdb, mpfdiag_right, NULL);

	// Block
	stime = get_secv();
	mul_qdmatrix_block(qdc_block, qda, qdb, 32);
	etime[2] = get_secv() - stime;
	normi_qdmatrix(qdtmp[0], qdc_block);
	printf("||qdc_block   || = "); rqd_out_str(qdtmp[0]); printf("\n");

	// Strassen 
	stime = get_secv();
//	mul_qdmatrix_strassen(qdc, qda, qdb, 8);
//	mul_qdmatrix_strassen(qdc, qda, qdb, 16);
	mul_qdmatrix_strassen(qdc, qda, qdb, 32);
	etime[0] = get_secv() - stime;
	normi_qdmatrix(qdtmp[0], qdc);
	printf("||qdc_strassen|| = "); rqd_out_str(qdtmp[0]); printf("\n");

	//mul_qdmatrix_qddiag(qdc, mpfdiag_left, 0, qdc, mpfdiag_right, 0);

	// difference
	sub_qdmatrix(qdc, qdc_normal, qdc);
	sub_qdmatrix(qdc_block, qdc_normal, qdc_block);
	...
	printf("normal     : %f\n", etime[1]);
	normi_qdmatrix(qdtmp[0], qdc_normal);
	normi_qdmatrix(qdtmp[1], qdc);
	normi_qdmatrix(qdtmp[2], qdc_block);
	rqd_div(qdtmp[1], qdtmp[1], qdtmp[0]);
	rqd_div(qdtmp[2], qdtmp[2], qdtmp[0]);
	printf("||reldiff_strassen||: "); rqd_out_str(qdtmp[1]); printf("\n");
	printf("||reldiff_block   ||: "); rqd_out_str(qdtmp[2]); printf("\n");

/* Inverse */

	frank_qdmatrix(qda, dim);
	frank_qdmatrix(qdb, dim);
//	lotkin_qdmatrix(qda, dim);
//	lotkin_qdmatrix(qdb, dim);
	...
	stime = get_secv();
	inv_qdmatrix_strassen_even(qdc, qdb, 32);
	etime[0] = get_secv() - stime;
...
//	print_dmatrix(dc);
	...
	// difference
	sub_qdmatrix(qdb, qda, qdc);
	normi_qdmatrix(qdtmp[0], qdb);
	normi_qdmatrix(qdtmp[1], qda);
	rqd_div(qdtmp[0], qdtmp[0], qdtmp[1]);
	...
	free_qdmatrix(qdc);
	free_qdmatrix(qdc_normal);
	free_qdmatrix(qdc_block);
	free_qdmatrix(qda);
	free_qdmatrix(qdb);

	free_qdvector(mpfdiag_left);
	free_qdvector(mpfdiag_right);
