...
void get_ceig_mpfqrh(CMPFArray eig, MPFMatrix hmat, mpf_t reps)
{
	unsigned long prec;
	long int i, dim;
	...
	for(i = 0; i < dim - 1; )
	{
		...
		if(mpf_cmp(tmp[2], tmp[1]) >= 0)
		{
			...
			set_cmpfarray_i_re(eig, i    , tmp_real[0]);
			set_cmpfarray_i_im(eig, i    , tmp_imag[0]);
			set_cmpfarray_i_re(eig, i + 1, tmp_real[1]);
			set_cmpfarray_i_im(eig, i + 1, tmp_imag[1]);
		...
		if(i == dim - 1)
		{
			...
			set_cmpfarray_i_re(eig, dim - 1, tmp_real[0]);
			set_cmpfarray_i_im(eig, dim - 1, tmp_imag[0]);
