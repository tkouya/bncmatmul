...
#else // others
	//#pragma omp parallel for schedule(static) private(thread_index, j, total_index) threadprivate(thread_index, j, total_index)
...
//#endif // 0
	...
	//total_index = 0;
	//#pragma omp parallel for schedule(static) threadprivate(thread_index, j, total_index,jmax, jres, col_dim, mat_vec_i, vset)
	#pragma omp parallel for schedule(static) private(thread_index, j, total_index)
	for(i = 0; i < mat->row_dim; i++)
	{
        thread_index = omp_get_thread_num();
