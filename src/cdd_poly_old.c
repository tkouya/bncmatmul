...
void _bncavx2_eval_cddpoly_estrin(cddfloat *ret, CDDPoly a, cddfloat *x)
{
	...
	__m512d old_coef8_real[DDSIZE], old_coef8_imag[DDSIZE];
    ...
    _bncavx512_rdd_set0(zero8); //  = _mm256_setzero_pd();
	num_loop_unit = 2 * _BNC_D_WIDTH;
    ...
    //while(num_in_coef > num_loop_unit)
    {
		...
		for(i = 0; i < (num_in_coef / num_loop_unit); i++)
        {
            old_coef8_real[0][0] = _mm512_load_pd(&get_dvector_i(in_coef_old_real,  i * num_loop_unit));
            old_coef8_real[1][0] = _mm512_load_pd(&get_dvector_i(in_coef_old_real, (i + 1) * num_loop_unit + _BNC_D_WIDTH));
            old_coef8_imag[0][0] = _mm512_load_pd(&get_dvector_i(in_coef_old_imag,  i * num_loop_unit));
            old_coef8_imag[0][1] = _mm512_load_pd(&get_dvector_i(in_coef_old_imag, (i + 1) * num_loop_unit + _BNC_D_WIDTH));
            ...
            // b0 b1 b2 b3 ;= a04 + a14 * x4
            //        =  b0 + b1 * y + b2 * y^2 + b3 * y^3
            a08_real[0] = _mm512_unpacklo_pd(old_coef8_real[0], old_coef8_real[1]);
            a18_real[0] = _mm512_unpackhi_pd(old_coef8_real[0], old_coef8_real[1]);
            a08_imag[0] = _mm512_unpacklo_pd(old_coef8_imag[0], old_coef8_imag[1]);
            a18_imag[0] = _mm512_unpackhi_pd(old_coef8_imag[0], old_coef8_imag[1]);

            //new_coef8 = _mm512_fmadd_pd(a18, x8, a08);
            // a14 * x4 -> (a14_re * x4_re - a14_im * x4_im) + (a14_re * x4_im + a14_im * x4_re) * I
            new_coef8_real = _mm512_sub_pd(_mm512_mul_pd(a18_real, x8_real), _mm256_mul_pd(a18_imag, x8_imag));
            new_coef8_imag = _mm512_add_pd(_mm512_mul_pd(a18_real, x8_imag), _mm256_mul_pd(a18_imag, x8_real));

            // a14_x4 + a04 -> a14_x4_re + a_04_re + (a_14_x4_im + a_04_im) * I
            new_coef8_real = _mm512_add_pd(new_coef8_real, a08_real);
            new_coef8_imag = _mm512_add_pd(new_coef8_imag, a08_imag);
            //new_coef8 = _mm512_permute_pd(new_coef8, );
            _mm512_store_pd(&get_dvector_i(in_coef_old_real, i), new_coef8_real);
            _mm512_store_pd(&get_dvector_i(in_coef_old_imag, i), new_coef8_imag);
        }
#endif // __AVX2__
