...
void _bncavx2_eval_tdpoly_estrin(double ret[TDSIZE], TDPoly a, double x[TDSIZE])
{
    ...
    //while(num_in_coef > num_loop_unit)
    {
        ...
        for(i = 0; i < (num_in_coef / num_loop_unit); i++)
        {
            old_coef8[0] = _mm512_load_pd(&get_dvector_i(in_coef_old,  i * num_loop_unit));
            old_coef8[1] = _mm512_load_pd(&get_dvector_i(in_coef_old, (i + 1) * num_loop_unit + _BNC_D_WIDTH));
            ...
            _mm512_store_pd(&get_dvector_i(in_coef_new, i), new_coef8);
        }
#endif // __AVX2__
