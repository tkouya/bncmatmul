...
int CMPFLUdecomp_square(CMPFMatrix a, long int start_index, long int min_dim)
{
	...
	mpc_init2(dtmp, a->prec);
	mpf_init2(dmaxii, a->prec);

	// dim > min_dim
	for(i = start_index; i < imax; i++)
	{
		mpc_abs(dmaxii, get_cmpfmatrix_ij(a, i, i), get_bnc_default_rounding_mode());
		if(mpf_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CMPFLUdecomp_square)!\n", i);
			mpc_clear(dtmp);
			mpf_clear(dmaxii);
		...
		for(j = (i + 1); j < jmax; j++)
		{
			mpc_div(dtmp, get_cmpfmatrix_ij(a, j, i), get_cmpfmatrix_ij(a, i, i), get_bnc_default_rounding_mode());
			set_cmpfmatrix_ij(a, j, i, dtmp);
		}
		...
		for(j = (i + 1); j < jmax; j++)
		{
			for(k = (i + 1); k < jmax; k++)
			{
				mpc_mul(dtmp, get_cmpfmatrix_ij(a, j, i), get_cmpfmatrix_ij(a, i, k), get_bnc_default_rounding_mode());
				mpc_sub(dtmp, get_cmpfmatrix_ij(a, j, k), dtmp, get_bnc_default_rounding_mode());
				set_cmpfmatrix_ij(a, j, k, dtmp);
			}
		}
	}
...
int CMPFLUdecomp_square_omp(CMPFMatrix a, long int start_index, long int min_dim)
{
	...
	#pragma omp parallel private(thread_index)
	{
		...
		mpc_init2(dtmp[thread_index], a->prec);
		//mpc_init2(dmaxii[thread_index], a->prec);
	}
	mpf_init2(dmaxii, a->prec);
	...
	for(i = start_index; i < imax; i++)
	{
		...
		if(mpf_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld (start_index = %ld, min_dim = %ld): Error! (CMPFLUdecomp_square)!\n", i, start_index, min_dim);
			mpf_clear(dmaxii);
			#pragma omp parallel private(thread_index)
			{
				thread_index = omp_get_thread_num();
				mpc_clear(dtmp[thread_index]);
			}
			return -1;
		}
		...
		for(j = (i + 1); j < jmax; j++)
		{
			...
			mpc_div(dtmp[thread_index], get_cmpfmatrix_ij(a, j, i), get_cmpfmatrix_ij(a, i, i), get_bnc_default_rounding_mode());
			set_cmpfmatrix_ij(a, j, i, dtmp[thread_index]);
		}
		...
		for(j = (i + 1); j < jmax; j++)
		{
			...
			for(k = (i + 1); k < jmax; k++)
			{
				mpc_mul(dtmp[thread_index], get_cmpfmatrix_ij(a, j, i), get_cmpfmatrix_ij(a, i, k), get_bnc_default_rounding_mode());
				mpc_sub(dtmp[thread_index], get_cmpfmatrix_ij(a, j, k), dtmp[thread_index], get_bnc_default_rounding_mode());
				set_cmpfmatrix_ij(a, j, k, dtmp[thread_index]);
			}
		}
	}
...
int CMPFLUdecomp_l21(CMPFMatrix l21, CMPFMatrix a, long int start_index, long int min_dim)
{
	...
	mpc_init2(dtmp, l21->prec);
	mpc_init2(dtmp1, l21->prec);
	...
	for(i = imax; i < dim; i++)
	{
		...
		if(mpf_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CMPFLUdecomp_l21)!\n", i);
			mpc_clear(dtmp);
            mpc_clear(dtmp1);
			mpf_clear(dmaxii);
			return -1;
		...
		{
			mpc_set(dtmp, get_cmpfmatrix_ij(a, i, j), get_bnc_default_rounding_mode());
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_dmatrix_ij(a, i, k) * get_dmatrix_ij(a, k, j);
				mpc_mul(dtmp1, get_cmpfmatrix_ij(a, i, k), get_cmpfmatrix_ij(a, k, j), get_bnc_default_rounding_mode());
				mpc_sub(dtmp, dtmp, dtmp1, get_bnc_default_rounding_mode());
			}
			mpc_div(dtmp, dtmp, get_cmpfmatrix_ij(a, j, j), get_bnc_default_rounding_mode());
			...
			set_cmpfmatrix_ij(a  , i              , j       , dtmp);
			set_cmpfmatrix_ij(l21, i - imax, j - start_index, dtmp);
		}
	}
...
int CMPFLUdecomp_l21_omp(CMPFMatrix l21, CMPFMatrix a, long int start_index, long int min_dim)
{
	...
	//mpc_init2(dmaxii, l21->prec);
	#pragma omp parallel private(thread_index)
	{
		thread_index = omp_get_thread_num();

		mpc_init2(dtmp[thread_index], l21->prec);
		mpc_init2(dtmp1[thread_index], l21->prec);
	...
	for(i = imax; i < dim; i++)
	{
		...
		mpc_abs(dmaxii[thread_index], get_cmpfmatrix_ij(a, i, i), get_bnc_default_rounding_mode());
		if(mpf_cmp_ui(dmaxii[thread_index], 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CMPFLUdecomp_l21_omp)!\n", i);
			//mpc_clear(dmaxii);
			#pragma omp parallel private(thread_index)
			{
				thread_index = omp_get_thread_num();
				mpc_clear(dtmp[thread_index]);
 				mpc_clear(dtmp1[thread_index]);
		...
		for(j = start_index; j < jmax; j++)
		{
			...
			mpc_set(dtmp[thread_index], get_cmpfmatrix_ij(a, i, j), get_bnc_default_rounding_mode());
			for(k = start_index; k < j; k++)
			{
				//thread_index = omp_get_thread_num();

				//dtmp -= get_dmatrix_ij(a, i, k) * get_dmatrix_ij(a, k, j);
				mpc_mul(dtmp1[thread_index], get_cmpfmatrix_ij(a, i, k), get_cmpfmatrix_ij(a, k, j), get_bnc_default_rounding_mode());
				mpc_sub(dtmp[thread_index], dtmp[thread_index], dtmp1[thread_index], get_bnc_default_rounding_mode());
			}
			mpc_div(dtmp[thread_index], dtmp[thread_index], get_cmpfmatrix_ij(a, j, j), get_bnc_default_rounding_mode());
			
			//printf("(i - start_index, j - imax) = (%ld, %ld), %ld, %ld\n", i - imax, j - start_index, l21->row_dim, l21->col_dim);
			//printf("(i              , j       ) = (%ld, %ld) %25.17e, %25.17e\n", i, j, dtmp, get_dmatrix_ij(a, j, j));
			set_cmpfmatrix_ij(a  , i              , j       , dtmp[thread_index]);
			set_cmpfmatrix_ij(l21, i - imax, j - start_index, dtmp[thread_index]);
		}
	}
...
int CMPFLUdecomp_u12(CMPFMatrix u12, CMPFMatrix a, long int start_index, long int min_dim)
{
	...
	mpc_init2(dtmp, u12->prec);
    mpc_init2(dtmp1, u12->prec);
	...
	for(i = imax; i < dim; i++)
	{
		...
		if(mpf_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CMPFLUdecomp_u12)!\n", i);
			mpc_clear(dtmp);
            mpc_clear(dtmp1);
			mpf_clear(dmaxii);
			return -1;
		...
		for(j = start_index; j < jmax; j++)
		{
			mpc_set(dtmp, get_cmpfmatrix_ij(a, j, i), get_bnc_default_rounding_mode());
			//printf("(j              , i              ) = (%ld, %ld) %25.17e\n", j, i , dtmp);
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_dmatrix_ij(a, j, k) * get_dmatrix_ij(a, k, i);
				mpc_mul(dtmp1, get_cmpfmatrix_ij(a, j, k), get_cmpfmatrix_ij(a, k, i), get_bnc_default_rounding_mode());
				mpc_sub(dtmp, dtmp, dtmp1, get_bnc_default_rounding_mode());
				//printf("(j - start_index, k - start_index) = (%ld, %ld)\n", j - start_index, k - start_index);
			}
			//printf("(j              , i              ) = (%ld, %ld) %25.17e\n", j, i , dtmp);
			set_cmpfmatrix_ij(a  , j              , i       , dtmp);
			set_cmpfmatrix_ij(u12, j - start_index, i - imax, dtmp);
		}
	}
...
int CMPFLUdecomp_u12_omp(CMPFMatrix u12, CMPFMatrix a, long int start_index, long int min_dim)
{
	...
	//mpc_init2(dmaxii, u12->prec);
	#pragma omp parallel private(thread_index)
	{
		thread_index = omp_get_thread_num();
		mpc_init2(dtmp[thread_index], a->prec);
		mpc_init2(dtmp1[thread_index], a->prec);
	...
	for(i = imax; i < dim; i++)
	{
		...
		if(mpf_cmp_ui(dmaxii[thread_index], 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CMPFLUdecomp_u12_omp)!\n", i);
			//mpc_clear(dmaxii);
			#pragma omp parallel private(thread_index)
			{
				thread_index = omp_get_thread_num();
		...
		for(j = start_index; j < jmax; j++)
		{
			...
			mpc_set(dtmp[thread_index], get_cmpfmatrix_ij(a, j, i), get_bnc_default_rounding_mode());
			//printf("(j              , i              ) = (%ld, %ld) %25.17e\n", j, i , dtmp);
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_dmatrix_ij(a, j, k) * get_dmatrix_ij(a, k, i);
				mpc_mul(dtmp1[thread_index], get_cmpfmatrix_ij(a, j, k), get_cmpfmatrix_ij(a, k, i), get_bnc_default_rounding_mode());
				mpc_sub(dtmp[thread_index], dtmp[thread_index], dtmp1[thread_index], get_bnc_default_rounding_mode());
				//printf("(j - start_index, k - start_index) = (%ld, %ld)\n", j - start_index, k - start_index);
			}
			//printf("(j              , i              ) = (%ld, %ld) %25.17e\n", j, i , dtmp);
			set_cmpfmatrix_ij(a  , j              , i       , dtmp[thread_index]);
			set_cmpfmatrix_ij(u12, j - start_index, i - imax, dtmp[thread_index]);
		}
	}
...
#ifdef USE_SCALING
	diag_left = init2_cmpfvector(l21->row_dim, d22->prec);
	diag_right = init2_cmpfvector(u12->col_dim, d22->prec);

	left_scaling_cmpfmatrix(l21, l21, diag_left, NULL);
	right_scaling_cmpfmatrix(u12, u12, diag_right, NULL);
#endif // USE_SCALING

//	mul_cmpfmatrix_strassen(d22, l21, u12, min_dim);
//	mul_cmpfmatrix_strassen(d22, l21, u12, min_dim / 2);
//	mul_cmpfmatrix_strassen(d22, l21, u12, min_dim / 4);
	mul_cmpfmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);

#ifdef USE_SCALING
	mul_cmpfmatrix_mpfdiag(d22, diag_left, 0, d22, diag_right, 0); 

	mul_cmpfmatrix_mpfdiag(l21, diag_left, 1, l21, NULL, 0);
	mul_cmpfmatrix_mpfdiag(u12, NULL, 0, u12, diag_right, 1);
...
#elif USE_BLOCK
	mul_cmpfmatrix_block(d22, l21, u12, STRASSEN_MIN_DIM);
#else // defined(USE_STRASSEN) || defined(USE_WINOGRAD)
	mul_cmpfmatrix_simple(d22, l21, u12);
#endif // defined(USE_STRASSEN) || defined(USE_WINOGRAD)
...
//	mul_cmpfmatrix_strassen(d22, l21, u12, min_dim / 4);
//	mul_cmpfmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);
	_bncomp_mul_cmpfmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);
...
#elif USE_BLOCK
	//mul_cmpfmatrix_block(d22, l21, u12, STRASSEN_MIN_DIM);
	_bncomp_mul_cmpfmatrix_block(d22, l21, u12, STRASSEN_MIN_DIM);
#else // defined(USE_STRASSEN) || defined(USE_WINOGRAD)
	_bncomp_mul_cmpfmatrix_simple(d22, l21, u12);
#endif // defined(USE_STRASSEN) || defined(USE_WINOGRAD)
...
*/		l21 = init2_cmpfmatrix(row_dim, min_dim, a->prec);
		...
		// (4) D22 := L21 * U12
		// (5) A22 := A22 - D22
		CMPFLUdecomp_a22(a, d22, l21, u12, i, min_dim);
		//printf("D22:\n");
		//print_dmatrix(d22);
		//print_dmatrix(a);
...
*/		l21 = init2_cmpfmatrix(row_dim, min_dim, a->prec);
		...
		// (4) D22 := L21 * U12
		// (5) A22 := A22 - D22
		CMPFLUdecomp_a22_omp(a, d22, l21, u12, i, min_dim);
		//printf("D22:\n");
		//print_dmatrix(d22);
		//print_dmatrix(a);
...
/*                                  (Multi-Precision)       */
/*                                                          */
/*                 ver. 0.0 2024.02.13 (Mon) Tomonori Kouya */
/*                                                          */
/************************************************************/
int CMPFLUdecompPM_omp(CMPFMatrix a, long int ch[])
/************************************************************/
/*                                                          */
/* entries                                                  */
/*       CMPFMatrix a: Matrix (given by user)                */
...
/************************************************************/
{
	...
	{
		thread_index = omp_get_thread_num();
		mpc_init2(tmp[thread_index], a->prec);
	}
	...
	for(i = 0; i < dim; i++)
	{
		...
		for(j = (i + 1); j < dim; j++)
		{
			...
			mpc_div(tmp[thread_index], get_cmpfmatrix_ij(a, j, i), get_cmpfmatrix_ij(a, i, i), get_bnc_default_rounding_mode());
			set_cmpfmatrix_ij(a, j, i, tmp[thread_index]);
		}
		...
		for(j = (i + 1); j < dim; j++)
		{
			...
			for(k = (i + 1); k < dim; k++)
			{
				mpc_mul(tmp[thread_index], get_cmpfmatrix_ij(a, j, i), get_cmpfmatrix_ij(a, i, k), get_bnc_default_rounding_mode());
				mpc_sub(tmp[thread_index], get_cmpfmatrix_ij(a, j, k), tmp[thread_index], get_bnc_default_rounding_mode());
				set_cmpfmatrix_ij(a, j, k, tmp[thread_index]);
			}
		}
	}
