/********************************************************************************/
/* matmul_strassen_general_mpfcmplx.c:                                          */
...
long int num_addsub_mul_cmpfmatrix_strassen;	// addition and subtraction
long int num_mul_mul_cmpfmatrix_strassen;	// multiplication
...
void add_cmpfmatrix_partial(CMPFMatrix ret, long int ret_index[4], CMPFMatrix mat_a, long int mat_a_index[4], CMPFMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			add_mpfcmplx(tmp_val, get_cmpfmatrix_ij(mat_a, a_i, a_j), get_cmpfmatrix_ij(mat_b, b_i, b_j));
			set_cmpfmatrix_ij(ret, ret_i, ret_j, tmp_val);
		}
	}

	free_mpfcmplx(tmp_val);
}
...
void sub_cmpfmatrix_partial(CMPFMatrix ret, long int ret_index[4], CMPFMatrix mat_a, long int mat_a_index[4], CMPFMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			//tmp_val = get_cmpfmatrix_ij(mat_a, a_i, a_j) - get_cmpfmatrix_ij(mat_b, b_i, b_j);
			sub_mpfcmplx(tmp_val, get_cmpfmatrix_ij(mat_a, a_i, a_j), get_cmpfmatrix_ij(mat_b, b_i, b_j));
			set_cmpfmatrix_ij(ret, ret_i, ret_j, tmp_val);
		}
	}
...
void neg_cmpfmatrix_partial(CMPFMatrix ret, long int ret_index[4], CMPFMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			ret_j = ret_index[2] + j;
			a_j = mat_a_index[2] + j;
			//mpf_neg(tmp, get_cmpfmatrix_ij(mat_a, a_i, a_j));
			neg_mpfcmplx(tmp, get_cmpfmatrix_ij(mat_a, a_i, a_j));
...
void mul_cmpfmatrix_simple(CMPFMatrix c, CMPFMatrix a, CMPFMatrix b)
{
	...
	for(i = 0; i < c->row_dim; i++)
	{
		for(j = 0; j < c->col_dim; j++)
		{
			...
			{
				mul_mpfcmplx(tmp, get_cmpfmatrix_ij(a, i, k), get_cmpfmatrix_ij(b, k, j));
				add_mpfcmplx(tmp2, tmp2, tmp);
			}
...
void mul_cmpfmatrix_strassen_odd_peeling(CMPFMatrix ret, CMPFMatrix mat_a, CMPFMatrix mat_b, long int min_dim)
{
	...
	//mpf_init2(c22, prec);
	//mpf_init2(tmp, prec);
	a22 = init2_mpfcmplx(prec);
	b22 = init2_mpfcmplx(prec);
	c22 = init2_mpfcmplx(prec);
	...
	if(row_dim_h < row_dim)
	{
        ...
        a22 = init2_mpfcmplx(prec);
		if(mid_dim_h < mid_dim)
			subst_mpfcmplx(a22, get_cmpfmatrix_ij(mat_a, mat_a->row_dim - 1, mat_a->col_dim - 1));
	...
	if(col_dim_h < col_dim)
	{
		...
		b22 = init2_mpfcmplx(prec);
		if(mid_dim_h < mid_dim)
			subst_mpfcmplx(b22, get_cmpfmatrix_ij(mat_b, mat_b->row_dim - 1, mat_b->col_dim - 1));
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		for(i = 0; i < row_dim_h; i++)
		{
			for(j = 0; j < col_dim_h; j++)
			{
				mul_mpfcmplx(get_cmpfmatrix_ij(mat_tmp, i, j), get_cmpfvector_i(vec_a12, i), get_cmpfvector_i(vec_b21, j));
			}
		}
	...
	if((mpf_cmp_ui(abs_a22, 0UL) != 0) || (mpf_cmp_ui(abs_b22, 0UL) != 0))
	{
		...
		mul_mpfcmplx(tmp, a22, b22);
		add_mpfcmplx(c22, c22, tmp);
	}
	//printf("c22 += a22 * b22\n");
...
void left_scaling_cmpfmatrix(CMPFMatrix ret, CMPFMatrix org_mat, CMPFVector scaling_diag_mat, long int *ret_col_index)
{
	...
	for(i = 0; i < row_dim; i++)
	{
		...
		//mpf_abs(absmax_val, get_cmpfmatrix_ij(org_mat, i, 0));
		abs_mpfcmplx(absmax_val, get_cmpfmatrix_ij(org_mat, i, 0));
		for(j = 1; j < col_dim; j++)
		{
			//tmp_abs = fabs(get_cmpfmatrix_ij(org_mat, i, j));
			//mpf_abs(tmp_abs, get_cmpfmatrix_ij(org_mat, i, j));
            abs_mpfcmplx(tmp_abs, get_cmpfmatrix_ij(org_mat, i, j));
		...
		else
		{
			for(j = 0; j < col_dim; j++)
			{
				...
				div_mpfcmplx_real(ctmp, get_cmpfmatrix_ij(org_mat, i, j), absmax_val);
...
void right_scaling_cmpfmatrix(CMPFMatrix ret, CMPFMatrix org_mat, CMPFVector scaling_diag_mat, long int *ret_row_index)
{
	...
	for(i = 0; i < col_dim; i++)
	{
		...
		//mpf_abs(absmax_val, get_cmpfmatrix_ij(org_mat, 0, i));
		abs_mpfcmplx(absmax_val, get_cmpfmatrix_ij(org_mat, 0, i));
		for(j = 1; j < row_dim; j++)
		{
			//tmp_abs = fabs(get_cmpfmatrix_ij(org_mat, j, i));
			//mpf_abs(tmp_abs, get_cmpfmatrix_ij(org_mat, j, i));
			abs_mpfcmplx(tmp_abs, get_cmpfmatrix_ij(org_mat, j, i));
			if(mpf_cmp(absmax_val, tmp_abs) < 0)
			{
				absmax_row_index = j;
				mpf_set(absmax_val, tmp_abs);
		...
		else
		{
			for(j = 0; j < row_dim; j++)
			{
				...
				div_mpfcmplx_real(tmp, get_cmpfmatrix_ij(org_mat, j, i), absmax_val);
		...
		for(i = 0; i < row_dim; i++)
		{
			...
			if(left_inv_flag == 0)
			{
				for(j = 0; j < col_dim; j++)
				{
					...
					mul_mpfcmplx(tmp, get_cmpfvector_i(left_diag_mat, i), get_cmpfmatrix_ij(org_mat, i, j));
			...
			else
			{
				for(j = 0; j < col_dim; j++)
				{
					...
					div_mpfcmplx(tmp, get_cmpfmatrix_ij(org_mat, i, j), get_cmpfvector_i(left_diag_mat, i));
					set_cmpfmatrix_ij(ret, i, j, tmp);
				}
			}
		}
		...
		for(i = 0; i < row_dim; i++)
		{
			...
			if(right_inv_flag == 0)
			{
				for(j = 0; j < col_dim; j++)
				{
					...
					mul_mpfcmplx(tmp, get_cmpfmatrix_ij(ret, i, j), get_cmpfvector_i(right_diag_mat, j));
					set_cmpfmatrix_ij(ret, i, j, tmp);
				}
			}
			...
			else
			{
				...
				{
					//tmp = get_cmpfmatrix_ij(ret, i, j) / get_dvector_i(right_diag_mat, j);
					div_mpfcmplx(tmp, get_cmpfmatrix_ij(ret, i, j), get_cmpfvector_i(right_diag_mat, j));
					set_cmpfmatrix_ij(ret, i, j, tmp);
				}
			}
		}
...
int main(int argc, char *argv[])
{
	...
	mpf_t mpftmp[2];
	MPFCmplx mpctmp[2];
	CMPFMatrix mpfc, mpfa, mpfb, mpfc_normal;
...
//	set_bnc_default_prec_decimal(prec_d);
	...
	mpf_init(mpftmp[0]);
	mpf_init(mpftmp[1]);
	mpctmp[0] = init_mpfcmplx();
	mpctmp[1] = init_mpfcmplx();

	mpfc = init_cmpfmatrix(dim, dim);
	mpfc_normal = init_cmpfmatrix(dim, dim);
	mpfa = init_cmpfmatrix(dim, dim);
	mpfb = init_cmpfmatrix(dim, dim);

	mpfdiag_left = init_cmpfvector(dim);
	mpfdiag_right = init_cmpfvector(dim);
	...
	for(i = 0; i < dim; i++)
	{
		for(j = 0; j < dim; j++)
		{
			...
			set_real_mpfcmplx_d(mpctmp[0], (double)rand());
			set_image_mpfcmplx_d(mpctmp[0], (double)rand());
			if(rand() % 2 != 0)
				neg_mpfcmplx(mpctmp[0], mpctmp[0]);
				//mpf_neg(mpftmp[0], mpftmp[0]);

			//mpf_set_d(mpftmp[1], (double)rand());
			//mpf_ui_div(mpftmp[1], 1UL, mpftmp[1]);
			set_real_mpfxmplx_d(mpctmp[1], (double)rand());
			set_image_mpfxmplx_d(mpctmp[1], (double)rand());
			//mpf_ui_div(mpftmp[1], 1UL, mpftmp[1]);
			inv_mpfcmplx(mpctmp[1], mpctmp[1]);
			if(rand() % 2 != 0)
				neg_mpfcmplx(mpctmp[1], mpctmp[1]);
				//mpf_neg(mpftmp[1], mpftmp[1]);

			set_cmpfmatrix_ij(mpfa, i, j, mpctmp[0]);
			set_cmpfmatrix_ij(mpfb, i, j, mpctmp[1]);
		}
	}
...
//	mul_dmatrix_strassen(dc, da, db, 512);
	mul_cmpfmatrix_strassen(mpfc, mpfa, mpfb, 8);
//	mul_cmpfmatrix_strassen(mpfc, mpfa, mpfb, 16);
//	mul_cmpfmatrix_strassen(mpfc, mpfa, mpfb, 32);
	etime[0] = get_secv() - stime;

	mul_cmpfmatrix_mpfdiag(mpfc, mpfdiag_left, 0, mpfc, mpfdiag_right, 0);

	// difference
	sub_cmpfmatrix(mpfa, mpfc_normal, mpfc);

	// print
	printf("dim        : %ld\n", dim);
	printf("strassen   : %f\n", etime[0]);
	printf("normal     : %f\n", etime[1]);
	normi_cmpfmatrix(mpftmp[0], mpfa);
	normi_cmpfmatrix(mpftmp[1], mpfc_normal);
	div_mpfcmplx(mpftmp[0], mpftmp[0], mpftmp[1]);
...
//	lotkin_cmpfmatrix(mpfb, dim);
	...
	stime = get_secv();
	inv_cmpfmatrix_strassen_even(mpfc, mpfb, 32);
	etime[0] = get_secv() - stime;
...
//	print_dmatrix(dc);
	...
	// difference
	sub_cmpfmatrix(mpfb, mpfa, mpfc);
	normi_cmpfmatrix(mpftmp[0], mpfb);
	normi_cmpfmatrix(mpftmp[1], mpfa);
	div_mpfcmplx(mpftmp[0], mpftmp[0], mpftmp[1]);
	...
	printf("normal     : %f\n", etime[1]);
	printf("||a^(-1)|| strass : ");
		normi_cmpfmatrix(mpftmp[1], mpfc);
	...
	free_cmpfvector(mpfdiag_left);
	free_cmpfvector(mpfdiag_right);

	free_mpfcmplx(mpftmp[0]);
	free_mpfcmplx(mpftmp[1]);
