...
{
    long int i, real_total_dim = mat->real_row_dim * mat->real_col_dim;
#if defined(__AVX2__) && !defined(__AVX512F__)
    __m256d z = _mm256_setzero_pd();
    for(i = 0; i < real_total_dim; i += _BNC_D_WIDTH){
        _mm256_store_pd(&(mat->element[0][i]), z);
        _mm256_store_pd(&(mat->element[1][i]), z);
        _mm256_store_pd(&(mat->element[2][i]), z);
        _mm256_store_pd(&(mat->element[3][i]), z);
    }
#elif defined(__AVX512F__)
    __m512d z = _mm512_setzero_pd();
    for(i = 0; i < real_total_dim; i += _BNC_D_WIDTH){
        _mm512_store_pd(&(mat->element[0][i]), z);
        _mm512_store_pd(&(mat->element[1][i]), z);
        _mm512_store_pd(&(mat->element[2][i]), z);
        _mm512_store_pd(&(mat->element[3][i]), z);
    }
#elif defined(__ARM_NEON) || defined(__ARM_NEON__)
    float64x2_t z = vdupq_n_f64(0.0);
    for(i = 0; i < real_total_dim; i += _BNC_D_WIDTH){
...
QDMatrix init_qdmatrix(long int row_dim, long int col_dim)
{
    ...
    QDMatrix ret = (QDMatrix)BNC_MALLOC(sizeof(qdmatrix));
    if(!ret) return NULL;
    ...
    ret->real_row_dim = real_row_dim; ret->real_col_dim = real_col_dim;
...
void free_qdmatrix(QDMatrix mat)
{
    int i;
    if(!mat) return;
    for(i = 0; i < QDSIZE; i++) free(mat->element[i]);
    free(mat);
}
...
void mul_qdmatrix(QDMatrix ret, QDMatrix a, QDMatrix b)
{
...
#elif defined(__ARM_NEON) || defined(__ARM_NEON__)
    long real_row_dim = a->real_row_dim, real_col_dim = b->real_col_dim, real_mid_dim = a->real_col_dim;
    float64x2_t cij2[QDSIZE], aik2[QDSIZE], bkj2[QDSIZE], tmp_mul2[QDSIZE];
    double cijval2[QDSIZE];
    for(i = 0; i < a->row_dim; i++)
    {
        for(j = 0; j < b->col_dim; j++)
        {
            ...
            _bncneon_rqd_sum128d(cijval2, cij2);
            set_qdmatrix_ij(ret, i, j, cijval2);
        }
    }
#else // ifdef BNC_USE_GCC
	long real_row_dim, real_col_dim, real_mid_dim;
	double cijval[4][QDSIZE];
    //__m256d cij[QDSIZE], aik[QDSIZE], bkj[QDSIZE], tmp_mul[QDSIZE];
    qdfloat cij[_BNC_D_WIDTH], aik[_BNC_D_WIDTH], bkj[_BNC_D_WIDTH], tmp_mul[_BNC_D_WIDTH];
    ...
    for(i = 0; i < real_row_dim; i++)
    {
        ...
        {
            //rqd_set_ui(cij.val, 0UL);
            cij[0].val[0] = 0.0; cij[0].val[1] = 0.0; cij[0].val[2] = 0.0; cij[0].val[3] = 0.0;
            cij[1].val[0] = 0.0; cij[1].val[1] = 0.0; cij[1].val[2] = 0.0; cij[1].val[3] = 0.0;
            cij[2].val[0] = 0.0; cij[2].val[1] = 0.0; cij[2].val[2] = 0.0; cij[2].val[3] = 0.0;
            cij[3].val[0] = 0.0; cij[3].val[1] = 0.0; cij[3].val[2] = 0.0; cij[3].val[3] = 0.0;
			for(k = 0; k < real_mid_dim; k += _BNC_D_WIDTH)
            {
           
                aik[0].val[0] = a->element[0][i * real_mid_dim + k];
                aik[1].val[0] = a->element[0][i * real_mid_dim + k + 1];
                aik[2].val[0] = a->element[0][i * real_mid_dim + k + 2];
                aik[3].val[0] = a->element[0][i * real_mid_dim + k + 3];
            ...
            */
                ...
                aik[0].val[1] = a->element[1][i * real_mid_dim + k];
                aik[1].val[1] = a->element[1][i * real_mid_dim + k + 1];
                aik[2].val[1] = a->element[1][i * real_mid_dim + k + 2];
                aik[3].val[1] = a->element[1][i * real_mid_dim + k + 3];
            ...
            */
                ...
                aik[0].val[2] = a->element[2][i * real_mid_dim + k];
                aik[1].val[2] = a->element[2][i * real_mid_dim + k + 1];
                aik[2].val[2] = a->element[2][i * real_mid_dim + k + 2];
                aik[3].val[2] = a->element[2][i * real_mid_dim + k + 3];          

                aik[0].val[3] = a->element[3][i * real_mid_dim + k];
                aik[1].val[3] = a->element[3][i * real_mid_dim + k + 1];
                aik[2].val[3] = a->element[3][i * real_mid_dim + k + 2];
                aik[3].val[3] = a->element[3][i * real_mid_dim + k + 3];          
            ...
            */    
                ...
                bkj[0].val[0] = b->element[0][k * real_col_dim + j];
                bkj[1].val[0] = b->element[0][(k + 1) * real_col_dim + j];
                bkj[2].val[0] = b->element[0][(k + 2) * real_col_dim + j];
                bkj[3].val[0] = b->element[0][(k + 3) * real_col_dim + j];
			...
			*/
                ...
                bkj[0].val[1] = b->element[1][k * real_col_dim + j];
                bkj[1].val[1] = b->element[1][(k + 1) * real_col_dim + j];
                bkj[2].val[1] = b->element[1][(k + 2) * real_col_dim + j];
                bkj[3].val[1] = b->element[1][(k + 3) * real_col_dim + j];
            
                bkj[0].val[2] = b->element[2][k * real_col_dim + j];
                bkj[1].val[2] = b->element[2][(k + 1) * real_col_dim + j];
                bkj[2].val[2] = b->element[2][(k + 2) * real_col_dim + j];
                bkj[3].val[2] = b->element[2][(k + 3) * real_col_dim + j];
            
                bkj[0].val[3] = b->element[3][k * real_col_dim + j];
                bkj[1].val[3] = b->element[3][(k + 1) * real_col_dim + j];
                bkj[2].val[3] = b->element[3][(k + 2) * real_col_dim + j];
                bkj[3].val[3] = b->element[3][(k + 3) * real_col_dim + j];
			...
			*/
            ...
            //    _bncavx2_rqd_mul(tmp_mul, aik, bkj);
                ...
                rqd_add(cij[0].val, cij[0].val, tmp_mul[0].val);
                rqd_add(cij[1].val, cij[1].val, tmp_mul[1].val);
                rqd_add(cij[2].val, cij[2].val, tmp_mul[2].val);
                rqd_add(cij[3].val, cij[3].val, tmp_mul[3].val);
            
            //    _bncavx2_rqd_add(cij, cij, tmp_mul);
            }

            cijval[0][0] = cij[0].val[0]; cijval[0][1] = cij[1].val[0]; cijval[0][2] = cij[2].val[0]; cijval[0][3] = cij[3].val[0];
            cijval[1][0] = cij[0].val[1]; cijval[1][1] = cij[1].val[1]; cijval[1][2] = cij[2].val[1]; cijval[1][3] = cij[3].val[1];
            cijval[2][0] = cij[0].val[2]; cijval[2][1] = cij[1].val[2]; cijval[2][2] = cij[2].val[2]; cijval[2][3] = cij[3].val[2];
            cijval[3][0] = cij[0].val[3]; cijval[3][1] = cij[1].val[3]; cijval[3][2] = cij[2].val[3]; cijval[3][3] = cij[3].val[3];
            rqd_add(cijval[0], cijval[0], cijval[1]);
            rqd_add(cijval[0], cijval[0], cijval[2]);
            rqd_add(cijval[0], cijval[0], cijval[3]);
...
void add_qdmatrix(QDMatrix c, QDMatrix a, QDMatrix b)
{
    ...
    {
        for(j = 0; j < c->real_col_dim; j += _BNC_D_WIDTH)
        {
            float64x2_t v0 = vdupq_n_f64(0.0);
            float64x2_t v1 = vdupq_n_f64(0.0);
            float64x2_t v2 = vdupq_n_f64(0.0);
            float64x2_t v3 = vdupq_n_f64(0.0);
            v0 = vsetq_lane_f64(a->element[0][(j    ) * c->real_col_dim + i], v0, 0);
            v0 = vsetq_lane_f64(a->element[0][(j + 1) * c->real_col_dim + i], v0, 1);
            v1 = vsetq_lane_f64(a->element[1][(j    ) * c->real_col_dim + i], v1, 0);
            v1 = vsetq_lane_f64(a->element[1][(j + 1) * c->real_col_dim + i], v1, 1);
            v2 = vsetq_lane_f64(a->element[2][(j    ) * c->real_col_dim + i], v2, 0);
            v2 = vsetq_lane_f64(a->element[2][(j + 1) * c->real_col_dim + i], v2, 1);
            v3 = vsetq_lane_f64(a->element[3][(j    ) * c->real_col_dim + i], v3, 0);
            v3 = vsetq_lane_f64(a->element[3][(j + 1) * c->real_col_dim + i], v3, 1);
            vst1q_f64(&(c->element[0][i * c->real_col_dim + j]), v0);
            vst1q_f64(&(c->element[1][i * c->real_col_dim + j]), v1);
            vst1q_f64(&(c->element[2][i * c->real_col_dim + j]), v2);
            vst1q_f64(&(c->element[3][i * c->real_col_dim + j]), v3);
        }
    }
    ...
    for(i = 0; i < real_row_dim; i++)
    {
        for(j = 0; j < real_col_dim; j += _BNC_D_WIDTH)
        {
            ...
            vst1q_f64(&(c->element[3][index]), vld1q_f64(&(a->element[3][index])));
        }
    }
#else // BNC_USE_GCC
	__m256d tmp4[QDSIZE], aij4[QDSIZE], bij4[QDSIZE];
...
// SIMD
    ...
    for(j = 0; j < a->col_dim; j++)
    {
        ...
        {
            aij4[0] = vld1q_f64(&(a->element[0][i * a->real_col_dim + j]));
            aij4[1] = vld1q_f64(&(a->element[1][i * a->real_col_dim + j]));
            aij4[2] = vld1q_f64(&(a->element[2][i * a->real_col_dim + j]));
            aij4[3] = vld1q_f64(&(a->element[3][i * a->real_col_dim + j]));
