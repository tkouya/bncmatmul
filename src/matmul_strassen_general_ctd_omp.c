/********************************************************************************/
/* matmul_strassen_general_ctd_omp.c:                                           */
...
{
    _bncomp_add_tdmatrix_partial(ret->re, ret_index, mat_a->re, mat_a_index, mat_b->re, mat_b_index);
    _bncomp_add_tdmatrix_partial(ret->im, ret_index, mat_a->im, mat_a_index, mat_b->im, mat_b_index);
}
...
{
    _bncomp_sub_tdmatrix_partial(ret->re, ret_index, mat_a->re, mat_a_index, mat_b->re, mat_b_index);
    _bncomp_sub_tdmatrix_partial(ret->im, ret_index, mat_a->im, mat_a_index, mat_b->im, mat_b_index);
}
...
{
    _bncomp_subst_tdmatrix_partial(ret->re, ret_index, mat_a->re, mat_a_index);
    _bncomp_subst_tdmatrix_partial(ret->im, ret_index, mat_a->im, mat_a_index);
}
...
{
    _bncomp_subst_tdmatrix_partial_checked(ret->re, ret_index, mat_a->re, mat_a_index);
    _bncomp_subst_tdmatrix_partial_checked(ret->im, ret_index, mat_a->im, mat_a_index);
}
...
void _bncomp_mul_ctdmatrix_block_3m(CTDMatrix ret, CTDMatrix mat_a, CTDMatrix mat_b, long int min_dim)
{
    ...
    */
    ...
    if(\
        ...
        fprintf(stderr, "ERROR: _bncomp_mul_ctdmatrix_block_3m\n");
        return;
    }
    t1 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    t2 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    ...
    _bncomp_mul_tdmatrix_block(t1, mat_a->re, mat_b->re, min_dim);
    _bncomp_mul_tdmatrix_block(t2, mat_a->im, mat_b->im, min_dim);
    _bncomp_sub_tdmatrix(ret->re, t1, t2);

    // 4M
    /*
    #ifdef USE_4M
        _bncomp_mul_tdmatrix_block(t3, mat_a->im, mat_b->re);
        _bncomp_mul_tdmatrix_block(t4, mat_a->re, mat_b->im);
        _bncomp_add_tdmatrix(ret->im, t1, t2);
    #else // USE_4M
    */
        // 3M
        _bncomp_add_tdmatrix(t3, mat_a->re, mat_a->im);
        _bncomp_add_tdmatrix(t4, mat_b->re, mat_b->im);
        _bncomp_mul_tdmatrix_block(ret->im, t3, t4, min_dim);
...
void _bncomp_mul_ctdmatrix_block_4m(CTDMatrix ret, CTDMatrix mat_a, CTDMatrix mat_b, long int min_dim)
{
    ...
    */
    ...
    if(\
        ...
        fprintf(stderr, "ERROR: _bncomp_mul_ctdmatrix_block_4m\n");
        return;
    }
    t1 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    t2 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    ...
    // 4M
    //#ifdef USE_4M
        _bncomp_mul_tdmatrix_block(t3, mat_a->im, mat_b->re, min_dim);
        _bncomp_mul_tdmatrix_block(t4, mat_a->re, mat_b->im, min_dim);
...
void _bncomp_mul_ctdmatrix_strassen_3m(CTDMatrix ret, CTDMatrix mat_a, CTDMatrix mat_b, long int min_dim)
{
    ...
    */
    ...
    if(\
        ...
        fprintf(stderr, "ERROR: _bncomp_mul_ctdmatrix_strassen_3m\n");
        return;
    }
    t1 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    t2 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    ...
    _bncomp_mul_tdmatrix_strassen(t1, mat_a->re, mat_b->re, min_dim);
    _bncomp_mul_tdmatrix_strassen(t2, mat_a->im, mat_b->im, min_dim);
    _bncomp_sub_tdmatrix(ret->re, t1, t2);

    // 4M
    /*
    ifdef USE_4M
        _bncomp_mul_tdmatrix_strassen(t3, mat_a->im, mat_b->re, min_dim);
        _bncomp_mul_tdmatrix_strassen(t4, mat_a->re, mat_b->im, min_dim);
        _bncomp_add_tdmatrix(ret->im, t1, t2);
    #else // USE_4M
    */
        // 3M
        _bncomp_add_tdmatrix(t3, mat_a->re, mat_a->im);
        _bncomp_add_tdmatrix(t4, mat_b->re, mat_b->im);
        _bncomp_mul_tdmatrix_strassen(ret->im, t3, t4, min_dim);
        _bncomp_sub_tdmatrix(ret->im, ret->im, t1);
        _bncomp_sub_tdmatrix(ret->im, ret->im, t2);
    //#endif // USE_4M
...
void _bncomp_mul_ctdmatrix_strassen_4m(CTDMatrix ret, CTDMatrix mat_a, CTDMatrix mat_b, long int min_dim)
{
    ...
    */
    ...
    if(\
        ...
        fprintf(stderr, "ERROR: _bncomp_mul_ctdmatrix_strassen_4m\n");
        return;
    }
    t1 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
    t2 = init_tdmatrix(ret->re->row_dim, ret->re->col_dim);
