/********************************************************************************/
/* gddlinear.cc: Double-double and Quadruple precision                          */
...
/* Headers only — gddlinear.cu owns the implementations.  Cross-TU
 * resolution relies on -rdc=true / nvcc -dlink in src/cuda/Makefile.in. */
#include "gqd.cuh"
...
#endif // __cplusplus
{
	...
	*ret = 0.0;
#else // __cplusplus
	static double tmp[DDSIZE], sum[DDSIZE];
...
{
	long int i, j;
	static double tmp0[DDSIZE], tmp1[DDSIZE];
...
void inv_ddmatrix(DDMatrix a)
{
	...
	dd_real tmp, aii;
#else // __cplusplus
	static double tmp[DDSIZE], aii[DDSIZE];
#endif // __cplusplus
