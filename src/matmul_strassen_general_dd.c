...
long int num_addsub_mul_ddmatrix_strassen;	// addition and subtraction
long int num_mul_mul_ddmatrix_strassen;		// multiplication
...
#ifdef __cplusplus
	static dd_real a22, b22, c22, tmp;
#else // __cplusplus
...
#else
	mul_ddmatrix_strassen_even(mat_c11, mat_a11, mat_b11, min_dim);
#endif
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		for(i = 0; i < row_dim_h; i++)
		{
			for(j = 0; j < col_dim_h; j++)
			{
				...
				rdd_mul(tmp, get_ddvector_i(vec_a12, i), get_ddvector_i(vec_b21, j));
				set_ddmatrix_ij(mat_tmp, i, j, tmp);
			}
		}
...
#ifdef __cplusplus
	static dd_real absmax_val, tmp_abs;
#else // __cplusplus
...
int main(int argc, char *argv[])
{
	...
	double ddtmp[3][DDSIZE];
	DDMatrix ddc, dda, ddb, ddc_normal, ddc_block;
	DDVector dddiag_left, dddiag_right;
...
/* double-double precision */
	...
	ddc = init_ddmatrix(dim, dim);
	ddc_normal = init_ddmatrix(dim, dim);
	ddc_block = init_ddmatrix(dim, dim);
	dda = init_ddmatrix(dim, dim);
	ddb = init_ddmatrix(dim, dim);

	dddiag_left = init_ddvector(dim);
	dddiag_right = init_ddvector(dim);
...
//	printf("a(%ld x %ld) -> (%ld, %ld)\n", dda->row_dim, dda->col_dim, dda->real_row_dim, dda->real_col_dim);
//	printf("b(%ld x %ld) -> (%ld, %ld)\n", ddb->row_dim, ddb->col_dim, ddb->real_row_dim, ddb->real_col_dim);
//	printf("c(%ld x %ld) -> (%ld, %ld)\n", ddc->row_dim, ddc->col_dim, ddc->real_row_dim, ddc->real_col_dim);


	// normal matrix mul
//	printf("Normal matmul(%ld x %ld) ...", dda->row_dim, dda->col_dim);
	stime = get_secv();
	mul_ddmatrix(ddc_normal, dda, ddb);
	etime[0] = get_secv() - stime;
//	printf("end\n");

//	left_scaling_ddmatrix(dda, dda, dddiag_left, NULL);
//	right_scaling_ddmatrix(ddb, ddb, dddiag_right, NULL);

	// Strassen 
	stime = get_secv();
//	mul_ddmatrix_strassen(ddc, dda, ddb, 8);
//	mul_ddmatrix_strassen(ddc, dda, ddb, 16);
	mul_ddmatrix_strassen(ddc, dda, ddb, 32);
	etime[1] = get_secv() - stime;

//	mul_ddmatrix_dddiag(ddc, dddiag_left, 0, ddc, dddiag_right, 0);

	// difference
	sub_ddmatrix(ddc, ddc_normal, ddc);

	// Block
	stime = get_secv();
	mul_ddmatrix_block(ddc_block, dda, ddb, 32);
//	mul_ddmatrix_block(ddc_block, dda, ddb, 1024);
	etime[2] = get_secv() - stime;

	// difference
	sub_ddmatrix(ddc_block, ddc_normal, ddc_block);
	...
	printf("block      : %f\n", etime[2]);
	printf("normal     : %f\n", etime[0]);
	normi_ddmatrix(ddtmp[0], ddc);
	normi_ddmatrix(ddtmp[1], ddc_block);
	normi_ddmatrix(ddtmp[2], ddc_normal);
	rdd_div(ddtmp[0], ddtmp[0], ddtmp[2]);
	rdd_div(ddtmp[1], ddtmp[1], ddtmp[2]);
	printf("||reldiff_strassen||: "); rdd_out_str(ddtmp[0]); printf("\n");
	printf("||reldiff_block   ||: "); rdd_out_str(ddtmp[1]); printf("\n");

/* Inverse */

	frank_ddmatrix(dda, dim);
	frank_ddmatrix(ddb, dim);
//	lotkin_ddmatrix(dda, dim);
//	lotkin_ddmatrix(ddb, dim);
	...
	stime = get_secv();
	inv_ddmatrix_strassen_even(ddc, ddb, 32);
	etime[0] = get_secv() - stime;
...
//	print_dmatrix(dc);
	...
	// difference
	sub_ddmatrix(ddb, dda, ddc);
	normi_ddmatrix(ddtmp[0], ddb);
	normi_ddmatrix(ddtmp[1], dda);
	...
	free_ddmatrix(ddc);
	free_ddmatrix(ddc_normal);
	free_ddmatrix(ddc_block);
	free_ddmatrix(dda);
	free_ddmatrix(ddb);

	free_ddvector(dddiag_left);
	free_ddvector(dddiag_right);
