...
//	set_bnc_default_prec_decimal(prec_d);
	...
	for(i = 0; i < dim; i++)
	{
		for(j = 0; j < dim; j++)
		{
			...
			mpf_set_d_d(mpctmp[0], (double)rand(), (double)rand(), get_bnc_default_rounding_mode());
			if(rand() % 2 != 0)
				mpc_neg(mpctmp[0], mpctmp[0], get_bnc_default_rounding_mode());
			...
			mpc_ui_div(mpftmp[1], 1UL, mpftmp[1], get_bnc_default_rounding_mode());
			//inv_mpc_t(mpctmp[1], mpctmp[1]);
			if(rand() % 2 != 0)
				mpc_neg(mpctmp[1], mpctmp[1], get_bnc_default_rounding_mode());
...
//	mul_cmpfmatrix_strassen(mpfc, mpfa, mpfb, 32);
	...
	mpc_div(mpftmp[0], mpftmp[0], mpftmp[1]);
