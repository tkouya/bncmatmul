/********************************************************************************/
/* matmul_strassen_general_ds_omp.c:                                            */
...
long int _bncomp_num_addsub_mul_dsmatrix_strassen;	// addition and subtraction
long int _bncomp_num_mul_mul_dsmatrix_strassen;		// multiplication

// Total number of threads currently used in _bncomp_matmul_dsmatrix_strassen
...
int _bncomp_total_num_threads_matmul_dsmatrix_strassen = 0; // -> _bncomp_total_num_threads[MATMUL_DSMATRIX_STRASSEN] in matmul_general.h;
...
void _bncomp_add_dsmatrix_partial(DSMatrix ret, long int ret_index[4], DSMatrix mat_a, long int mat_a_index[4], DSMatrix mat_b, long int mat_b_index[4])
{
	...
	long int imax, jmax;
#ifdef __cplusplus
	static float tmp_val[BNCOMP_MAX_NUM_THREADS][DSSIZE];
#else // __cplusplus
	static float tmp_val[BNCOMP_MAX_NUM_THREADS][DSSIZE];
#endif // __cpplusplus
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			rds_add(tmp_val[thread_index], get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]), get_dsmatrix_ij(mat_b, b_i[thread_index], b_j[thread_index]));
			set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], tmp_val[thread_index]);
		}
	}
}
...
void _bncomp_sub_dsmatrix_partial(DSMatrix ret, long int ret_index[4], DSMatrix mat_a, long int mat_a_index[4], DSMatrix mat_b, long int mat_b_index[4])
{
	...
	static float tmp_val[BNCOMP_MAX_NUM_THREADS][DSSIZE];
#else // __cplusplus
	float tmp_val[BNCOMP_MAX_NUM_THREADS][DSSIZE];
#endif // _cplusplus
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			//tmp_val = get_dsmatrix_ij(mat_a, a_i, a_j) - get_dsmatrix_ij(mat_b, b_i, b_j);
			rds_sub(tmp_val[thread_index], get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]), get_dsmatrix_ij(mat_b, b_i[thread_index], b_j[thread_index]));
			set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], tmp_val[thread_index]);
		}
	}
}
...
void _bncomp_subst_dsmatrix_partial(DSMatrix ret, long int ret_index[4], DSMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
		}
	}
}
			...
			for(j = 0; j < jmax; j++)
			{
				...
				if((ret_j[thread_index] >= 0) && (ret_j[thread_index] < ret->col_dim))
				{
					if((a_i[thread_index] >= 0) && (a_i[thread_index] < mat_a->row_dim) && (a_j[thread_index] >= 0) && (a_j[thread_index] < mat_a->col_dim))
					{
						...
						set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
					#else // __cplusplus
						//ptr_dstmp = get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]);
						//set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], ptr_dstmp);
						set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
					#endif // __cplusplus
					}
					else
						set0_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index]); // Padding
...
void _bncomp_neg_dsmatrix_partial(DSMatrix ret, long int ret_index[4], DSMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			ret_j[thread_index] = ret_index[2] + j;
			a_j[thread_index] = mat_a_index[2] + j;
			rds_neg(tmp[thread_index], get_dsmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
			set_dsmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], tmp[thread_index]);
		}
	}
}

/* c = a * b */
//void _bncomp_mul_dsmatrix_simple(DSMatrix ret, DSMatrix a, DSMatrix b)
//{
//	_bncomp_mul_dsmatrix(ret, a, b);
//}
...
void _bncomp_mul_dsmatrix_block(DSMatrix ret, DSMatrix mat_a, DSMatrix mat_b, long int min_dim)
{
	...
	{
		fprintf(stderr, "ERROR: mat_a's col_dim %ld does not just fit mat_b's row_dim %ld!(_bncomp_mul_dsmatrix_block)\n", mat_a->col_dim, mat_b->row_dim);
		return;
	}
	...
	{
		_bncomp_mul_dsmatrix_simple(ret, mat_a, mat_b);
		return;
	}
	...
	small_ret = (DSMatrix *)calloc(num_div_col, sizeof(DSMatrix));
	small_mat_a = (DSMatrix *)calloc(num_div_mid, sizeof(DSMatrix));
	small_mat_b = (DSMatrix *)calloc(num_div_mid, sizeof(DSMatrix));
	small_tmp_mat = (DSMatrix *)calloc(num_div_mid, sizeof(DSMatrix));
	...
	for(i = 0; i < num_div_mid; i++)
	{
		...
		small_tmp_mat[i] = init_dsmatrix(min_dim, min_dim);
	}
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		for(j = 0; j < num_div_mid; j++)
		{
			...
			subst_dsmatrix_partial_checked(small_mat_a[j], small_mat_a_index, mat_a, mat_a_index[j]);

			//printf("small_mat_a[%ld]:\n", j); print_dsmatrix(small_mat_a[j]);
		}
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			for(k = 0; k < num_div_mid; k++)
			{
				...
				subst_dsmatrix_partial_checked(small_mat_b[k], small_mat_b_index, mat_b, mat_b_index[k]);
				//_bncomp_subst_dsmatrix_partial_checked(small_mat_b[k], small_mat_b_index, mat_b, mat_b_index[k]);
				// ret[j] += small_mat_a[i][k] * small_mat_b[k][j];
				mul_dsmatrix(small_tmp_mat[k], small_mat_a[k], small_mat_b[k]);
			}

			//for(k = 0; k < num_div_mid; k++)
			//	_bncomp_add_dsmatrix(small_ret[j], small_ret[j], small_tmp_mat[k]);
			for(k = 0; k < num_div_mid; k++)
				add_dsmatrix(small_ret[j], small_ret[j], small_tmp_mat[k]);

			ret_index[j][0] = i * min_dim;
			ret_index[j][1] = (i + 1) * min_dim;
			ret_index[j][2] = j * min_dim;
			ret_index[j][3] = (j + 1) * min_dim;
			subst_dsmatrix_partial_checked(ret, ret_index[j], small_ret[j], small_ret_index);
			//_bncomp_subst_dsmatrix_partial_checked(ret, ret_index[j], small_ret[j], small_ret_index);
		}
	}
...
void _bncomp_mul_dsmatrix_block_old(DSMatrix ret, DSMatrix mat_a, DSMatrix mat_b, long int min_dim)
{
	int thread_num, thread_index;
	int row_padding_flag = 0, col_padding_flag = 0, mid_padding_flag = 0;
	long i, j, k, row_dim, col_dim, mid_dim;
	...
	//DMatrix small_ret[1024], small_mat_a[1024], small_mat_b[1024], small_tmp_mat;
	DSMatrix *small_ret, *small_mat_a, *small_mat_b, small_tmp_mat[BNCOMP_MAX_NUM_THREADS];
	...
	{
		thread_index = omp_get_thread_num();
		small_tmp_mat[thread_index] = init_dsmatrix(min_dim, min_dim);
	}
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		for(j = 0; j < num_div_mid; j++)
		{
			...
			subst_dsmatrix_partial_checked(small_mat_a[j], small_mat_a_index[thread_index], mat_a, mat_a_index[thread_index]);
		}
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			for(k = 0; k < num_div_mid; k++)
			{
				...
				subst_dsmatrix_partial_checked(small_mat_b[k], small_mat_b_index[thread_index], mat_b, mat_b_index[thread_index]);

				// ret[j] += small_mat_a[i][k] * small_mat_b[k][j];
				mul_dsmatrix(small_tmp_mat[thread_index], small_mat_a[k], small_mat_b[k]);
				add_dsmatrix(small_ret[j], small_ret[j], small_tmp_mat[thread_index]);
			}
			...
			subst_dsmatrix_partial_checked(ret, ret_index[thread_index], small_ret[j], small_ret_index[thread_index]);
		}
	}
	...
	{
		thread_index = omp_get_thread_num();
		free_dsmatrix(small_tmp_mat[thread_index]);
	}
...
void _bncomp_mul_dsmatrix_strassen_odd_padding(DSMatrix ret, DSMatrix mat_a, DSMatrix mat_b, long int min_dim)
{
	...
	tmp_ret = _bncomp_init_static_padding_dsmatrix_strassen(ret);
	tmp_mat_a = _bncomp_init_static_padding_dsmatrix_strassen(mat_a);
	tmp_mat_b = _bncomp_init_static_padding_dsmatrix_strassen(mat_b);
#else
//	tmp_ret = _bncomp_init_dynamic_padding_dsmatrix_strassen(ret);
//	tmp_mat_a = _bncomp_init_dynamic_padding_dsmatrix_strassen(mat_a);
//	tmp_mat_b = _bncomp_init_dynamic_padding_dsmatrix_strassen(mat_b);
	tmp_ret = _bncomp_init_dynamic_padding_dsmatrix_strassen2(ret, min_dim);
	tmp_mat_a = _bncomp_init_dynamic_padding_dsmatrix_strassen2(mat_a, min_dim);
	tmp_mat_b = _bncomp_init_dynamic_padding_dsmatrix_strassen2(mat_b, min_dim);
#endif

	// strassen
#ifdef USE_WINOGRAD
//	_bncomp_mul_dsmatrix_winograd_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
	_bncomp_mul_dsmatrix_winograd_even_psec(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#else
//	_bncomp_mul_dsmatrix_strassen_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
	_bncomp_mul_dsmatrix_strassen_even_psec(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#endif
...
//	printf("    ret->row_dim, col_dim: %ld, %ld\n", ret->row_dim, ret->col_dim);
	...
	_bncomp_subst_dsmatrix_partial(ret, ret_index, tmp_ret, tmp_ret_index);
...
{
	_bncomp_num_addsub_mul_dsmatrix_strassen = 0;
	_bncomp_num_mul_mul_dsmatrix_strassen = 0;
	_bncomp_total_num_threads_matmul_dsmatrix_strassen = 0;
}
...
void _bncomp_get_num_mul_dsmatrix_strassen(long int *num_addsub, long int *num_mul)
{
	...
	if(num_addsub != NULL)
		*num_addsub = _bncomp_num_addsub_mul_dsmatrix_strassen;
	if(num_mul != NULL)
		*num_mul = _bncomp_num_mul_mul_dsmatrix_strassen;
}

// print counters
void _bncomp_print_num_mul_dsmatrix_strassen(long int *num_addsub, long int *num_mul)
{
	printf("_bncomp_num_addsub_mul_dsmatrix_strassen: %ld\n", _bncomp_num_addsub_mul_dsmatrix_strassen);
	printf("_bncomp_num_mul_mul_dsmatrix_strassen   : %ld\n", _bncomp_num_mul_mul_dsmatrix_strassen);

	_bncomp_get_num_mul_dsmatrix_strassen(num_addsub, num_mul);
...
//	DSVector diag_left, diag_right;
	...
	if((ret->row_dim <= min_dim) && (ret->col_dim <= min_dim) && (mid_dim <= min_dim))
	{
		...
		_bncomp_num_addsub_mul_dsmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
		_bncomp_num_mul_mul_dsmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);

		//mul_dsmatrix_simple(ret, mat_a, mat_b);
		_bncomp_mul_dsmatrix_simple(ret, mat_a, mat_b);
		//print_normf_dsmatrix("||ret1||_F = ", ret);

	/*	if(_bncomp_total_num_threads_matmul_dsmatrix_strassen >= MAX_TOTAL_NUM_THREADS)
		{
			_bncomp_total_num_threads_matmul_dsmatrix_strassen++;
			mul_dsmatrix(ret, mat_a, mat_b);
			_bncomp_total_num_threads_matmul_dsmatrix_strassen--;
		}
		else
		{
			_bncomp_total_num_threads_matmul_dsmatrix_strassen += thread_num;
			_bncomp_mul_dsmatrix_simple(ret, mat_a, mat_b);
			_bncomp_total_num_threads_matmul_dsmatrix_strassen -= thread_num;
		}
	*/
...
//	right_scaling_dsmatrix(mat_b, diag_right, NULL);
	...
	{
#ifdef PEELING_ONLY
		_bncomp_mul_dsmatrix_strassen_odd_peeling(ret, mat_a, mat_b, min_dim);
		...
		*/
#elif PADDING_ONLY
		_bncomp_mul_dsmatrix_strassen_odd_padding(ret, mat_a, mat_b, min_dim);
	...
	else
	{
...
#ifdef USE_WINOGRAD
		//_bncomp_mul_dsmatrix_winograd_even(ret, mat_a, mat_b, min_dim);
		_bncomp_mul_dsmatrix_winograd_even_psec(ret, mat_a, mat_b, min_dim);
		...
		*/
#else
		//_bncomp_mul_dsmatrix_strassen_even(ret, mat_a, mat_b, min_dim);
		_bncomp_mul_dsmatrix_strassen_even_psec(ret, mat_a, mat_b, min_dim);
...
void _bncomp_mul_dsmatrix_strassen_odd_peeling(DSMatrix ret, DSMatrix mat_a, DSMatrix mat_b, long int min_dim)
{
	...
	static float tmp_array[BNCOMP_MAX_NUM_THREADS][DSSIZE];
	...
	// C11 = A11 * B11
#ifdef USE_WINOGRAD
	//_bncomp_mul_dsmatrix_winograd_even(mat_c11, mat_a11, mat_b11, min_dim);
	_bncomp_mul_dsmatrix_winograd_even_psec(mat_c11, mat_a11, mat_b11, min_dim);
#else
//	_bncomp_mul_dsmatrix_strassen_even(mat_c11, mat_a11, mat_b11, min_dim);
	_bncomp_mul_dsmatrix_strassen_even_psec(mat_c11, mat_a11, mat_b11, min_dim);
#endif
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += row_dim_h * col_dim_h;
		_bncomp_num_mul_mul_dsmatrix_strassen += row_dim_h * col_dim_h;
		...
		for(i = 0; i < row_dim_h; i++)
		{
			...
			for(j = 0; j < col_dim_h; j++)
			{
				...
				rds_mul(tmp_array[thread_index], get_dsvector_i(vec_a12, i), get_dsvector_i(vec_b21, j));
				set_dsmatrix_ij(mat_tmp, i, j, tmp_array[thread_index]);
			}
		}
		_bncomp_add_dsmatrix(mat_c11, mat_c11, mat_tmp);
		//printf("C11 += a12 * b21...\n");
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;
		_bncomp_num_mul_mul_dsmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;
		...
		_bncomp_mul_dsmatrix_dsvec(vec_c12, mat_a11, vec_b12);
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += vec_c12->dim;
		_bncomp_num_mul_mul_dsmatrix_strassen += vec_tmp12->dim;

		_bncomp_cmul_dsvector(vec_tmp12, b22, vec_a12);
		_bncomp_add_dsvector(vec_c12, vec_c12, vec_tmp12);
	}
	//printf("c12 += b22 * a12\n");
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;
		_bncomp_num_mul_mul_dsmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;
		...
		_bncomp_mul_dsmatrixt_dsvec(vec_c21, mat_b11, vec_a21);
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += vec_c21->dim;
		_bncomp_num_mul_mul_dsmatrix_strassen += vec_tmp21->dim;

		_bncomp_cmul_dsvector(vec_tmp21, a22, vec_b21);
		_bncomp_add_dsvector(vec_c21, vec_c21, vec_tmp21);
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += vec_a21->dim;
		_bncomp_num_mul_mul_dsmatrix_strassen += vec_tmp21->dim;

#ifdef __cplusplus
		_bncomp_ip_dsvector(c22, vec_a21, vec_b12);
#else
		_bncomp_ip_dsvector(c22, vec_a21, vec_b12);
#endif // __cplusplus
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_dsmatrix_strassen += 1;
		_bncomp_num_mul_mul_dsmatrix_strassen += 1;
...
//	printf("P1a:\n");
	...
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_dsmatrix_strassen += row_dim_h * mid_dim_h;

//	add_dsmatrix_partial(mat_tmp_a[0], ret_index, mat_a, mat_ar_index[0], mat_a, mat_al_index[0]);
	_bncomp_add_dsmatrix_partial(mat_tmp_a[0], mat_tmp_a_index, mat_a, mat_ar_index[0], mat_a, mat_al_index[0]);
...
//	printf("P1b:\n");
	...
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_dsmatrix_strassen += mid_dim_h * col_dim_h;

//	add_dsmatrix_partial(mat_tmp_b[0], ret_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
	_bncomp_add_dsmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);

	// P1 = tmp_a * tmp_b
	printf("P1:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim);

	print_dsmatrix(mat_p[0]);
...
//	add_dsmatrix_partial(mat_tmp_a[1], ret_index, mat_a, mat_ar_index[1], mat_a, mat_al_index[1]);
	_bncomp_add_dsmatrix_partial(mat_tmp_a[1], mat_tmp_a_index, mat_a, mat_ar_index[1], mat_a, mat_al_index[1]);
...
//	subst_dsmatrix_partial(mat_tmp_b[1], ret_index, mat_b, mat_br_index[1]);
	_bncomp_subst_dsmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);

	// P2 = tmp_a * tmp_b
	//printf("P2:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim);
	print_dsmatrix(mat_p[1]);
...
//	subst_dsmatrix_partial(mat_tmp_a[2], ret_index, mat_a, mat_ar_index[2]);
	_bncomp_subst_dsmatrix_partial(mat_tmp_a[2], mat_tmp_a_index, mat_a, mat_ar_index[2]);
...
//	sub_dsmatrix_partial(mat_tmp_b[2], ret_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
	_bncomp_sub_dsmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);

	// P3 = tmp_a * tmp_b
	//printf("P3:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim);
	print_dsmatrix(mat_p[2]);
...
//	subst_dsmatrix_partial(mat_tmp_a[3], ret_index, mat_a, mat_ar_index[3]);
	_bncomp_subst_dsmatrix_partial(mat_tmp_a[3], mat_tmp_a_index, mat_a, mat_ar_index[3]);
...
//	sub_dsmatrix_partial(mat_tmp_b[3], ret_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
	_bncomp_sub_dsmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);

	// P4 = tmp_a * tmp_b
	//printf("P4:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim);
	print_dsmatrix(mat_p[3]);
...
//	add_dsmatrix_partial(mat_tmp_a[4], ret_index, mat_a, mat_ar_index[4], mat_a, mat_al_index[4]);
	_bncomp_add_dsmatrix_partial(mat_tmp_a[4], mat_tmp_a_index, mat_a, mat_ar_index[4], mat_a, mat_al_index[4]);
...
//	subst_dsmatrix_partial(mat_tmp_b[4], ret_index, mat_b, mat_br_index[4]);
	_bncomp_subst_dsmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);

	// P5 = tmp_a * tmp_b
	//printf("P5:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim);
	print_dsmatrix(mat_p[4]);
...
//	sub_dsmatrix_partial(mat_tmp_a[5], ret_index, mat_a, mat_ar_index[5], mat_a, mat_al_index[5]);
	_bncomp_sub_dsmatrix_partial(mat_tmp_a[5], mat_tmp_a_index, mat_a, mat_ar_index[5], mat_a, mat_al_index[5]);
...
//	add_dsmatrix_partial(mat_tmp_b[5], ret_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
	_bncomp_add_dsmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);

	// P6 = tmp_a * tmp_b
	//printf("P6:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim);
	print_dsmatrix(mat_p[5]);
...
//	sub_dsmatrix_partial(mat_tmp_a[6], ret_index, mat_a, mat_ar_index[6], mat_a, mat_al_index[6]);
	_bncomp_sub_dsmatrix_partial(mat_tmp_a[6], mat_tmp_a_index, mat_a, mat_ar_index[6], mat_a, mat_al_index[6]);
...
//	add_dsmatrix_partial(mat_tmp_b[6], ret_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
	_bncomp_add_dsmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);

	// P7 = tmp_a * tmp_b
	//printf("P7:\n");
	_bncomp_mul_dsmatrix_strassen(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim);
	print_dsmatrix(mat_p[6]);

	// -------------------------------
	// C11 := P1 + P4 - P5 + P7
	// -------------------------------
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_dsmatrix_strassen += 3 * row_dim_h * col_dim_h;
	...
	_bncomp_add_dsmatrix(mat_tmp_c[0], mat_p[0], mat_p[3]);
	_bncomp_sub_dsmatrix(mat_tmp_c[0], mat_tmp_c[0], mat_p[4]);
	_bncomp_add_dsmatrix(mat_tmp_c[0], mat_tmp_c[0], mat_p[6]);
	mat_c_index[0][0] = 0;
	...
	_bncomp_subst_dsmatrix_partial(ret, mat_c_index[0], mat_tmp_c[0], ret_index);
	...
	_bncomp_add_dsmatrix(mat_tmp_c[1], mat_p[2], mat_p[4]);
	mat_c_index[1][0] = 0;
	...
	_bncomp_subst_dsmatrix_partial(ret, mat_c_index[1], mat_tmp_c[1], ret_index);
	...
	_bncomp_add_dsmatrix(mat_tmp_c[2], mat_p[1], mat_p[3]);
	...
	_bncomp_subst_dsmatrix_partial(ret, mat_c_index[2], mat_tmp_c[2], ret_index);
	...
	_bncomp_add_dsmatrix(mat_tmp_c[3], mat_p[0], mat_p[2]);
	_bncomp_sub_dsmatrix(mat_tmp_c[3], mat_tmp_c[3], mat_p[1]);
	_bncomp_add_dsmatrix(mat_tmp_c[3], mat_tmp_c[3], mat_p[5]);
	...
	_bncomp_subst_dsmatrix_partial(ret, mat_c_index[3], mat_tmp_c[3], ret_index);
...
#pragma omp parallel sections //num_threads(7)
{
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim);
		//print_normf_dsmatrix("||P1||_F = ", mat_p[0]);
	...
	//	_bncomp_subst_dsmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim);
		//print_normf_dsmatrix("||P2||_F = ", mat_p[1]);
	}
	...
	//	_bncomp_sub_dsmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim);
		//print_normf_dsmatrix("||P3||_F = ", mat_p[2]);
	...
	//	_bncomp_sub_dsmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim);
		//print_normf_dsmatrix("||P4||_F = ", mat_p[3]);
	...
	//	_bncomp_subst_dsmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim);
		//print_normf_dsmatrix("||P5||_F = ", mat_p[4]);
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim);
		//print_normf_dsmatrix("||P6||_F = ", mat_p[5]);
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
		...
		_bncomp_mul_dsmatrix_strassen(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim);
		//print_normf_dsmatrix("||P7||_F = ", mat_p[6]);
...
#pragma omp parallel sections
{
	...
	#pragma omp section
	{
		...
		subst_dsmatrix_partial(ret, mat_c_index[0], mat_tmp_c[0], ret_index);
		//_bncomp_subst_dsmatrix_partial(ret, mat_c_index[0], mat_tmp_c[0], ret_index);
		//print_normf_dsmatrix("||c11||_F = ", ret);
	}
	...
	#pragma omp section
	{
		...
		subst_dsmatrix_partial(ret, mat_c_index[1], mat_tmp_c[1], ret_index);
		//_bncomp_subst_dsmatrix_partial(ret, mat_c_index[1], mat_tmp_c[1], ret_index);
		//print_normf_dsmatrix("||c12||_F = ", ret);
	}
	...
	#pragma omp section
	{
		...
		subst_dsmatrix_partial(ret, mat_c_index[2], mat_tmp_c[2], ret_index);
		//_bncomp_subst_dsmatrix_partial(ret, mat_c_index[2], mat_tmp_c[2], ret_index);
		//print_normf_dsmatrix("||c21||_F = ", ret);
	}
	...
	#pragma omp section
	{
		...
		subst_dsmatrix_partial(ret, mat_c_index[3], mat_tmp_c[3], ret_index);
		//_bncomp_subst_dsmatrix_partial(ret, mat_c_index[3], mat_tmp_c[3], ret_index);
		//print_normf_dsmatrix("||c22||_F = ", ret);
	}
...
//	if((ret->row_dim <= min_dim) && (ret->col_dim <= min_dim) && (mid_dim <= min_dim))
	...
	for(i = 0; i < 4; i++)
	{
		_bncomp_subst_dsmatrix_partial(mat_tmp_a[i], mat_tmp_a_index, mat_a, mat_a_index[i]);
		_bncomp_subst_dsmatrix_partial(mat_tmp_b[i], mat_tmp_b_index, mat_b, mat_b_index[i]);
	}
//	printf("subst a, b...\n");
	...
	_bncomp_add_dsmatrix(mat_s[0], mat_tmp_a[2], mat_tmp_a[3]);
	...
	_bncomp_sub_dsmatrix(mat_s[1], mat_s[0], mat_tmp_a[0]);
	...
	_bncomp_sub_dsmatrix(mat_s[2], mat_tmp_a[0], mat_tmp_a[2]);
	...
	_bncomp_sub_dsmatrix(mat_s[3], mat_tmp_a[1], mat_s[1]);
	...
	_bncomp_sub_dsmatrix(mat_s[4], mat_tmp_b[1], mat_tmp_b[0]);
	...
	_bncomp_sub_dsmatrix(mat_s[5], mat_tmp_b[3], mat_s[4]);
	...
	_bncomp_sub_dsmatrix(mat_s[6], mat_tmp_b[3], mat_tmp_b[1]);
	...
	_bncomp_sub_dsmatrix(mat_s[7], mat_s[5], mat_tmp_b[2]);
...
//	printf("s...\n");
	...
	// M1 := S2 * S6
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[0], mat_s[1], mat_s[5], min_dim);

	// -------------------------------
	// M2 := A11 * B11
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[1], mat_tmp_a[0], mat_tmp_b[0], min_dim);

	// -------------------------------
	// M3 := A12 * B21
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[2], mat_tmp_a[1], mat_tmp_b[2], min_dim);

	// -------------------------------
	// M4 := S3 * S7
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[3], mat_s[2], mat_s[6], min_dim);

	// -------------------------------
	// M5 := S1 * S5
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[4], mat_s[0], mat_s[4], min_dim);

	// -------------------------------
	// M6 := S4 * B22
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[5], mat_s[3], mat_tmp_b[3], min_dim);

	// -------------------------------
	// M7 := A22 * S8
	// -------------------------------
	_bncomp_mul_dsmatrix_strassen(mat_m[6], mat_tmp_a[3], mat_s[7], min_dim);
...
//	printf("m...\n");
	...
	_bncomp_add_dsmatrix(mat_t[0], mat_m[0], mat_m[1]);
	...
	_bncomp_add_dsmatrix(mat_t[1], mat_t[0], mat_m[3]);
...
//	printf("t...\n");
	...
	_bncomp_add_dsmatrix(mat_tmp_c[0], mat_m[1], mat_m[2]);

	// -------------------------------
	// C12 := T1 + M5 + M6
	// -------------------------------
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_dsmatrix_strassen += 2 * row_dim_h * col_dim_h;

	_bncomp_add_dsmatrix(mat_tmp_c[1], mat_t[0], mat_m[4]);
	_bncomp_add_dsmatrix(mat_tmp_c[1], mat_tmp_c[1], mat_m[5]);
	...
	_bncomp_sub_dsmatrix(mat_tmp_c[2], mat_t[1], mat_m[6]);
	...
	_bncomp_add_dsmatrix(mat_tmp_c[3], mat_t[1], mat_m[4]);
...
//	printf("c...\n");
	...
	// -------------------------------
	for(i = 0; i < 4; i++)
		_bncomp_subst_dsmatrix_partial(ret, mat_c_index[i], mat_tmp_c[i], ret_index);
...
void _bncomp_mul_dsmatrix_strassen_nonrec(DSMatrix ret, DSMatrix mat_a, DSMatrix mat_b, long int min_dim)
{
	...
	DSMatrix a11, a12, a21, a22;
	DSMatrix b11, b12, b21, b22;
	DSMatrix ret11, ret12, ret21, ret22, tmp_mat;
	...
	if(mid_dim != mat_b->row_dim)
	{
		fprintf(stderr, "ERROR: mat_a's col_dim %ld does not just fit mat_b's row_dim %ld!(_bncomp_mul_dsmatrix_strassen_nonrec)\n", mat_a->col_dim, mat_b->row_dim);
		return;
	}
...
*/
	...
	{
		//_bncomp_mul_dsmatrix_block(ret, mat_a, mat_b, min_dim);
		_bncomp_mul_dsmatrix_simple(ret, mat_a, mat_b);
		return;
	}
	...
	// Strassen Area
	a11 = init_dsmatrix(a11_row_dim, a11_col_dim);
	...
	_bncomp_subst_dsmatrix_partial_checked(a11, a11_index, mat_a, mat_a_index[0]);

	a12 = NULL;
	if(a12_col_dim > 0)
	{
		a12 = init_dsmatrix(a12_row_dim, a12_col_dim);
		a12_index[0] = 0;
		...
		_bncomp_subst_dsmatrix_partial_checked(a12, a12_index, mat_a, mat_a_index[1]);
	}

	a21 = NULL;
	if(a21_row_dim > 0)
	{
		a21 = init_dsmatrix(a21_row_dim, a21_col_dim);
		a21_index[0] = 0;
		...
		_bncomp_subst_dsmatrix_partial_checked(a21, a21_index, mat_a, mat_a_index[2]);
	}

	a22 = NULL;
	if((a22_row_dim > 0) && (a22_col_dim > 0))
	{
		a22 = init_dsmatrix(a22_row_dim, a22_col_dim);
		a22_index[0] = 0;
		...
		_bncomp_subst_dsmatrix_partial_checked(a22, a22_index, mat_a, mat_a_index[3]);
	}
	...
	// Strassen Area
	b11 = init_dsmatrix(b11_row_dim, b11_col_dim);
	...
	_bncomp_subst_dsmatrix_partial_checked(b11, b11_index, mat_b, mat_b_index[0]);

	b12 = NULL;
	if(b12_col_dim > 0)
	{
		b12 = init_dsmatrix(b12_row_dim, b12_col_dim);
		b12_index[0] = 0;
		...
		_bncomp_subst_dsmatrix_partial_checked(b12, b12_index, mat_b, mat_b_index[1]);
	}

	b21 = NULL;
	if(b21_row_dim > 0)
	{
		b21 = init_dsmatrix(b21_row_dim, b21_col_dim);
		b21_index[0] = 0;
		...
		_bncomp_subst_dsmatrix_partial_checked(b21, b21_index, mat_b, mat_b_index[2]);
	}

	b22 = NULL;
	if((b22_row_dim > 0) && (b22_col_dim > 0))
	{
		b22 = init_dsmatrix(b22_row_dim, b22_col_dim);
		b22_index[0] = 0;
		...
		_bncomp_subst_dsmatrix_partial_checked(b22, b22_index, mat_b, mat_b_index[3]);
	}	
	...
	ret11 = init_dsmatrix(ret11_row_dim, ret11_col_dim);

	// Strassen Area
	// a11 * b11
#ifdef USE_WINOGRAD
//	_bncomp_mul_dsmatrix_winograd_even(ret11, a11, b11, min_dim);
	_bncomp_mul_dsmatrix_winograd_even2(ret11, a11, b11, min_dim);
#else
	//#pragma omp parallel
//	_bncomp_mul_dsmatrix_strassen_even3(ret11, a11, b11, min_dim, 0);
	_bncomp_mul_dsmatrix_strassen_even2(ret11, a11, b11, min_dim, 0);
//	_bncomp_mul_dsmatrix_strassen_even(ret11, a11, b11, min_dim);
#endif
	//_bncomp_mul_dsmatrix_block(ret11, a11, b11, min_dim);
	//_bncomp_mul_dsmatrix_simple(ret11, a11, b11);
	
	// ret11 += a12 * b21
	if((a12 != NULL) && (b21 != NULL))
	{
		tmp_mat = init_dsmatrix(ret11_row_dim, ret11_col_dim);

		//_bncomp_mul_dsmatrix_block(tmp_mat, a12, b21, min_dim);
		_bncomp_mul_dsmatrix_simple(tmp_mat, a12, b21);
		_bncomp_add_dsmatrix(ret11, ret11, tmp_mat);
	...
	_bncomp_subst_dsmatrix_partial_checked(ret, ret_index, ret11, ret11_index);
	...
	if(ret12_col_dim > 0)
	{
		ret12 = init_dsmatrix(ret12_row_dim, ret12_col_dim);

		// a11 * b12
		if(b12 != NULL)
			_bncomp_mul_dsmatrix_simple(ret12, a11, b12);
		//	_bncomp_mul_dsmatrix_block(ret12, a11, b12, min_dim);

		// ret12 += a12 * b22
		if((a12 != NULL) && (b22 != NULL))
		{
			tmp_mat = init_dsmatrix(ret12_row_dim, ret12_col_dim);

		//	_bncomp_mul_dsmatrix_block(tmp_mat, a12, b22, min_dim);
			_bncomp_mul_dsmatrix_simple(tmp_mat, a12, b22);
			_bncomp_add_dsmatrix(ret12, ret12, tmp_mat);
		...
		_bncomp_subst_dsmatrix_partial_checked(ret, ret_index, ret12, ret12_index);
	...
	if(ret21_row_dim > 0)
	{
		ret21 = init_dsmatrix(ret21_row_dim, ret21_col_dim);

		// a21 * b11
		if(a21 != NULL)
			_bncomp_mul_dsmatrix_simple(ret21, a21, b11);
		//	_bncomp_mul_dsmatrix_block(ret21, a21, b11, min_dim);

		// ret21 += a22 * b21
		if((a22 != NULL) && (b21 != NULL))
		{
			tmp_mat = init_dsmatrix(ret21_row_dim, ret21_col_dim);
			
			//_bncomp_mul_dsmatrix_block(tmp_mat, a22, b21, min_dim);
			_bncomp_mul_dsmatrix_simple(tmp_mat, a22, b21);
			_bncomp_add_dsmatrix(ret21, ret21, tmp_mat);
		...
		_bncomp_subst_dsmatrix_partial_checked(ret, ret_index, ret21, ret21_index);
	...
	{
		ret22 = init_dsmatrix(ret22_row_dim, ret22_col_dim);

		// a21 * b12
		if((a21 != NULL) && (b12 != NULL))
			_bncomp_mul_dsmatrix_simple(ret22, a21, b12);
		//	_bncomp_mul_dsmatrix_block(ret22, a21, b12, min_dim);

		// ret22 += a22 * b22
		if((a22 != NULL) && (b22 != NULL))
		{
			tmp_mat = init_dsmatrix(ret22_row_dim, ret22_col_dim);
			
			//_bncomp_mul_dsmatrix_block(tmp_mat, a22, b22, min_dim);
			_bncomp_mul_dsmatrix_simple(tmp_mat, a22, b22);
			_bncomp_add_dsmatrix(ret22, ret22, tmp_mat);
		...
		_bncomp_subst_dsmatrix_partial_checked(ret, ret_index, ret22, ret22_index);
...
*/
	...
	free_dsmatrix(a11);
	free_dsmatrix(b11);
	if(a12 != NULL) free_dsmatrix(a12);
	if(a21 != NULL) free_dsmatrix(a21);
	if(a22 != NULL) free_dsmatrix(a22);
	if(b12 != NULL) free_dsmatrix(b12);
	if(b21 != NULL) free_dsmatrix(b21);
	if(b22 != NULL) free_dsmatrix(b22);
...
#pragma omp parallel sections shared(mat_tmp_a, mat_tmp_b, mat_a, mat_b, mat_br_index, mat_bl_index, mat_ar_index, mat_al_index, row_dim, col_dim, mid_dim, row_dim_h, col_dim_h, mid_dim_h, mat_tmp_a_index, mat_tmp_b_index, ret_index) //num_threads(7)
{
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
		...
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_dsmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);
		...
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_dsmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
		...
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_dsmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
		...
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_dsmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);
		...
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim, rec_num + 1);
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
		...
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim, rec_num + 1);
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
		...
		// P7 = tmp_a * tmp_b
		//printf("P7: %ld\n", rec_num);
		_bncomp_mul_dsmatrix_strassen_even2(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim, rec_num + 1);
...
//#pragma omp parallel
{
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
		...
		//printf("P1: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_dsmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);
		...
		//printf("P2: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_dsmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
		...
		//printf("P3: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_dsmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
		...
		//printf("P4: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_dsmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);
		...
		//printf("P5: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim, rec_num + 1);
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
		...
		//printf("P6: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim, rec_num + 1);
	...
	//	_bncomp_add_dsmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
		...
		//printf("P7: %ld\n", rec_num);
		#pragma omp task
		_bncomp_mul_dsmatrix_strassen_even3(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim, rec_num + 1);
...
#pragma omp parallel sections
{
	#pragma omp section
	{
		...
		// M1 := S2 * S6
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[0], mat_s[1], mat_s[5], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M2 := A11 * B11
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[1], mat_tmp_a[0], mat_tmp_b[0], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M3 := A12 * B21
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[2], mat_tmp_a[1], mat_tmp_b[2], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M4 := S3 * S7
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[3], mat_s[2], mat_s[6], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M5 := S1 * S5
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[4], mat_s[0], mat_s[4], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M6 := S4 * B22
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[5], mat_s[3], mat_tmp_b[3], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M7 := A22 * S8
		// -------------------------------
		_bncomp_mul_dsmatrix_winograd_even2(mat_m[6], mat_tmp_a[3], mat_s[7], min_dim);
	}
} // pragma omp parallel sections
...
int main(int argc, char *argv[])
{
	...
	float dstmp[4][DSSIZE];
	DSMatrix mpfc, mpfa, mpfb, mpfc_normal, mpfc_block, mpfc_tmp;
	DSVector mpfdiag_left, mpfdiag_right;
...
//	printf("OpenMP #Threads = %ld\n", omp_get_num_threads());
	...
	mpfc = init_dsmatrix(row_dim, col_dim);
	mpfc_tmp = init_dsmatrix(row_dim, col_dim);
	mpfc_normal = init_dsmatrix(row_dim, col_dim);
	mpfc_block = init_dsmatrix(row_dim, col_dim);
	mpfa = init_dsmatrix(row_dim, mid_dim);
	mpfb = init_dsmatrix(mid_dim, col_dim);
...
//	mpfdiag_right = init_dsvector(dim);
	...
	for(i = 0; i < row_dim; i++)
	{
		for(j = 0; j < mid_dim; j++)
		{
			...
			set_dsmatrix_ij(mpfa, i, j, dstmp[0]);
		}
	}
	...
	for(i = 0; i < mid_dim; i++)
	{
		for(j = 0; j < col_dim; j++)
		{
			...
			set_dsmatrix_ij(mpfb, i, j, dstmp[1]);
		}
	}

	// normal matrix mul
	printf("_bncomp_mul_dsmatrix_simple...\n");
	stime = get_real_secv();
	_bncomp_mul_dsmatrix(mpfc_normal, mpfa, mpfb);
	etime[0] = get_real_secv() - stime;

	//left_scaling_dsmatrix(mpfa, mpfa, mpfdiag_left, NULL);
	//right_scaling_dsmatrix(mpfb, mpfb, mpfdiag_right, NULL);

	// blocked matrix mul
	printf("_bncomp_mul_dsmatrix_block(%ld)...\n", min_dim);
	//stime = get_secv();
	stime = get_real_secv();
	_bncomp_mul_dsmatrix_block(mpfc_block, mpfa, mpfb, min_dim);
	//etime[1] = get_secv() - stime;
	etime[1] = get_real_secv() - stime;

	// Strassen 
	printf("_bncomp_mul_dsmatrix_strassen(%ld)...\n", min_dim);
	stime = get_real_secv();
	_bncomp_mul_dsmatrix_strassen(mpfc, mpfa, mpfb, min_dim);
//	mul_dsmatrix_strassen(mpfc, mpfa, mpfb, 16);
//	mul_dsmatrix_strassen(mpfc, mpfa, mpfb, 32);
	etime[2] = get_real_secv() - stime;

	//mul_dsmatrix_dsdiag(mpfc, mpfdiag_left, 0, mpfc, mpfdiag_right, 0);

	// difference
	sub_dsmatrix(mpfc_tmp  , mpfc_normal, mpfc);
	sub_dsmatrix(mpfc      , mpfc_normal, mpfc_block);
	...
	normi_dsmatrix(dstmp[0], mpfc_tmp);
	normi_dsmatrix(dstmp[1], mpfc);
	normi_dsmatrix(dstmp[2], mpfc_block);
	normi_dsmatrix(dstmp[3], mpfc_normal);

	rds_div(dstmp[0], dstmp[0], dstmp[3]);
	rds_div(dstmp[1], dstmp[1], dstmp[3]);
	printf("||reldiff_block   ||       : "); rds_out_str(dstmp[1]); printf("\n");
	printf("||reldiff_strassen||       : "); rds_out_str(dstmp[0]); printf("\n");

/* Inverse */

//	frank_dsmatrix(mpfa, dim);
//	frank_dsmatrix(mpfb, dim);
//	lotkin_dsmatrix(mpfa, dim);
//	lotkin_dsmatrix(mpfb, dim);

	free_dsmatrix(mpfc);
	free_dsmatrix(mpfc_tmp);
	free_dsmatrix(mpfc_normal);
	free_dsmatrix(mpfc_block);
	free_dsmatrix(mpfa);
	free_dsmatrix(mpfb);
