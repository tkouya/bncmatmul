...
void _mpi_init_dex_nim(long int stage, long int dimension, MPI_Comm comm)
{
	...
	dex_nim_local_dim = _mpi_divide_dim(dex_nim_ddim, dimension, num_procs);

	for(i = 0; i < 4; i++)
	{
		dex_nim_ytmp[i] = init_dvector(dimension);
		dex_nim_local_ytmp[i] = _mpi_init_dvector(dex_nim_ddim, dimension, comm);
	}

	dex_nim_stage = stage < (MPI_MAX_DEX_NIM_STAGE - 1) ? stage : (MPI_MAX_DEX_NIM_STAGE - 1);

	for(i = 0; i < dex_nim_stage; i++)
		for(j = 0; j < dex_nim_stage; j++)
			dex_nim_local_y[i][j] = init_dvector(dex_nim_local_dim);
...
void _mpi_clear_dex_nim(void)
{
	...
	for(i = 0; i < 4; i++)
	{
		free_dvector(dex_nim_ytmp[i]);
		_mpi_free_dvector(dex_nim_local_ytmp[i]);
	}

	for(i = 0; i < dex_nim_stage; i++)
		for(j = 0; j < dex_nim_stage; j++)
			free_dvector(dex_nim_local_y[i][j]);
...
/*                          */
int _mpi_dex_nim_1step(DVector local_y, double x0, DVector local_y0, DVector y0, double h, void (* func)(DVector, double, DVector, MPI_Comm), double rtol, double atol, MPI_Comm comm)
{
...
/* First, Using Euler Method */
		func(dex_nim_local_ytmp[0], x0, y0, comm);
...
/* Next, Using Midpoint Rule */
		for(i = 1; i <= istep_l; i++)
		{
			...
			_mpi_collect_dvector(dex_nim_ytmp[1], dex_nim_ddim, dex_nim_local_ytmp[1], comm);
			_mpi_bcast_dvector(dex_nim_ytmp[1], comm);
			func(dex_nim_local_ytmp[2], ftmp, dex_nim_ytmp[1], comm);
...
/* NIM Extrapolation Method */
/*                          */
void _mpi_dex_nim_fs(FILE *fp, double x, DVector y, double x0, DVector y0, long int div_num, void (* func)(DVector, double, DVector, MPI_Comm), double rtol, double atol, long int stage, MPI_Comm comm)
{
	...
	for(steps = 0; steps < div_num; steps++)
	{
		...
		num_stage = _mpi_dex_nim_1step(local_tmp_y, old_x, local_old_y, old_y, in_h, func, rtol, atol, comm);
...
/* NIM Extrapolation Method */
/*                          */
void _mpi_dex_nim(FILE *fp, double x, DVector y, double x0, DVector y0, double max_h, void (* func)(DVector, double, DVector, MPI_Comm), double rtol, double atol, long int stage, void (* ansfunc)(DVector, double), MPI_Comm comm)
{
...
//		in_h = max_h;
		...
		{
			/* tmp_y := y0 + h * ex(x0, y0) */
			if(_mpi_dex_nim_1step(local_tmp_y, old_x, local_old_y, old_y, in_h, func, rtol, atol, comm) == 0)
				break;
			...
			if(tmp[0] <= old_x)
			{
				...
				_mpi_dex_nim_1step(local_tmp_y, old_x, local_old_y, old_y, in_h, func, rtol, atol, comm);
...
void _mpi_init_mpf_ex_nim(long int stage, long int dimension, unsigned long prec, MPI_Comm comm)
{
	...
	MPI_Comm_size(comm, &num_procs);
	ex_nim_local_dim = _mpi_divide_dim(ex_nim_ddim, dimension, num_procs);

	mpf_init2(ex_nim_in_h, prec); mpf_init2(ex_nim_mpf_tmp, prec);
	for(i = 0; i < 4; i++)
	{
		ex_nim_ytmp[i] = init2_mpfvector(dimension, prec);
		ex_nim_local_ytmp[i] = _mpi_init_mpfvector(ex_nim_ddim, dimension, comm);
	}

	ex_nim_stage = stage < (MPI_MAX_EX_NIM_STAGE - 1) ? stage : (MPI_MAX_EX_NIM_STAGE - 1);

	for(i = 0; i < ex_nim_stage; i++)
		for(j = 0; j < ex_nim_stage; j++)
			ex_nim_local_y[i][j] = init2_mpfvector(ex_nim_local_dim, prec);
...
void _mpi_clear_mpf_ex_nim(void)
{
	...
	mpf_clear(ex_nim_in_h); mpf_clear(ex_nim_mpf_tmp);
	for(i = 0; i < 4; i++)
	{
		free_mpfvector(ex_nim_ytmp[i]);
		_mpi_free_mpfvector(ex_nim_local_ytmp[i]);
	}

	for(i = 0; i < ex_nim_stage; i++)
		for(j = 0; j < ex_nim_stage; j++)
			free_mpfvector(ex_nim_local_y[i][j]);
...
/* NIM Extrapolation Method */
/*                          */
int _mpi_mpf_ex_nim_1step(MPFVector local_y, mpf_t x0, MPFVector local_y0, MPFVector y0, mpf_t h, void (* func)(MPFVector, mpf_t, MPFVector, MPI_Comm), mpf_t rtol, mpf_t atol, MPI_Comm comm)
{
...
/* First, Using Euler Method */
		func(ex_nim_local_ytmp[0], x0, y0, comm);
...
/* Next, Using Midpoint Rule */
		for(i = 1; i <= istep_l; i++)
		{
			...
			_mpi_collect_mpfvector(ex_nim_ytmp[1], ex_nim_ddim, ex_nim_local_ytmp[1], comm);
			_mpi_bcast_mpfvector(ex_nim_ytmp[1], comm);
			func(ex_nim_local_ytmp[2], ex_nim_mpf_tmp, ex_nim_ytmp[1], comm);
...
/* NIM Extrapolation Method */
/* Stepsize fixed           */
void _mpi_mpf_ex_nim_fs(FILE *fp, mpf_t x, MPFVector y, mpf_t x0, MPFVector y0, long int div_num, void (* func)(MPFVector, mpf_t, MPFVector, MPI_Comm), mpf_t rtol, mpf_t atol, long int stage, MPI_Comm comm)
{
	...
	for(steps = 0; steps < div_num; steps++)
	{
		...
		_mpi_mpf_ex_nim_1step(local_tmp_y, old_x, local_old_y, old_y, in_h, func, rtol, atol, comm);
...
/* NIM Extrapolation Method */
/*                          */
void _mpi_mpf_ex_nim(FILE *fp, mpf_t x, MPFVector y, mpf_t x0, MPFVector y0, mpf_t max_h, void (* func)(MPFVector, mpf_t, MPFVector, MPI_Comm), mpf_t rtol, mpf_t atol, long int stage, void (* ansfunc)(MPFVector, mpf_t), long int itimes_iv, MPI_Comm comm)
{
	long int steps, i, dim;
	...
	for(steps = 0; ; steps++)
	{
		...
		{
			/* tmp_y := y0 + h * euler(x0, y0) */
			if(_mpi_mpf_ex_nim_1step(local_tmp_y, old_x, local_old_y, old_y, in_h, func, rtol, atol, comm) == 0)
				break;
