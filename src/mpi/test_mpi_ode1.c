...
//	goto local_ret;
	...
	istart = (DIM / num_procs) * myrank;
	iend   = (DIM / num_procs) * (myrank + 1);

//	printf("num_procs %d, myrank %d: istart %d, iend %d\n", num_procs, myrank, istart, iend);
	//for(i = 0; i < DIM; i++)
	for(i = istart, itmp = 0; i < iend; i++, itmp += 2)
	{
		im1 = i - 1;
		if(im1 < 0)
			im1 += DIM;
...
//		print_dvector(tmp_tmp_xi);
		copy_dvector_ij(local_y, itmp * DIM, (itmp + 1) * DIM - 1    , tmp_tmp_xi, 0, DIM - 1);
		copy_dvector_ij(local_y, (itmp + 1) * DIM, (itmp + 2) * DIM - 1, tmp_tmp_yi, 0, DIM - 1);
	}
...
//		print_mpfvector(tmp_tmp_xi);
		copy_mpfvector_ij(local_y, itmp * DIM, (itmp + 1) *  DIM - 1    , tmp_tmp_xi, 0, DIM - 1);
		copy_mpfvector_ij(local_y, (itmp + 1) * DIM, (itmp + 2) * DIM - 1, tmp_tmp_yi, 0, DIM - 1);
...
*/
	...
	stime = MPI_Wtime();
	_mpi_dex_harmonic(NULL, dx, dy, init_dx, init_dy, dstepsize, ddf, drel_tol, dabs_tol, 4, dans, MPI_COMM_WORLD);
	etime = MPI_Wtime();
	if(myrank == 0)
		printf("time(sec): %f\n", etime - stime);
...
*/
	...
	stime = MPI_Wtime();
	_mpi_mpf_ex_harmonic(NULL, x, y, init_x, init_y, stepsize, df, rel_tol, abs_tol, 4, ans, 1, MPI_COMM_WORLD);
	etime = MPI_Wtime();
	if(myrank == 0)
		printf("time(sec): %f\n", etime - stime);
