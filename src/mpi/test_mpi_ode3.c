...
void ddf(DVector local_y, double x0, DVector y0, MPI_Comm comm)
{
	...
	base_index = (y0->dim / num_procs) * myrank;
	for(i = 0; i < local_y->dim; i++)
	{
		index = base_index + i;
		set_dvector_i(local_y, i, (index + 1) * get_dvector_i(y0, index));
	}
...
void df(MPFVector local_y, mpf_t x0, MPFVector y0, MPI_Comm comm)
{
	...
	{
		index = base_index + i;
		mpf_mul_ui(tmp, get_mpfvector_i(y0, index), index + 1);
...
//	goto HARMONIC;
	...
	for(div_num = 2; div_num <= 128; div_num *= 2)
	{
		...
		initial_value(init_x, x, init_y);
		stime = MPI_Wtime();
		_mpi_mpf_ex_nim_fs(NULL, x, y, init_x, init_y, div_num, df, rel_tol, abs_tol, 8, MPI_COMM_WORLD);
		etime = MPI_Wtime();
		if(myrank == 0)
		{
			printf("time(sec): %f\n", etime - stime);
...
HARMONIC:
	...
	for(div_num = 2; div_num <= 128; div_num *= 2)
	{
		initial_dvalue(&init_dx, &dx, init_dy);
		stime = MPI_Wtime();
		_mpi_dex_harmonic_fs(NULL, dx, dy, init_dx, init_dy, div_num, ddf, drel_tol, dabs_tol, 4, MPI_COMM_WORLD);
		etime = MPI_Wtime();
		if(myrank == 0)
		{
			printf("time(sec): %f\n", etime - stime);
