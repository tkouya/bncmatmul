/********************************************************************************/
/* matmul_strassen_general_td_omp.c:                                            */
...
long int _bncomp_num_addsub_mul_tdmatrix_strassen;	// addition and subtraction
long int _bncomp_num_mul_mul_tdmatrix_strassen;		// multiplication
...
void _bncomp_add_tdmatrix_partial(TDMatrix ret, long int ret_index[4], TDMatrix mat_a, long int mat_a_index[4], TDMatrix mat_b, long int mat_b_index[4])
{
	...
	long int imax, jmax;
#ifdef __cplusplus
	 td_real tmp_val[BNCOMP_MAX_NUM_THREADS];
#else // __cplusplus
	 double tmp_val[BNCOMP_MAX_NUM_THREADS][TDSIZE];
#endif // __cpplusplus
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			rtd_add(tmp_val[thread_index], get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]), get_tdmatrix_ij(mat_b, b_i[thread_index], b_j[thread_index]));
			set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], tmp_val[thread_index]);
		}
	}
}
...
void _bncomp_sub_tdmatrix_partial(TDMatrix ret, long int ret_index[4], TDMatrix mat_a, long int mat_a_index[4], TDMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			//tmp_val = get_tdmatrix_ij(mat_a, a_i, a_j) - get_tdmatrix_ij(mat_b, b_i, b_j);
			rtd_sub(tmp_val[thread_index], get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]), get_tdmatrix_ij(mat_b, b_i[thread_index], b_j[thread_index]));
			set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], tmp_val[thread_index]);
		}
	}
}
	...
	#pragma omp parallel private(thread_index, i, j)
	{
		...
		for(i = thread_index; i < imax; i += thread_num)
		{
			...
			for(j = 0; j < jmax; j++)
			{
				...
				set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
			}
		}
	} // omp parallel
}
				...
				for(j = 0; j < jmax; j++)
				{
					...
					if((ret_j[thread_index] >= 0) && (ret_j[thread_index] < ret->col_dim))
					{
						if((a_i[thread_index] >= 0) && (a_i[thread_index] < mat_a->row_dim) && (a_j[thread_index] >= 0) && (a_j[thread_index] < mat_a->col_dim))
						{
							...
							set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
						#else // __cplusplus
							//ptr_tdtmp = get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]);
							//set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], ptr_tdtmp);
							set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
						#endif // __cplusplus
						}
						else
							set0_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index]); // Padding
...
void _bncomp_neg_tdmatrix_partial(TDMatrix ret, long int ret_index[4], TDMatrix mat_a, long int mat_a_index[4])
{
	...
	long int imax, jmax;
#ifdef __cplusplus
	td_real tmp[BNCOMP_MAX_NUM_THREADS];
#else
...
//	}
		...
		for(i = thread_index; i < imax; i += thread_num)
		{
			...
			for(j = 0; j < jmax; j++)
			{
				...
				set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], -get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
	#else // __cplusplus
				rtd_neg(tmp[thread_index], get_tdmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
				set_tdmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], tmp[thread_index]);
	#endif // __cplusplus
			}
		}
	} // omp parallel
...
void _bncomp_mul_tdmatrix_block(TDMatrix ret, TDMatrix mat_a, TDMatrix mat_b, long int min_dim)
{
	...
	//TDMatrix small_ret[1024], small_mat_a[1024], small_mat_b[1024], small_tmp_mat;
	...
	{
		fprintf(stderr, "ERROR: mat_a's col_dim %ld does not just fit mat_b's row_dim %ld!(_bncomp_mul_tdmatrix_block)\n", mat_a->col_dim, mat_b->row_dim);
		return;
	}
	...
	{
		_bncomp_mul_tdmatrix_simple(ret, mat_a, mat_b);
		return;
	}
	...
	small_tmp_mat = (TDMatrix *)calloc(num_div_mid, sizeof(TDMatrix));
	...
	for(i = 0; i < num_div_mid; i++)
	{
		...
		small_tmp_mat[i] = init_tdmatrix(min_dim, min_dim);
	}
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		for(j = 0; j < num_div_mid; j++)
		{
			...
			subst_tdmatrix_partial_checked(small_mat_a[j], small_mat_a_index, mat_a, mat_a_index[j]);
		}
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			for(k = 0; k < num_div_mid; k++)
			{
				...
				subst_tdmatrix_partial_checked(small_mat_b[k], small_mat_b_index, mat_b, mat_b_index[k]);
				//_bncomp_subst_tdmatrix_partial_checked(small_mat_b[k], small_mat_b_index, mat_b, mat_b_index[k]);
				// ret[j] += small_mat_a[i][k] * small_mat_b[k][j];
				mul_tdmatrix(small_tmp_mat[k], small_mat_a[k], small_mat_b[k]);
			}

			// Fix! 2017-09-12(Tue) by T.Kouya
			//for(k = 0; k < num_div_mid; k++)
			//	_bncomp_add_tdmatrix(small_ret[j], small_ret[j], small_tmp_mat[k]);
			for(k = 0; k < num_div_mid; k++)
				add_tdmatrix(small_ret[j], small_ret[j], small_tmp_mat[k]);

			ret_index[j][0] = i * min_dim;
			ret_index[j][1] = (i + 1) * min_dim;
			ret_index[j][2] = j * min_dim;
			ret_index[j][3] = (j + 1) * min_dim;
			subst_tdmatrix_partial_checked(ret, ret_index[j], small_ret[j], small_ret_index);
			// Fix! 2017-09-12(Tue) by T.Kouya
			//_bncomp_subst_tdmatrix_partial_checked(ret, ret_index[j], small_ret[j], small_ret_index);
		}
	}
...
void _bncomp_mul_tdmatrix_block_old(TDMatrix ret, TDMatrix mat_a, TDMatrix mat_b, long int min_dim)
{
	int thread_num, thread_index;
	int row_padding_flag = 0, col_padding_flag = 0, mid_padding_flag = 0;
	long i, j, k, row_dim, col_dim, mid_dim;
	...
	TDMatrix *small_ret, *small_mat_a, *small_mat_b, small_tmp_mat[BNCOMP_MAX_NUM_THREADS];
	...
	small_ret = (TDMatrix *)calloc(sizeof(TDMatrix), num_div_col);
	small_mat_a = (TDMatrix *)calloc(sizeof(TDMatrix), num_div_mid);
	small_mat_b = (TDMatrix *)calloc(sizeof(TDMatrix), num_div_mid);
	...
	{
		thread_index = omp_get_thread_num();
		small_tmp_mat[thread_index] = init_tdmatrix(min_dim, min_dim);
	}
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		for(j = 0; j < num_div_mid; j++)
		{
			...
			subst_tdmatrix_partial_checked(small_mat_a[j], small_mat_a_index[thread_index], mat_a, mat_a_index[thread_index]);
		}
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			for(k = 0; k < num_div_mid; k++)
			{
				...
				subst_tdmatrix_partial_checked(small_mat_b[k], small_mat_b_index[thread_index], mat_b, mat_b_index[thread_index]);

				// ret[j] += small_mat_a[i][k] * small_mat_b[k][j];
				mul_tdmatrix(small_tmp_mat[thread_index], small_mat_a[k], small_mat_b[k]);
				add_tdmatrix(small_ret[j], small_ret[j], small_tmp_mat[thread_index]);
			}
			...
			subst_tdmatrix_partial_checked(ret, ret_index[thread_index], small_ret[j], small_ret_index[thread_index]);
		}
	}
	...
	{
		thread_index = omp_get_thread_num();
		free_tdmatrix(small_tmp_mat[thread_index]);
	}
...
void _bncomp_mul_tdmatrix_strassen_odd_padding(TDMatrix ret, TDMatrix mat_a, TDMatrix mat_b, long int min_dim)
{
	...
	tmp_ret = _bncomp_init_static_padding_tdmatrix_strassen(ret);
	tmp_mat_a = _bncomp_init_static_padding_tdmatrix_strassen(mat_a);
	tmp_mat_b = _bncomp_init_static_padding_tdmatrix_strassen(mat_b);
#else
//	tmp_ret = _bncomp_init_dynamic_padding_tdmatrix_strassen(ret);
//	tmp_mat_a = _bncomp_init_dynamic_padding_tdmatrix_strassen(mat_a);
//	tmp_mat_b = _bncomp_init_dynamic_padding_tdmatrix_strassen(mat_b);
	tmp_ret = _bncomp_init_dynamic_padding_tdmatrix_strassen2(ret, min_dim);
	tmp_mat_a = _bncomp_init_dynamic_padding_tdmatrix_strassen2(mat_a, min_dim);
	tmp_mat_b = _bncomp_init_dynamic_padding_tdmatrix_strassen2(mat_b, min_dim);
#endif

	// strassen
#ifdef USE_WINOGRAD
	_bncomp_mul_tdmatrix_winograd_even_psec(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
	//_bncomp_mul_tdmatrix_winograd_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#else
	//printf("strassen_even_psec start ... ");fflush(stdout);
	_bncomp_mul_tdmatrix_strassen_even_psec(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
	//_bncomp_mul_tdmatrix_strassen_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
	//printf("end\n"); fflush(stdout);
#endif
	...
	_bncomp_subst_tdmatrix_partial(ret, ret_index, tmp_ret, tmp_ret_index);
...
{
	_bncomp_num_addsub_mul_tdmatrix_strassen = 0;
	_bncomp_num_mul_mul_tdmatrix_strassen = 0;
}
...
void _bncomp_get_num_mul_tdmatrix_strassen(long int *num_addsub, long int *num_mul)
{
	...
	if(num_addsub != NULL)
		*num_addsub = _bncomp_num_addsub_mul_tdmatrix_strassen;
	if(num_mul != NULL)
		*num_mul = _bncomp_num_mul_mul_tdmatrix_strassen;
}

// print counters
void _bncomp_print_num_mul_tdmatrix_strassen(long int *num_addsub, long int *num_mul)
{
	printf("_bncomp_num_addsub_mul_tdmatrix_strassen: %ld\n", _bncomp_num_addsub_mul_tdmatrix_strassen);
	printf("_bncomp_num_mul_mul_tdmatrix_strassen   : %ld\n", _bncomp_num_mul_mul_tdmatrix_strassen);

	_bncomp_get_num_mul_tdmatrix_strassen(num_addsub, num_mul);

}

/* c = a * b */
//void _bncomp_mul_tdmatrix_simple(TDMatrix ret, TDMatrix a, TDMatrix b)
void _bncomp_mul_tdmatrix_simple_old(TDMatrix ret, TDMatrix a, TDMatrix b)
{
	int thread_num, thread_index;
	long int i, j, k;
	long row_dim, col_dim, mid_dim;
#ifdef __cplusplus
	td_real tmp[128], tmp1[128];
#else // __cplusplus
	double tmp[128][TDSIZE];
#endif // __cplusplus
	...
	for(i = 0; i < row_dim; i++)
	{
		...
		for(j = 0; j < col_dim; j++)
		{
			//thread_index = omp_get_thread_num();
#ifdef __cplusplus
			tmp1[thread_index] = (td_real)0.0;
			tmp[thread_index] = (td_real)0.0;
			for(k = 0; k < mid_dim; k++)
			{
...
//	TDVector diag_left, diag_right;
	...
	if((ret->row_dim <= min_dim) && (ret->col_dim <= min_dim) && (mid_dim <= min_dim))
	{
		...
		_bncomp_num_addsub_mul_tdmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
		_bncomp_num_mul_mul_tdmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
...
//	right_scaling_tdmatrix(mat_b, diag_right, NULL);
	...
	{
#ifdef PEELING_ONLY
		_bncomp_mul_tdmatrix_strassen_odd_peeling(ret, mat_a, mat_b, min_dim);
#elif PADDING_ONLY
		_bncomp_mul_tdmatrix_strassen_odd_padding(ret, mat_a, mat_b, min_dim);
#else
...
#ifdef USE_WINOGRAD
		_bncomp_mul_tdmatrix_winograd_even_psec(ret, mat_a, mat_b, min_dim);
		//_bncomp_mul_tdmatrix_winograd_even(ret, mat_a, mat_b, min_dim);
#else
		_bncomp_mul_tdmatrix_strassen_even_psec(ret, mat_a, mat_b, min_dim);
		//_bncomp_mul_tdmatrix_strassen_even(ret, mat_a, mat_b, min_dim);
#endif
		//printf("end\n");
	}

//	mul_tdmatrix_tdiag_mat(ret, diag_left, 0, ret, diag_right, 0);
...
//	 double a22[TDSIZE], b22[TDSIZE], c22[TDSIZE], tmp[TDSIZE];
	double a22[TDSIZE], b22[TDSIZE], c22[TDSIZE], tmp[TDSIZE], tmp_array[BNCOMP_MAX_NUM_THREADS][TDSIZE];
#endif // __cplusplus
	...
	// C11 = A11 * B11
#ifdef USE_WINOGRAD
	_bncomp_mul_tdmatrix_winograd_even_psec(mat_c11, mat_a11, mat_b11, min_dim);
	//_bncomp_mul_tdmatrix_winograd_even(mat_c11, mat_a11, mat_b11, min_dim);
#else
	_bncomp_mul_tdmatrix_strassen_even_psec(mat_c11, mat_a11, mat_b11, min_dim);
	//_bncomp_mul_tdmatrix_strassen_even(mat_c11, mat_a11, mat_b11, min_dim);
#endif
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += row_dim_h * col_dim_h;
		_bncomp_num_mul_mul_tdmatrix_strassen += row_dim_h * col_dim_h;
		...
		for(i = 0; i < row_dim_h; i++)
		{
			...
			for(j = 0; j < col_dim_h; j++)
			{
				...
				rtd_mul(tmp_array[thread_index], get_tdvector_i(vec_a12, i), get_tdvector_i(vec_b21, j));
				set_tdmatrix_ij(mat_tmp, i, j, tmp_array[thread_index]);
			}
		}
		_bncomp_add_tdmatrix(mat_c11, mat_c11, mat_tmp);
		//printf("C11 += a12 * b21...\n");
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;
		_bncomp_num_mul_mul_tdmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;

		_bncomp_mul_tdmatrix_tdvec(vec_c12, mat_a11, vec_b12);
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += vec_c12->dim;
		_bncomp_num_mul_mul_tdmatrix_strassen += vec_tmp12->dim;

		_bncomp_cmul_tdvector(vec_tmp12, b22, vec_a12);
		_bncomp_add_tdvector(vec_c12, vec_c12, vec_tmp12);
	}
	//printf("c12 += b22 * a12\n");
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;
		_bncomp_num_mul_mul_tdmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;

		_bncomp_mul_tdmatrixt_tdvec(vec_c21, mat_b11, vec_a21);
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += vec_c21->dim;
		_bncomp_num_mul_mul_tdmatrix_strassen += vec_tmp21->dim;

		_bncomp_cmul_tdvector(vec_tmp21, a22, vec_b21);
		_bncomp_add_tdvector(vec_c21, vec_c21, vec_tmp21);
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += vec_a21->dim;
		_bncomp_num_mul_mul_tdmatrix_strassen += vec_tmp21->dim;

#ifdef __cplusplus
		_bncomp_ip_tdvector(&c22, vec_a21, vec_b12);
#else
		_bncomp_ip_tdvector(c22, vec_a21, vec_b12);
#endif // __cplusplus
	}
	...
	{
		// counting the number of arithmetic
		_bncomp_num_addsub_mul_tdmatrix_strassen += 1;
		_bncomp_num_mul_mul_tdmatrix_strassen += 1;
...
//	printf("P1a:\n");
	...
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_tdmatrix_strassen += row_dim_h * mid_dim_h;

//	add_tdmatrix_partial(mat_tmp_a[0], ret_index, mat_a, mat_ar_index[0], mat_a, mat_al_index[0]);
	_bncomp_add_tdmatrix_partial(mat_tmp_a[0], mat_tmp_a_index, mat_a, mat_ar_index[0], mat_a, mat_al_index[0]);
...
//	printf("P1b:\n");
	...
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_tdmatrix_strassen += mid_dim_h * col_dim_h;

//	add_tdmatrix_partial(mat_tmp_b[0], ret_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
	_bncomp_add_tdmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);

	// P1 = tmp_a * tmp_b
//	printf("P1:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim);
...
//	add_tdmatrix_partial(mat_tmp_a[1], ret_index, mat_a, mat_ar_index[1], mat_a, mat_al_index[1]);
	_bncomp_add_tdmatrix_partial(mat_tmp_a[1], mat_tmp_a_index, mat_a, mat_ar_index[1], mat_a, mat_al_index[1]);
...
//	subst_tdmatrix_partial(mat_tmp_b[1], ret_index, mat_b, mat_br_index[1]);
	_bncomp_subst_tdmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);

	// P2 = tmp_a * tmp_b
	//printf("P2:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim);
...
//	subst_tdmatrix_partial(mat_tmp_a[2], ret_index, mat_a, mat_ar_index[2]);
	_bncomp_subst_tdmatrix_partial(mat_tmp_a[2], mat_tmp_a_index, mat_a, mat_ar_index[2]);
...
//	sub_tdmatrix_partial(mat_tmp_b[2], ret_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
	_bncomp_sub_tdmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);

	// P3 = tmp_a * tmp_b
	//printf("P3:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim);
...
//	subst_tdmatrix_partial(mat_tmp_a[3], ret_index, mat_a, mat_ar_index[3]);
	_bncomp_subst_tdmatrix_partial(mat_tmp_a[3], mat_tmp_a_index, mat_a, mat_ar_index[3]);
...
//	sub_tdmatrix_partial(mat_tmp_b[3], ret_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
	_bncomp_sub_tdmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);

	// P4 = tmp_a * tmp_b
	//printf("P4:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim);
...
//	add_tdmatrix_partial(mat_tmp_a[4], ret_index, mat_a, mat_ar_index[4], mat_a, mat_al_index[4]);
	_bncomp_add_tdmatrix_partial(mat_tmp_a[4], mat_tmp_a_index, mat_a, mat_ar_index[4], mat_a, mat_al_index[4]);
...
//	subst_tdmatrix_partial(mat_tmp_b[4], ret_index, mat_b, mat_br_index[4]);
	_bncomp_subst_tdmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);

	// P5 = tmp_a * tmp_b
	//printf("P5:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim);
...
//	sub_tdmatrix_partial(mat_tmp_a[5], ret_index, mat_a, mat_ar_index[5], mat_a, mat_al_index[5]);
	_bncomp_sub_tdmatrix_partial(mat_tmp_a[5], mat_tmp_a_index, mat_a, mat_ar_index[5], mat_a, mat_al_index[5]);
...
//	add_tdmatrix_partial(mat_tmp_b[5], ret_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
	_bncomp_add_tdmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);

	// P6 = tmp_a * tmp_b
	//printf("P6:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim);
...
//	sub_tdmatrix_partial(mat_tmp_a[6], ret_index, mat_a, mat_ar_index[6], mat_a, mat_al_index[6]);
	_bncomp_sub_tdmatrix_partial(mat_tmp_a[6], mat_tmp_a_index, mat_a, mat_ar_index[6], mat_a, mat_al_index[6]);
...
//	add_tdmatrix_partial(mat_tmp_b[6], ret_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
	_bncomp_add_tdmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);

	// P7 = tmp_a * tmp_b
	//printf("P7:\n");
	_bncomp_mul_tdmatrix_strassen(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim);

	// -------------------------------
	// C11 := P1 + P4 - P5 + P7
	// -------------------------------
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_tdmatrix_strassen += 3 * row_dim_h * col_dim_h;

	_bncomp_add_tdmatrix(mat_tmp_c[0], mat_p[0], mat_p[3]);
	_bncomp_sub_tdmatrix(mat_tmp_c[0], mat_tmp_c[0], mat_p[4]);
	_bncomp_add_tdmatrix(mat_tmp_c[0], mat_tmp_c[0], mat_p[6]);
	mat_c_index[0][0] = 0;
	mat_c_index[0][1] = row_dim_h;
	mat_c_index[0][2] = 0;
	mat_c_index[0][3] = col_dim_h;

	_bncomp_subst_tdmatrix_partial(ret, mat_c_index[0], mat_tmp_c[0], ret_index);
	...
	_bncomp_add_tdmatrix(mat_tmp_c[1], mat_p[2], mat_p[4]);
	mat_c_index[1][0] = 0;
	mat_c_index[1][1] = row_dim_h;
	mat_c_index[1][2] = col_dim_h;
	mat_c_index[1][3] = col_dim;
	_bncomp_subst_tdmatrix_partial(ret, mat_c_index[1], mat_tmp_c[1], ret_index);
	...
	_bncomp_add_tdmatrix(mat_tmp_c[2], mat_p[1], mat_p[3]);
	mat_c_index[2][0] = row_dim_h;
	mat_c_index[2][1] = row_dim;
	mat_c_index[2][2] = 0;
	mat_c_index[2][3] = col_dim_h;
	_bncomp_subst_tdmatrix_partial(ret, mat_c_index[2], mat_tmp_c[2], ret_index);
	...
	_bncomp_add_tdmatrix(mat_tmp_c[3], mat_p[0], mat_p[2]);
	_bncomp_sub_tdmatrix(mat_tmp_c[3], mat_tmp_c[3], mat_p[1]);
	_bncomp_add_tdmatrix(mat_tmp_c[3], mat_tmp_c[3], mat_p[5]);
	mat_c_index[3][0] = row_dim_h;
	mat_c_index[3][1] = row_dim;
	mat_c_index[3][2] = col_dim_h;
	mat_c_index[3][3] = col_dim;
	_bncomp_subst_tdmatrix_partial(ret, mat_c_index[3], mat_tmp_c[3], ret_index);
...
void _bncomp_mul_tdmatrix_strassen_even_psec_old(TDMatrix ret, TDMatrix mat_a, TDMatrix mat_b, long int min_dim)
{
	int thread_num, thread_index;
//	long int min_dim = 4; // = 2^2
	TDMatrix mat_p[7], mat_tmp_a[7], mat_tmp_b[7], mat_tmp_c[4];
...
//	if((ret->row_dim <= min_dim) && (ret->col_dim <= min_dim) && (mid_dim <= min_dim))
	...
	for(i = 0; i < 4; i++)
	{
		_bncomp_subst_tdmatrix_partial(mat_tmp_a[i], mat_tmp_a_index, mat_a, mat_a_index[i]);
		_bncomp_subst_tdmatrix_partial(mat_tmp_b[i], mat_tmp_b_index, mat_b, mat_b_index[i]);
	}
//	printf("subst a, b...\n");
	...
	_bncomp_add_tdmatrix(mat_s[0], mat_tmp_a[2], mat_tmp_a[3]);
	...
	_bncomp_sub_tdmatrix(mat_s[1], mat_s[0], mat_tmp_a[0]);
	...
	_bncomp_sub_tdmatrix(mat_s[2], mat_tmp_a[0], mat_tmp_a[2]);
	...
	_bncomp_sub_tdmatrix(mat_s[3], mat_tmp_a[1], mat_s[1]);
	...
	_bncomp_sub_tdmatrix(mat_s[4], mat_tmp_b[1], mat_tmp_b[0]);
	...
	_bncomp_sub_tdmatrix(mat_s[5], mat_tmp_b[3], mat_s[4]);
	...
	_bncomp_sub_tdmatrix(mat_s[6], mat_tmp_b[3], mat_tmp_b[1]);
	...
	_bncomp_sub_tdmatrix(mat_s[7], mat_s[5], mat_tmp_b[2]);
...
//	printf("s...\n");
	...
	// M1 := S2 * S6
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[0], mat_s[1], mat_s[5], min_dim);

	// -------------------------------
	// M2 := A11 * B11
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[1], mat_tmp_a[0], mat_tmp_b[0], min_dim);

	// -------------------------------
	// M3 := A12 * B21
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[2], mat_tmp_a[1], mat_tmp_b[2], min_dim);

	// -------------------------------
	// M4 := S3 * S7
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[3], mat_s[2], mat_s[6], min_dim);

	// -------------------------------
	// M5 := S1 * S5
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[4], mat_s[0], mat_s[4], min_dim);

	// -------------------------------
	// M6 := S4 * B22
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[5], mat_s[3], mat_tmp_b[3], min_dim);

	// -------------------------------
	// M7 := A22 * S8
	// -------------------------------
	_bncomp_mul_tdmatrix_strassen(mat_m[6], mat_tmp_a[3], mat_s[7], min_dim);
...
//	printf("m...\n");
	...
	_bncomp_add_tdmatrix(mat_t[0], mat_m[0], mat_m[1]);
	...
	_bncomp_add_tdmatrix(mat_t[1], mat_t[0], mat_m[3]);
...
//	printf("t...\n");
	...
	_bncomp_add_tdmatrix(mat_tmp_c[0], mat_m[1], mat_m[2]);

	// -------------------------------
	// C12 := T1 + M5 + M6
	// -------------------------------
	// counting the number of arithmetic
	_bncomp_num_addsub_mul_tdmatrix_strassen += 2 * row_dim_h * col_dim_h;

	_bncomp_add_tdmatrix(mat_tmp_c[1], mat_t[0], mat_m[4]);
	_bncomp_add_tdmatrix(mat_tmp_c[1], mat_tmp_c[1], mat_m[5]);
	...
	_bncomp_sub_tdmatrix(mat_tmp_c[2], mat_t[1], mat_m[6]);
	...
	_bncomp_add_tdmatrix(mat_tmp_c[3], mat_t[1], mat_m[4]);
...
//	printf("c...\n");
	...
	// -------------------------------
	for(i = 0; i < 4; i++)
		_bncomp_subst_tdmatrix_partial(ret, mat_c_index[i], mat_tmp_c[i], ret_index);
...
void _bncomp_mul_tdmatrix_strassen_nonrec(TDMatrix ret, TDMatrix mat_a, TDMatrix mat_b, long int min_dim)
{
	...
	TDMatrix a11, a12, a21, a22;
	TDMatrix b11, b12, b21, b22;
	TDMatrix ret11, ret12, ret21, ret22, tmp_mat;
	...
	if(mid_dim != mat_b->row_dim)
	{
		fprintf(stderr, "ERROR: mat_a's col_dim %ld does not just fit mat_b's row_dim %ld!(_bncomp_mul_tdmatrix_strassen_nonrec)\n", mat_a->col_dim, mat_b->row_dim);
		return;
	}
...
*/
	...
	{
		//_bncomp_mul_tdmatrix_block(ret, mat_a, mat_b, min_dim);
		_bncomp_mul_tdmatrix_simple(ret, mat_a, mat_b);
		return;
	}
	...
	// Strassen Area
	a11 = init_tdmatrix(a11_row_dim, a11_col_dim);
	...
	_bncomp_subst_tdmatrix_partial_checked(a11, a11_index, mat_a, mat_a_index[0]);

	a12 = NULL;
	if(a12_col_dim > 0)
	{
		a12 = init_tdmatrix(a12_row_dim, a12_col_dim);
		a12_index[0] = 0;
		...
		_bncomp_subst_tdmatrix_partial_checked(a12, a12_index, mat_a, mat_a_index[1]);
	}

	a21 = NULL;
	if(a21_row_dim > 0)
	{
		a21 = init_tdmatrix(a21_row_dim, a21_col_dim);
		a21_index[0] = 0;
		...
		_bncomp_subst_tdmatrix_partial_checked(a21, a21_index, mat_a, mat_a_index[2]);
	}

	a22 = NULL;
	if((a22_row_dim > 0) && (a22_col_dim > 0))
	{
		a22 = init_tdmatrix(a22_row_dim, a22_col_dim);
		a22_index[0] = 0;
		...
		_bncomp_subst_tdmatrix_partial_checked(a22, a22_index, mat_a, mat_a_index[3]);
	}
	...
	// Strassen Area
	b11 = init_tdmatrix(b11_row_dim, b11_col_dim);
	...
	_bncomp_subst_tdmatrix_partial_checked(b11, b11_index, mat_b, mat_b_index[0]);

	b12 = NULL;
	if(b12_col_dim > 0)
	{
		b12 = init_tdmatrix(b12_row_dim, b12_col_dim);
		b12_index[0] = 0;
		...
		_bncomp_subst_tdmatrix_partial_checked(b12, b12_index, mat_b, mat_b_index[1]);
	}

	b21 = NULL;
	if(b21_row_dim > 0)
	{
		b21 = init_tdmatrix(b21_row_dim, b21_col_dim);
		b21_index[0] = 0;
		...
		_bncomp_subst_tdmatrix_partial_checked(b21, b21_index, mat_b, mat_b_index[2]);
	}

	b22 = NULL;
	if((b22_row_dim > 0) && (b22_col_dim > 0))
	{
		b22 = init_tdmatrix(b22_row_dim, b22_col_dim);
		b22_index[0] = 0;
		...
		_bncomp_subst_tdmatrix_partial_checked(b22, b22_index, mat_b, mat_b_index[3]);
	}	
	...
	ret11 = init_tdmatrix(ret11_row_dim, ret11_col_dim);

	// Strassen Area
	// a11 * b11
#ifdef USE_WINOGRAD
//	_bncomp_mul_tdmatrix_winograd_even(ret11, a11, b11, min_dim);
	_bncomp_mul_tdmatrix_winograd_even2(ret11, a11, b11, min_dim);
#else
	//#pragma omp parallel
//	_bncomp_mul_tdmatrix_strassen_even3(ret11, a11, b11, min_dim, 0);
	_bncomp_mul_tdmatrix_strassen_even2(ret11, a11, b11, min_dim, 0);
//	_bncomp_mul_tdmatrix_strassen_even(ret11, a11, b11, min_dim);
#endif
	//_bncomp_mul_tdmatrix_block(ret11, a11, b11, min_dim);
	//_bncomp_mul_tdmatrix_simple(ret11, a11, b11);
	
	// ret11 += a12 * b21
	if((a12 != NULL) && (b21 != NULL))
	{
		tmp_mat = init_tdmatrix(ret11_row_dim, ret11_col_dim);

		//_bncomp_mul_tdmatrix_block(tmp_mat, a12, b21, min_dim);
		_bncomp_mul_tdmatrix_simple(tmp_mat, a12, b21);
		_bncomp_add_tdmatrix(ret11, ret11, tmp_mat);
	...
	_bncomp_subst_tdmatrix_partial_checked(ret, ret_index, ret11, ret11_index);
	...
	if(ret12_col_dim > 0)
	{
		ret12 = init_tdmatrix(ret12_row_dim, ret12_col_dim);

		// a11 * b12
		if(b12 != NULL)
			_bncomp_mul_tdmatrix_simple(ret12, a11, b12);
		//	_bncomp_mul_tdmatrix_block(ret12, a11, b12, min_dim);

		// ret12 += a12 * b22
		if((a12 != NULL) && (b22 != NULL))
		{
			tmp_mat = init_tdmatrix(ret12_row_dim, ret12_col_dim);

		//	_bncomp_mul_tdmatrix_block(tmp_mat, a12, b22, min_dim);
			_bncomp_mul_tdmatrix_simple(tmp_mat, a12, b22);
			_bncomp_add_tdmatrix(ret12, ret12, tmp_mat);
		...
		_bncomp_subst_tdmatrix_partial_checked(ret, ret_index, ret12, ret12_index);
	...
	if(ret21_row_dim > 0)
	{
		ret21 = init_tdmatrix(ret21_row_dim, ret21_col_dim);

		// a21 * b11
		if(a21 != NULL)
			_bncomp_mul_tdmatrix_simple(ret21, a21, b11);
		//	_bncomp_mul_tdmatrix_block(ret21, a21, b11, min_dim);

		// ret21 += a22 * b21
		if((a22 != NULL) && (b21 != NULL))
		{
			tmp_mat = init_tdmatrix(ret21_row_dim, ret21_col_dim);
			
			//_bncomp_mul_tdmatrix_block(tmp_mat, a22, b21, min_dim);
			_bncomp_mul_tdmatrix_simple(tmp_mat, a22, b21);
			_bncomp_add_tdmatrix(ret21, ret21, tmp_mat);
		...
		_bncomp_subst_tdmatrix_partial_checked(ret, ret_index, ret21, ret21_index);
	...
	{
		ret22 = init_tdmatrix(ret22_row_dim, ret22_col_dim);

		// a21 * b12
		if((a21 != NULL) && (b12 != NULL))
			_bncomp_mul_tdmatrix_simple(ret22, a21, b12);
		//	_bncomp_mul_tdmatrix_block(ret22, a21, b12, min_dim);

		// ret22 += a22 * b22
		if((a22 != NULL) && (b22 != NULL))
		{
			tmp_mat = init_tdmatrix(ret22_row_dim, ret22_col_dim);
			
			//_bncomp_mul_tdmatrix_block(tmp_mat, a22, b22, min_dim);
			_bncomp_mul_tdmatrix_simple(tmp_mat, a22, b22);
			_bncomp_add_tdmatrix(ret22, ret22, tmp_mat);
		...
		_bncomp_subst_tdmatrix_partial_checked(ret, ret_index, ret22, ret22_index);
...
*/
	...
	free_tdmatrix(a11);
	free_tdmatrix(b11);
	if(a12 != NULL) free_tdmatrix(a12);
	if(a21 != NULL) free_tdmatrix(a21);
	if(a22 != NULL) free_tdmatrix(a22);
	if(b12 != NULL) free_tdmatrix(b12);
	if(b21 != NULL) free_tdmatrix(b21);
	if(b22 != NULL) free_tdmatrix(b22);
...
#pragma omp parallel sections shared(mat_tmp_a, mat_tmp_b, mat_a, mat_b, mat_br_index, mat_bl_index, mat_ar_index, mat_al_index, row_dim, col_dim, mid_dim, row_dim_h, col_dim_h, mid_dim_h, mat_tmp_a_index, mat_tmp_b_index, ret_index) //num_threads(7)
{
	...
	//	_bncomp_add_tdmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
		...
		// P1 = tmp_a * tmp_b
		//printf("P1: %ld, ", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_tdmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);
		...
		// P2 = tmp_a * tmp_b
		//printf("P2: %ld, ", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_tdmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
		...
		// P3 = tmp_a * tmp_b
		//printf("P3: %ld, ", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_tdmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
		...
		// P4 = tmp_a * tmp_b
		//printf("P4: %ld, ", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_tdmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);
		...
		// P5 = tmp_a * tmp_b
		//printf("P5: %ld, ", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim, rec_num + 1);
	...
	//	_bncomp_add_tdmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
		...
		// P6 = tmp_a * tmp_b
		//printf("P6: %ld, ", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim, rec_num + 1);
	...
	//	_bncomp_add_tdmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
		...
		// P7 = tmp_a * tmp_b
		//printf("P7: %ld\n", rec_num);
		_bncomp_mul_tdmatrix_strassen_even2(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim, rec_num + 1);
...
//#pragma omp parallel
{
	...
	//	_bncomp_add_tdmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
		...
		//printf("P1: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_tdmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);
		...
		//printf("P2: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_tdmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
		...
		//printf("P3: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim, rec_num + 1);
	...
	//	_bncomp_sub_tdmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
		...
		//printf("P4: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim, rec_num + 1);
	...
	//	_bncomp_subst_tdmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);
		...
		//printf("P5: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim, rec_num + 1);
	...
	//	_bncomp_add_tdmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
		...
		//printf("P6: %ld, ", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim, rec_num + 1);
	...
	//	_bncomp_add_tdmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
		...
		//printf("P7: %ld\n", rec_num);
		#pragma omp task
		_bncomp_mul_tdmatrix_strassen_even3(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim, rec_num + 1);
...
#pragma omp parallel sections
{
	#pragma omp section
	{
		...
		// M1 := S2 * S6
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[0], mat_s[1], mat_s[5], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M2 := A11 * B11
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[1], mat_tmp_a[0], mat_tmp_b[0], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M3 := A12 * B21
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[2], mat_tmp_a[1], mat_tmp_b[2], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M4 := S3 * S7
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[3], mat_s[2], mat_s[6], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M5 := S1 * S5
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[4], mat_s[0], mat_s[4], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M6 := S4 * B22
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[5], mat_s[3], mat_tmp_b[3], min_dim);
	}
	...
	#pragma omp section
	{
		...
		// M7 := A22 * S8
		// -------------------------------
		_bncomp_mul_tdmatrix_winograd_even2(mat_m[6], mat_tmp_a[3], mat_s[7], min_dim);
	}
} // pragma omp parallel sections
...
int main(int argc, char *argv[])
{
	...
	td_real ddtmp[4];
#else // __cplusplus
	double ddtmp[4][TDSIZE];
#endif // __cplusplus
	TDMatrix mpfc, mpfa, mpfb, mpfc_normal, mpfc_block, mpfc_nonrec, mpfc_tmp;
	TDVector mpfdiag_left, mpfdiag_right;
...
//	printf("OpenMP #Threads = %ld\n", omp_get_num_threads());
	...
	mpfc = init_tdmatrix(row_dim, col_dim);
	mpfc_tmp = init_tdmatrix(row_dim, col_dim);
	mpfc_normal = init_tdmatrix(row_dim, col_dim);
	mpfc_block = init_tdmatrix(row_dim, col_dim);
	mpfa = init_tdmatrix(row_dim, mid_dim);
	mpfb = init_tdmatrix(mid_dim, col_dim);
...
//	mpfdiag_right = init_ddvector(dim);
	...
	for(i = 0; i < row_dim; i++)
	{
		for(j = 0; j < mid_dim; j++)
		{
			rtd_set_d(ddtmp[0], (double)rand());
			if(rand() % 2 != 0)
				rtd_neg(ddtmp[0], ddtmp[0]);

			rtd_set_d(ddtmp[1], (double)rand());
			rtd_ui_div(ddtmp[1], 1UL, ddtmp[1]);
			if(rand() % 2 != 0)
				rtd_neg(ddtmp[1], ddtmp[1]);

			set_tdmatrix_ij(mpfa, i, j, ddtmp[0]);
		}
	}
	...
	for(i = 0; i < mid_dim; i++)
	{
		for(j = 0; j < col_dim; j++)
		{
			...
			set_tdmatrix_ij(mpfb, i, j, ddtmp[1]);
		}
	}

	// normal matrix mul
	printf("_bncomp_mul_tdmatrix_simple...\n");
	stime = get_real_secv();
	_bncomp_mul_tdmatrix(mpfc_normal, mpfa, mpfb);
	etime[0] = get_real_secv() - stime;

	//left_scaling_tdmatrix(mpfa, mpfa, mpfdiag_left, NULL);
	//right_scaling_tdmatrix(mpfb, mpfb, mpfdiag_right, NULL);

	// blocked matrix mul
	printf("_bncomp_mul_tdmatrix_block(%ld)...\n", min_dim);
	//stime = get_secv();
	stime = get_real_secv();
	_bncomp_mul_tdmatrix_block(mpfc_block, mpfa, mpfb, min_dim);
	//etime[1] = get_secv() - stime;
	etime[1] = get_real_secv() - stime;

	// Strassen 
	printf("_bncomp_mul_tdmatrix_strassen(%ld)...\n", min_dim);
	stime = get_real_secv();
	_bncomp_mul_tdmatrix_strassen(mpfc, mpfa, mpfb, min_dim);
//	mul_tdmatrix_strassen(mpfc, mpfa, mpfb, 16);
//	mul_tdmatrix_strassen(mpfc, mpfa, mpfb, 32);
	etime[2] = get_real_secv() - stime;

	//mul_tdmatrix_dddiag(mpfc, mpfdiag_left, 0, mpfc, mpfdiag_right, 0);

	// difference
	sub_tdmatrix(mpfc_tmp  , mpfc_normal, mpfc);
	sub_tdmatrix(mpfc      , mpfc_normal, mpfc_block);
	...
	printf("strassen       : %f\n", etime[2]);
#ifdef __cplusplus
	normi_tdmatrix(&ddtmp[0], mpfc_tmp);
	normi_tdmatrix(&ddtmp[1], mpfc);
	normi_tdmatrix(&ddtmp[2], mpfc_normal);
#else // __cplusplus
	normi_tdmatrix(ddtmp[0], mpfc_tmp);
	normi_tdmatrix(ddtmp[1], mpfc);
	normi_tdmatrix(ddtmp[2], mpfc_normal);
#endif // __cplusplus
	rtd_div(ddtmp[0], ddtmp[0], ddtmp[2]);
	rtd_div(ddtmp[1], ddtmp[1], ddtmp[2]);
	printf("||reldiff_block   ||       : "); rtd_out_str(ddtmp[1]); printf("\n");
	printf("||reldiff_strassen||       : "); rtd_out_str(ddtmp[0]); printf("\n");

/* Inverse */

//	frank_tdmatrix(mpfa, dim);
//	frank_tdmatrix(mpfb, dim);
//	lotkin_tdmatrix(mpfa, dim);
//	lotkin_tdmatrix(mpfb, dim);

	free_tdmatrix(mpfc);
	free_tdmatrix(mpfc_tmp);
	free_tdmatrix(mpfc_normal);
	free_tdmatrix(mpfc_block);
	free_tdmatrix(mpfa);
	free_tdmatrix(mpfb);
