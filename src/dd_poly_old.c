/********************************************************************************/
/* dd_poly.c: Algebraic Equations and Polynomials                               */
...
void _bncavx2_eval_ddpoly_estrin(double ret[DDSIZE], DDPoly a, double x[DDSIZE])
{
    ...
    //while(num_in_coef > num_loop_unit)
    {
        ...
        for(i = 0; i < (num_in_coef / num_loop_unit); i++)
        {
            ...
            new_coef8[0] = _mm512_permute8x64_pd(new_coef8[0], (int)(3*64 + 1*16 + 2*4 + 0)); //(int)(3*64 + 1*16 + 2*4 + 0));
            new_coef8[1] = _mm512_permute8x64_pd(new_coef8[1], (int)(3*64 + 1*16 + 2*4 + 0)); //(int)(3*64 + 1*16 + 2*4 + 0));
            // AVX2   : b0, b2, b1, b3 -> b0, b1, b2, b3
            // AVX-512: b0, b4, b3, b4, b2, b1, b3 -> b0, b1, b2, b3, b4, b5, b6, b7
            //_mm256_permute4x64_pd(new_coef4, (int)(3*64 + 1*16 + 2*4 + 0));
            //printf("   new_coef4 = "); PRINT_M256D_SL(new_coef4);
            // embed 0s
