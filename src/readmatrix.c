...
//        gmp_fscanf(f, "%d %d %Fg\n", &I[i], &J[i], &val[i]);
	...
	if(symmetric_flag == 1)
	{
		...
		if(new_nzero_total_num > nzero_total_num)
		{
			...
			new_buf = (mmcoordinate *)malloc(sizeof(mmcoordinate) * new_nzero_total_num);
			if(new_buf == NULL)
			{
				fprintf(stderr, "Error: cannot allocate buf or nzero_col_dim!\n");
				free(buf);
	...
	qsort(new_buf, new_nzero_total_num, sizeof(mmcoordinate), compare_mmcoordinate);
//	for(i = 0; i < nzero_total_num; i++)
//		printf("%d %d %10.3g %d\n", buf[i].row_index, buf[i].col_index, buf[i].val, nzero_col_dim[buf[i].row_index]);
	...
	dsmat = init_drsmatrix(row_dim, nzero_col_dim, new_nzero_total_num);
	if(dsmat == NULL)
	{
		fprintf(stderr, "ERROR: cannot allocate matrix area(dim= %d x %d\n",row_dim, col_dim);
		return NULL;
	...
	for(i = 0; i < row_dim; i++)
	{
		for(j = 0; j < dsmat->nzero_col_dim[i]; j++)
		{
			//dsmat->element[total_index] = buf[buf_total_index].val;
			//dsmat->nzero_index[i][j] = buf[buf_total_index].col_index;
			dsmat->element[total_index] = new_buf[buf_total_index].val;
			dsmat->nzero_index[i][j] = new_buf[buf_total_index].col_index;
			buf_total_index++;
			total_index++;
		}
		// Fix! 2024-08-05(Mon) T.Kouya
		for(j = dsmat->nzero_col_dim[i]; j < dsmat->real_nzero_col_dim[i]; j++)
		{
			dsmat->element[total_index] = 0.0;
			total_index++;
		}
	}
...
//		return NULL;
	...
	buf = (mmcoordinate_str *)calloc(nzero_total_num, sizeof(mmcoordinate_str));
