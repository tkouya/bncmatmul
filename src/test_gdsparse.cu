/********************************************************************************/
/* test_gdsparse.cu: verify & benchmark native double GPU sparse SpMV (gdsparse)  */
/*   GPU CSR SpMV (A*x, A^T*x)  vs  CPU DRS sparse (mul_drsmatrix_dvec / _t)        */
/********************************************************************************/
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
// DRS -> standard CSR (host).  element[] uses real_nzero_col_dim stride per row;
// genuine non-zeros are the first nzero_col_dim[i] entries of each row.
static void drs_to_csr(DRSMatrix m, long int **row_ptr, long int **col_idx, double **val, long int *nnz_out)
{
...
static double max_relerr_vec(DVector ref, GDVector gpu_dev)
{
	...
	for(i = 0; i < n; i++)
	{
		double a = ref->element[i], b = h->element[i], e = fabs(a - b);
		if(a != 0.0) e /= fabs(a);
		if(e > maxe) maxe = e;
	}
	free_gdvector(h);
...
{
	long int dim = 4096, i, j;
	int nbg = 64, ntb = 128;
	if(argc >= 2) dim = atol(argv[1]);
	cudaSetDevice(0);

	// build a sparse-ish dense matrix (tridiagonal + a couple of scattered entries)
	...
	double *val;
	drs_to_csr(drs, &row_ptr, &col_idx, &val, &nnz);

	GDSPMatrix gA = init_gdspmatrix_dev(dim, dim, nnz, row_ptr, col_idx, val);
	GDVector gx = init_gdvector_dev(dim), gy = init_gdvector_dev(dim);
	subst_gdvector_dev_dvec(gx, x);

	printf("===== native double GPU sparse SpMV (gdsparse), dim = %ld, nnz = %ld =====\n", dim, nnz);

	mul_drsmatrix_dvec(y, drs, x);
	mul_gdspmatrix_gdvec(gy, gA, gx, nbg, ntb);
	cudaDeviceSynchronize();
	printf("[SpMV  A*x ] max rel err (GPU vs CPU): %.3e\n", max_relerr_vec(y, gy));

	mul_drsmatrixt_dvec(y, drs, x);
	mul_gdspmatrixt_gdvec(gy, gA, gx, nbg, ntb);
	cudaDeviceSynchronize();
	printf("[SpMV A^T*x] max rel err (GPU vs CPU): %.3e\n", max_relerr_vec(y, gy));

	// performance (repeat to amortize launch overhead)
	int reps = 100;
	double t0, t1, gpu_s, cpu_s;
	double flop = 2.0 * (double)nnz;
	t0 = wtime();
	for(int r = 0; r < reps; r++) mul_gdspmatrix_gdvec(gy, gA, gx, nbg, ntb);
	cudaDeviceSynchronize();
	t1 = wtime(); gpu_s = (t1 - t0) / reps;
	t0 = wtime();
	for(int r = 0; r < reps; r++) mul_drsmatrix_dvec(y, drs, x);
	t1 = wtime(); cpu_s = (t1 - t0) / reps;
	printf("[perf      ] SpMV GPU: %.4e s (%.2f GFLOPS) | CPU: %.4e s (%.2f GFLOPS) | speedup x%.1f\n",
