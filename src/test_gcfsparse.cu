/********************************************************************************/
/* test_gcfsparse.cu: native complex float GPU sparse SpMV (gcfsparse)            */
...
static void dense_to_csr(CDMatrix a, long int n, long int **rp_o, long int **ci_o, float **vr_o, float **vi_o, long int *nnz_o)
{
	...
	float *vr = (float *)malloc(nnz * sizeof(double)), *vi = (float *)malloc(nnz * sizeof(double));
	rp[0] = 0;
	for(i = 0; i < n; i++)
	{
		for(j = 0; j < n; j++)
...
static void transpose_csr(long int n, long int nnz, const long int *rp, const long int *ci, const float *vr, const float *vi,
                          long int **trp_o, long int **tci_o, float **tvr_o, float **tvi_o)
{
	long int i, k;
	long int *trp = (long int *)calloc(n + 1, sizeof(long int));
	long int *tci = (long int *)malloc(nnz * sizeof(long int));
	float *tvr = (float *)malloc(nnz * sizeof(double)), *tvi = (float *)malloc(nnz * sizeof(double));
...
static double max_relerr(CDVector ref, float *gyre, float *gyim, long int n)
{
	float *hr = (float *)malloc(n * sizeof(double)), *hi = (float *)malloc(n * sizeof(double)), maxe = 0.0;
...
int main(int argc, char *argv[])
{
	...
	long int *rp, *ci, nnz; float *vr, *vi;
	dense_to_csr(dense, dim, &rp, &ci, &vr, &vi, &nnz);
	GCFSPMatrix gA = init_gcfspmatrix_dev(dim, dim, nnz, rp, ci, vr, vi);

	float *hxr = (float *)malloc(dim * sizeof(double)), *hxi = (float *)malloc(dim * sizeof(double));
	for(i = 0; i < dim; i++) { double _Complex z = get_cdvector_i(x, i); hxr[i] = _cre(z); hxi[i] = _cim(z); }
	float *gxr, *gxi, *gyr, *gyi;
	...
	printf("===== native complex float GPU sparse SpMV (gcfsparse), dim = %ld, nnz = %ld =====\n", dim, nnz);

	mul_cdmatrix_cdvec(y, dense, x);
	mul_gcfspmatrix(gyr, gyi, gA, gxr, gxi, nbg, ntb);
	cudaDeviceSynchronize();
	printf("[SpMV  A*x ] max rel err (GPU sparse vs CPU dense): %.3e\n", max_relerr(y, gyr, gyi, dim));

	long int *trp, *tci; float *tvr, *tvi;
	transpose_csr(dim, nnz, rp, ci, vr, vi, &trp, &tci, &tvr, &tvi);
	GCFSPMatrix gAt = init_gcfspmatrix_dev(dim, dim, nnz, trp, tci, tvr, tvi);
	mul_cdmatrixt_cdvec(y, dense, x);
	mul_gcfspmatrix(gyr, gyi, gAt, gxr, gxi, nbg, ntb);
	cudaDeviceSynchronize();
	printf("[SpMV A^T*x] max rel err (GPU sparse vs CPU dense): %.3e\n", max_relerr(y, gyr, gyi, dim));

	int reps = 200; double t0, t1, gpu_s;
	t0 = wtime(); for(int r = 0; r < reps; r++) mul_gcfspmatrix(gyr, gyi, gA, gxr, gxi, nbg, ntb); cudaDeviceSynchronize(); t1 = wtime(); gpu_s = (t1 - t0) / reps;
