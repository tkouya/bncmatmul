...
void _bncomp_mul_qdmatrix_oz(QDMatrix ret, QDMatrix a, int max_num_div_a, QDMatrix b, int max_num_div_b) //, int num_digits)
{
    int i, j;
...
#endif // USE_IMKL
            //add_qdmatrix_dmat(ret, ret, div_ret[thread_index]);	 
            //add_qdmatrix_dmat(ret, ret, div_ret[j]);
			#pragma omp critical
				add_qdmatrix_dmat(ret, ret, div_ret[i]);
    	}
