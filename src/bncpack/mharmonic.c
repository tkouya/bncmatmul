...
/* 2^n - 1 */
unsigned long ex_nim_den(long int n)
{
...
double dmharmonic_integral_1step(double x_start, double x_end, double (* func)(double), long int num_stage, double reps, double aeps, long int *conv_flag)
{
	...
	{
		num_steps += 2;
		ex_table[i][0] = dmtrapezoidal_fs(x_start, x_end, func, num_steps);
...
double dmharmonic_integral(double x_start, double x_end, double (* func)(double), long int max_num_stage)
{
	...
	double ret, tmp, in_h, old_in_h, old_in_x_end, in_x_start, in_x_end;
...
//	in_num_stage = max_num_stage;
	...
	do
	{
		tmp = dmharmonic_integral_1step(in_x_start, in_x_end, func, in_num_stage, 1.0e-12, 0.0, &conv_flag);
		if((tmp + ret) == ret)
			conv_flag = 1;
		if(conv_flag != 1)
		{
//			printf("Not Convergent! %e->%e\n", in_h, in_h/2);
		...
		else
		{
			...
			ret += tmp;
			printf("y %5ld %10.3e [%25.17e, %10.3e] %5ld %10.3e\n", times++, in_h, in_x_start, in_x_end, in_num_stage, tmp);
			if(in_x_end >= x_end)
				break;
...
{
	long int steps, conv_flag, end_flag, times, in_num_stage;
	double ret, tmp, in_h, old_in_h, old_in_x_start, in_x_start, in_x_end;

	in_h = (x_end - x_start) / 2;
	old_in_h = in_h;
	in_x_end = x_end;
	in_x_start = in_x_end - in_h;
	in_num_stage = max_num_stage / 2;
//	in_num_stage = max_num_stage;
...
//	in_num_stage = max_num_stage;
	...
	do
	{
		tmp = dmromberg_integral_1step(in_x_start, in_x_end, func, in_num_stage, 1.0e-12, 0.0, &conv_flag);
		if((tmp + ret) == ret)
			conv_flag = 1;
		if(conv_flag != 1)
		{
...
void mpf_mharmonic_integral_1step(mpf_t ret, mpf_t x_start, mpf_t x_end, void (* func)(mpf_t, mpf_t), long int num_stage, mpf_t reps, mpf_t aeps, long int *conv_flag)
{
	...
	{
		num_steps += 2;
		mpf_mtrapezoidal_fs(ex_table[i][0], x_start, x_end, func, num_steps);
...
//	in_num_stage = max_num_stage;
	...
	do
	{
		mpf_mharmonic_integral_1step(tmp, in_x_start, in_x_end, func, in_num_stage, reps, aeps, &conv_flag);
...
//	in_num_stage = max_num_stage;
	...
	do
	{
		mpf_mromberg_integral_1step(tmp, in_x_start, in_x_end, func, in_num_stage, reps, aeps, &conv_flag);
		mpf_add(tmp1, tmp, ret);
		if(mpf_cmp(tmp1, ret) == 0)
			conv_flag = 1;
		if(conv_flag != 1)
