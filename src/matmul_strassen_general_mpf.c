...
long int num_addsub_mul_mpfmatrix_strassen;	// addition and subtraction
long int num_mul_mul_mpfmatrix_strassen;	// multiplication
...
void add_mpfmatrix_partial(MPFMatrix ret, long int ret_index[4], MPFMatrix mat_a, long int mat_a_index[4], MPFMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			mpf_add(tmp_val, get_mpfmatrix_ij(mat_a, a_i, a_j), get_mpfmatrix_ij(mat_b, b_i, b_j));
			set_mpfmatrix_ij(ret, ret_i, ret_j, tmp_val);
		}
	}
...
void sub_mpfmatrix_partial(MPFMatrix ret, long int ret_index[4], MPFMatrix mat_a, long int mat_a_index[4], MPFMatrix mat_b, long int mat_b_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			//tmp_val = get_mpfmatrix_ij(mat_a, a_i, a_j) - get_mpfmatrix_ij(mat_b, b_i, b_j);
			mpf_sub(tmp_val, get_mpfmatrix_ij(mat_a, a_i, a_j), get_mpfmatrix_ij(mat_b, b_i, b_j));
			set_mpfmatrix_ij(ret, ret_i, ret_j, tmp_val);
		}
	}
...
void neg_mpfmatrix_partial(MPFMatrix ret, long int ret_index[4], MPFMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			ret_j = ret_index[2] + j;
			a_j = mat_a_index[2] + j;
			mpf_neg(tmp, get_mpfmatrix_ij(mat_a, a_i, a_j));
			set_mpfmatrix_ij(ret, ret_i, ret_j, tmp);
		}
	}
...
void mul_mpfmatrix_simple(MPFMatrix c, MPFMatrix a, MPFMatrix b)
{
	...
	{
		fprintf(stderr, "ERROR: mul_mpfmatrix_simple(c->row_dim, a->row_dim) = (%ld, %ld)\n", c->row_dim, a->row_dim);
		return;
	}
	if(c->col_dim != b->col_dim)
	{
		fprintf(stderr, "ERROR: mul_mpfmatrix_simple(c->col_dim, b->col_dim) = (%ld, %ld)\n", c->col_dim, b->col_dim);
		return;
	}
	if(a->col_dim != b->row_dim)
	{
		fprintf(stderr, "ERROR: mul_mpfmatrix_simple(a->col_dim, b->col_dim) = (%ld, %ld)\n", a->col_dim, b->row_dim);
		return;
	}

	mpf_init2(tmp , c->prec);
	mpf_init2(tmp2, c->prec);
	...
	for(i = 0; i < c->row_dim; i++)
	{
		for(j = 0; j < c->col_dim; j++)
		{
			...
			{
				mpf_mul(tmp, get_mpfmatrix_ij(a, i, k), get_mpfmatrix_ij(b, k, j));
				mpf_add(tmp2, tmp2, tmp);
			}
			set_mpfmatrix_ij(c, i, j, tmp2);
		}
	}
...
void mul_mpfmatrix_block(MPFMatrix ret, MPFMatrix mat_a, MPFMatrix mat_b, long int min_dim)
{
	...
	small_tmp_mat = init2_mpfmatrix(min_dim, min_dim, prec);
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		for(j = 0; j < num_div_mid; j++)
		{
			...
			subst_mpfmatrix_partial_checked(small_mat_a[j], small_mat_a_index, mat_a, mat_a_index);
		}
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			for(k = 0; k < num_div_mid; k++)
			{
				...
				subst_mpfmatrix_partial_checked(small_mat_b[k], small_mat_b_index, mat_b, mat_b_index);

				// ret[j] += small_mat_a[i][k] * small_mat_b[k][j];
				mul_mpfmatrix(small_tmp_mat, small_mat_a[k], small_mat_b[k]);
				add_mpfmatrix(small_ret[j], small_ret[j], small_tmp_mat);
			}
			...
			subst_mpfmatrix_partial_checked(ret, ret_index, small_ret[j], small_ret_index);
		}
	}

	// free
	free_mpfmatrix(small_tmp_mat);
	for(i = 0; i < num_div_col; i++)
...
// Padding to even dimensional matrix
MPFMatrix init_dynamic_padding_mpfmatrix_strassen2(MPFMatrix orig_mat, long int min_dim)
{
	unsigned long prec;
	MPFMatrix ret = NULL;
...
void mul_mpfmatrix_strassen_odd_padding(MPFMatrix ret, MPFMatrix mat_a, MPFMatrix mat_b, long int min_dim)
{
	...
	//printf("start mul_mpfmatrix_strassen_odd_padding\n");

	// padding
#ifdef USE_STATIC_PADDING
	tmp_ret = init_static_padding_mpfmatrix_strassen(ret);
	tmp_mat_a = init_static_padding_mpfmatrix_strassen(mat_a);
	tmp_mat_b = init_static_padding_mpfmatrix_strassen(mat_b);
#else
//	tmp_ret = init_dynamic_padding_mpfmatrix_strassen(ret);
//	tmp_mat_a = init_dynamic_padding_mpfmatrix_strassen(mat_a);
//	tmp_mat_b = init_dynamic_padding_mpfmatrix_strassen(mat_b);
	...
	// strassen
#ifdef USE_WINOGRAD
	mul_mpfmatrix_winograd_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#else
	mul_mpfmatrix_strassen_even(tmp_ret, tmp_mat_a, tmp_mat_b, min_dim);
#endif
	...
	//printf("end of mul_mpfmatrix_strassen_odd_padding\n");
...
{
	num_addsub_mul_mpfmatrix_strassen = 0;
	num_mul_mul_mpfmatrix_strassen = 0;
}
...
void get_num_mul_mpfmatrix_strassen(long int *num_addsub, long int *num_mul)
{
	...
	if(num_addsub != NULL)
		*num_addsub = num_addsub_mul_mpfmatrix_strassen;
	if(num_mul != NULL)
		*num_mul = num_mul_mul_mpfmatrix_strassen;
}
...
//	MPFVector diag_left, diag_right;
	...
	if((ret->row_dim <= min_dim) && (ret->col_dim <= min_dim) && (mid_dim <= min_dim))
	{
		...
		num_addsub_mul_mpfmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
		num_mul_mul_mpfmatrix_strassen += mat_a->col_dim * (mat_a->row_dim * mat_b->col_dim);
...
#elif PADDING_ONLY
		mul_mpfmatrix_strassen_odd_padding(ret, mat_a, mat_b, min_dim);
#else
...
#ifdef USE_WINOGRAD
		mul_mpfmatrix_winograd_even(ret, mat_a, mat_b, min_dim);
#else
		mul_mpfmatrix_strassen_even(ret, mat_a, mat_b, min_dim);
#endif
		//printf("end\n");
	}
...
void mul_mpfmatrix_strassen_odd_peeling(MPFMatrix ret, MPFMatrix mat_a, MPFMatrix mat_b, long int min_dim)
{
	...
	//printf("start mul_mpfmatrix_strassen_odd_peeling\n");
	...
	// C11 = A11 * B11
#ifdef USE_WINOGRAD
	mul_mpfmatrix_winograd_even(mat_c11, mat_a11, mat_b11, min_dim);
#else
	mul_mpfmatrix_strassen_even(mat_c11, mat_a11, mat_b11, min_dim);
#endif
	...
	if((vec_a12 != NULL) && (vec_b21 != NULL))
	{
		...
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += row_dim_h * col_dim_h;
		num_mul_mul_mpfmatrix_strassen += row_dim_h * col_dim_h;
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;
		num_mul_mul_mpfmatrix_strassen += mat_a11->row_dim * mat_a11->col_dim;
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += vec_c12->dim;
		num_mul_mul_mpfmatrix_strassen += vec_tmp12->dim;
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;
		num_mul_mul_mpfmatrix_strassen += mat_b11->row_dim * mat_b11->col_dim;
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += vec_c21->dim;
		num_mul_mul_mpfmatrix_strassen += vec_tmp21->dim;
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += vec_a21->dim;
		num_mul_mul_mpfmatrix_strassen += vec_tmp21->dim;
	...
	{
		// counting the number of arithmetic
		num_addsub_mul_mpfmatrix_strassen += 1;
		num_mul_mul_mpfmatrix_strassen += 1;
	...
	mpf_clear(tmp);
	//printf("mpfclear\n");
	//printf("end of mul_mpfmatrix_strassen_odd_peeling\n");
}
...
//	printf("P1a:\n");
	...
	// counting the number of arithmetic
	num_addsub_mul_mpfmatrix_strassen += row_dim_h * mid_dim_h;

//	add_mpfmatrix_partial(mat_tmp_a[0], ret_index, mat_a, mat_ar_index[0], mat_a, mat_al_index[0]);
...
//	printf("P1b:\n");
	...
	// counting the number of arithmetic
	num_addsub_mul_mpfmatrix_strassen += mid_dim_h * col_dim_h;

//	add_mpfmatrix_partial(mat_tmp_b[0], ret_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);
	add_mpfmatrix_partial(mat_tmp_b[0], mat_tmp_b_index, mat_b, mat_br_index[0], mat_b, mat_bl_index[0]);

	// P1 = tmp_a * tmp_b
//	printf("P1:\n");
	mul_mpfmatrix_strassen(mat_p[0], mat_tmp_a[0], mat_tmp_b[0], min_dim);
...
//	add_mpfmatrix_partial(mat_tmp_a[1], ret_index, mat_a, mat_ar_index[1], mat_a, mat_al_index[1]);
...
//	subst_mpfmatrix_partial(mat_tmp_b[1], ret_index, mat_b, mat_br_index[1]);
	subst_mpfmatrix_partial(mat_tmp_b[1], mat_tmp_b_index, mat_b, mat_br_index[1]);

	// P2 = tmp_a * tmp_b
	//printf("P2:\n");
	mul_mpfmatrix_strassen(mat_p[1], mat_tmp_a[1], mat_tmp_b[1], min_dim);
...
//	subst_mpfmatrix_partial(mat_tmp_a[2], ret_index, mat_a, mat_ar_index[2]);
...
//	sub_mpfmatrix_partial(mat_tmp_b[2], ret_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);
	sub_mpfmatrix_partial(mat_tmp_b[2], mat_tmp_b_index, mat_b, mat_br_index[2], mat_b, mat_bl_index[2]);

	// P3 = tmp_a * tmp_b
	//printf("P3:\n");
	mul_mpfmatrix_strassen(mat_p[2], mat_tmp_a[2], mat_tmp_b[2], min_dim);
...
//	subst_mpfmatrix_partial(mat_tmp_a[3], ret_index, mat_a, mat_ar_index[3]);
...
//	sub_mpfmatrix_partial(mat_tmp_b[3], ret_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);
	sub_mpfmatrix_partial(mat_tmp_b[3], mat_tmp_b_index, mat_b, mat_br_index[3], mat_b, mat_bl_index[3]);

	// P4 = tmp_a * tmp_b
	//printf("P4:\n");
	mul_mpfmatrix_strassen(mat_p[3], mat_tmp_a[3], mat_tmp_b[3], min_dim);
...
//	add_mpfmatrix_partial(mat_tmp_a[4], ret_index, mat_a, mat_ar_index[4], mat_a, mat_al_index[4]);
...
//	subst_mpfmatrix_partial(mat_tmp_b[4], ret_index, mat_b, mat_br_index[4]);
	subst_mpfmatrix_partial(mat_tmp_b[4], mat_tmp_b_index, mat_b, mat_br_index[4]);

	// P5 = tmp_a * tmp_b
	//printf("P5:\n");
	mul_mpfmatrix_strassen(mat_p[4], mat_tmp_a[4], mat_tmp_b[4], min_dim);
...
//	sub_mpfmatrix_partial(mat_tmp_a[5], ret_index, mat_a, mat_ar_index[5], mat_a, mat_al_index[5]);
...
//	add_mpfmatrix_partial(mat_tmp_b[5], ret_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);
	add_mpfmatrix_partial(mat_tmp_b[5], mat_tmp_b_index, mat_b, mat_br_index[5], mat_b, mat_bl_index[5]);

	// P6 = tmp_a * tmp_b
	//printf("P6:\n");
	mul_mpfmatrix_strassen(mat_p[5], mat_tmp_a[5], mat_tmp_b[5], min_dim);
...
//	sub_mpfmatrix_partial(mat_tmp_a[6], ret_index, mat_a, mat_ar_index[6], mat_a, mat_al_index[6]);
...
//	add_mpfmatrix_partial(mat_tmp_b[6], ret_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);
	add_mpfmatrix_partial(mat_tmp_b[6], mat_tmp_b_index, mat_b, mat_br_index[6], mat_b, mat_bl_index[6]);

	// P7 = tmp_a * tmp_b
	//printf("P7:\n");
	mul_mpfmatrix_strassen(mat_p[6], mat_tmp_a[6], mat_tmp_b[6], min_dim);

	// -------------------------------
	// C11 := P1 + P4 - P5 + P7
	// -------------------------------
	// counting the number of arithmetic
	num_addsub_mul_mpfmatrix_strassen += 3 * row_dim_h * col_dim_h;
...
//	printf("s...\n");
	...
	// M1 := S2 * S6
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[0], mat_s[1], mat_s[5], min_dim);

	// -------------------------------
	// M2 := A11 * B11
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[1], mat_tmp_a[0], mat_tmp_b[0], min_dim);

	// -------------------------------
	// M3 := A12 * B21
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[2], mat_tmp_a[1], mat_tmp_b[2], min_dim);

	// -------------------------------
	// M4 := S3 * S7
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[3], mat_s[2], mat_s[6], min_dim);

	// -------------------------------
	// M5 := S1 * S5
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[4], mat_s[0], mat_s[4], min_dim);

	// -------------------------------
	// M6 := S4 * B22
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[5], mat_s[3], mat_tmp_b[3], min_dim);

	// -------------------------------
	// M7 := A22 * S8
	// -------------------------------
	mul_mpfmatrix_strassen(mat_m[6], mat_tmp_a[3], mat_s[7], min_dim);
...
//	printf("t...\n");
	...
	// -------------------------------
	// counting the number of arithmetic
	num_addsub_mul_mpfmatrix_strassen += 2 * row_dim_h * col_dim_h;
...
{
	unsigned long prec;
	MPFMatrix mat_p[6], mat_tmp_a[4], mat_p3p6, mat_p6p2, mat_p3p6p2;
	long int dim_h, dim;
	...
	{
		fprintf(stderr, "ERROR: Impossible to get inverse matrix for rectangle matrix!(inv_mpfmatrix_strassen_even)\n");
		return;
	}
	...
	mat_p3p6 = init2_mpfmatrix(dim_h, dim_h, prec);
	mat_p6p2 = init2_mpfmatrix(dim_h, dim_h, prec);
	mat_p3p6p2 = init2_mpfmatrix(dim_h, dim_h, prec);

	for(i = 0; i < 6; i++)
		mat_p[i] = init2_mpfmatrix(dim_h, dim_h, prec);

	for(i = 0; i < 4; i++)
		mat_tmp_a[i] = init2_mpfmatrix(dim_h, dim_h, prec);
	...
	subst_mpfmatrix_partial(mat_tmp_a[0], ret_index, mat_a, mat_a_index[0]);

	// P1 = tmp_a * tmp_b
	//printf("P1: %ld -> %ld start \n", dim, dim_h);
	inv_mpfmatrix_strassen_even(mat_p[0], mat_tmp_a[0], min_dim);
	//inv_mpfmatrix_strassen(mat_p[0], mat_tmp_a[0], min_dim);
	//printf("P1: %ld -> %ld end \n", dim, dim_h);
	...
	subst_mpfmatrix_partial(mat_tmp_a[2], ret_index, mat_a, mat_a_index[2]);

	// P2 := A21 * P1
	mul_mpfmatrix_strassen(mat_p[1], mat_tmp_a[2], mat_p[0], min_dim);
	...
	subst_mpfmatrix_partial(mat_tmp_a[1], ret_index, mat_a, mat_a_index[1]);

	// P3 = P1 * A12
	mul_mpfmatrix_strassen(mat_p[2], mat_p[0], mat_tmp_a[1], min_dim);

	// -------------------------------
	// P4 := A21 * P3
	// -------------------------------
	mul_mpfmatrix_strassen(mat_p[3], mat_tmp_a[2], mat_p[2], min_dim);
	...
	mat_a_index[3][3] = dim;
	subst_mpfmatrix_partial(mat_tmp_a[3], ret_index, mat_a, mat_a_index[3]);

	// P5 = P4 - A22
	sub_mpfmatrix(mat_p[4], mat_p[3], mat_tmp_a[3]);
	...
	inv_mpfmatrix_strassen_even(mat_p[5], mat_p[4], min_dim);
	//inv_mpfmatrix_strassen(mat_p[5], mat_p[5], min_dim);
	//printf("P6: %ld -> %ld end\n", dim, dim_h);

	// -------------------------------
	// P3P6 := P3 * P6
	// -------------------------------
	mul_mpfmatrix_strassen(mat_p3p6, mat_p[2], mat_p[5], min_dim);

	// -------------------------------
	// P6P2 := P6 * P2
	// -------------------------------
	mul_mpfmatrix_strassen(mat_p6p2, mat_p[5], mat_p[1], min_dim);

	// -------------------------------
	// P3P6P2 := P6 * P2
	// -------------------------------
	mul_mpfmatrix_strassen(mat_p3p6p2, mat_p3p6, mat_p[1], min_dim);

	// -------------------------------
	// RET11 := P1 - P3 * P6 * P2
	// -------------------------------
	sub_mpfmatrix_partial(ret, mat_a_index[0], mat_p[0], ret_index, mat_p3p6p2, ret_index);

	// -------------------------------
	// RET12 := P3 * P6
	// -------------------------------
	subst_mpfmatrix_partial(ret, mat_a_index[1], mat_p3p6, ret_index);

	// -------------------------------
	// RET21 := P6 * P2
	// -------------------------------
	subst_mpfmatrix_partial(ret, mat_a_index[2], mat_p6p2, ret_index);

	// -------------------------------
	// RET22 := -P6
	// -------------------------------
	neg_mpfmatrix_partial(ret, mat_a_index[3], mat_p[5], ret_index);

	// free
	free_mpfmatrix(mat_p3p6);
	free_mpfmatrix(mat_p6p2);
	free_mpfmatrix(mat_p3p6p2);
...
void left_scaling_mpfmatrix(MPFMatrix ret, MPFMatrix org_mat, MPFVector scaling_diag_mat, long int *ret_col_index)
{
	unsigned long prec;
	...
	for(i = 0; i < row_dim; i++)
	{
		// search absmax value
		absmax_col_index = 0;
		//absmax_val = fabs(get_mpfmatrix_ij(org_mat, i, 0));
		mpf_abs(absmax_val, get_mpfmatrix_ij(org_mat, i, 0));
		for(j = 1; j < col_dim; j++)
		{
			//tmp_abs = fabs(get_mpfmatrix_ij(org_mat, i, j));
			mpf_abs(tmp_abs, get_mpfmatrix_ij(org_mat, i, j));
		...
		if(scaling_diag_mat != NULL)
			set_mpfvector_i(scaling_diag_mat, i, absmax_val);
		...
		{
			for(j = 0; j < col_dim; j++)
				set_mpfmatrix_ij(ret, i, j, get_mpfmatrix_ij(org_mat, i, j));
		}
		else
		{
			for(j = 0; j < col_dim; j++)
			{
				//tmp_abs = get_mpfmatrix_ij(org_mat, i, j) / absmax_val;
				mpf_div(tmp_abs, get_mpfmatrix_ij(org_mat, i, j), absmax_val);
				set_mpfmatrix_ij(ret, i, j, tmp_abs);
			}
		}
	}
...
void right_scaling_mpfmatrix(MPFMatrix ret, MPFMatrix org_mat, MPFVector scaling_diag_mat, long int *ret_row_index)
{
	unsigned long prec;
	...
	for(i = 0; i < col_dim; i++)
	{
		// search absmax value
		absmax_row_index = 0;
		//absmax_val = fabs(get_mpfmatrix_ij(org_mat, 0, i));
		mpf_abs(absmax_val, get_mpfmatrix_ij(org_mat, 0, i));
		for(j = 1; j < row_dim; j++)
		{
			//tmp_abs = fabs(get_mpfmatrix_ij(org_mat, j, i));
			mpf_abs(tmp_abs, get_mpfmatrix_ij(org_mat, j, i));
			if(mpf_cmp(absmax_val, tmp_abs) < 0)
			{
				absmax_row_index = j;
				mpf_set(absmax_val, tmp_abs);
		...
		{
			for(j = 0; j < row_dim; j++)
				set_mpfmatrix_ij(ret, j, i, get_mpfmatrix_ij(org_mat, j, i));
		}
		else
		{
			for(j = 0; j < row_dim; j++)
			{
				//tmp_abs = get_mpfmatrix_ij(org_mat, j, i) / absmax_val;
				mpf_div(tmp_abs, get_mpfmatrix_ij(org_mat, j, i), absmax_val);
				set_mpfmatrix_ij(ret, j, i, tmp_abs);
			}
		}
	}
...
void mul_mpfmatrix_mpfdiag(MPFMatrix ret, MPFVector left_diag_mat, int left_inv_flag, MPFMatrix org_mat, MPFVector right_diag_mat, int right_inv_flag)
{
	unsigned long prec;
	...
	if(left_diag_mat != NULL)
	{
		...
		for(i = 0; i < row_dim; i++)
		{
			...
			if(left_inv_flag == 0)
			{
				for(j = 0; j < col_dim; j++)
				{
					//tmp = get_mpfvector_i(left_diag_mat, i) * get_mpfmatrix_ij(org_mat, i, j);
					mpf_mul(tmp, get_mpfvector_i(left_diag_mat, i), get_mpfmatrix_ij(org_mat, i, j));
			...
			else
			{
				for(j = 0; j < col_dim; j++)
				{
					//tmp = get_mpfmatrix_ij(org_mat, i, j) / get_dvector_i(left_diag_mat, i);
					mpf_div(tmp, get_mpfmatrix_ij(org_mat, i, j), get_mpfvector_i(left_diag_mat, i));
					set_mpfmatrix_ij(ret, i, j, tmp);
				}
			}
		}
	...
	if(right_diag_mat != NULL)
	{
		...
		for(i = 0; i < row_dim; i++)
		{
			...
			if(right_inv_flag == 0)
			{
				for(j = 0; j < col_dim; j++)
				{
					//tmp = get_mpfmatrix_ij(ret, i, j) * get_dvector_i(right_diag_mat, j);
					mpf_mul(tmp, get_mpfmatrix_ij(ret, i, j), get_mpfvector_i(right_diag_mat, j));
					set_mpfmatrix_ij(ret, i, j, tmp);
				}
			}
			...
			else
			{
				...
				{
					//tmp = get_mpfmatrix_ij(ret, i, j) / get_dvector_i(right_diag_mat, j);
					mpf_div(tmp, get_mpfmatrix_ij(ret, i, j), get_mpfvector_i(right_diag_mat, j));
					set_mpfmatrix_ij(ret, i, j, tmp);
				}
			}
		}
...
int main(int argc, char *argv[])
{
	...
	mpf_t mpftmp[2];
	MPFMatrix mpfc, mpfa, mpfb, mpfc_normal;
...
//	set_bnc_default_prec_decimal(prec_d);
 	set_bnc_default_prec(prec);

	mpf_init(mpftmp[0]);
	mpf_init(mpftmp[1]);

	mpfc = init_mpfmatrix(dim, dim);
	mpfc_normal = init_mpfmatrix(dim, dim);
	mpfa = init_mpfmatrix(dim, dim);
	mpfb = init_mpfmatrix(dim, dim);

	mpfdiag_left = init_mpfvector(dim);
	mpfdiag_right = init_mpfvector(dim);

	for(i = 0; i < dim; i++)
	{
		for(j = 0; j < dim; j++)
		{
			mpf_set_d(mpftmp[0], (double)rand());
			if(rand() % 2 != 0)
				mpf_neg(mpftmp[0], mpftmp[0]);

			mpf_set_d(mpftmp[1], (double)rand());
			mpf_ui_div(mpftmp[1], 1UL, mpftmp[1]);
			if(rand() % 2 != 0)
				mpf_neg(mpftmp[1], mpftmp[1]);

			set_mpfmatrix_ij(mpfa, i, j, mpftmp[0]);
			set_mpfmatrix_ij(mpfb, i, j, mpftmp[1]);
		}
	}
...
//	mul_dmatrix_strassen(dc, da, db, 512);
	mul_mpfmatrix_strassen(mpfc, mpfa, mpfb, 8);
//	mul_mpfmatrix_strassen(mpfc, mpfa, mpfb, 16);
//	mul_mpfmatrix_strassen(mpfc, mpfa, mpfb, 32);
	etime[0] = get_secv() - stime;

	mul_mpfmatrix_mpfdiag(mpfc, mpfdiag_left, 0, mpfc, mpfdiag_right, 0);

	// difference
	sub_mpfmatrix(mpfa, mpfc_normal, mpfc);

	// print
	printf("dim        : %ld\n", dim);
	printf("strassen   : %f\n", etime[0]);
	printf("normal     : %f\n", etime[1]);
	normi_mpfmatrix(mpftmp[0], mpfa);
	normi_mpfmatrix(mpftmp[1], mpfc_normal);
	mpf_div(mpftmp[0], mpftmp[0], mpftmp[1]);
	printf("||reldiff||: "); mpf_out_str(stdout, 10, 5, mpftmp[0]); printf("\n");
...
//	lotkin_mpfmatrix(mpfb, dim);
	...
	stime = get_secv();
	inv_mpfmatrix_strassen_even(mpfc, mpfb, 32);
	etime[0] = get_secv() - stime;
...
//	print_dmatrix(dc);
	...
	// difference
	sub_mpfmatrix(mpfb, mpfa, mpfc);
	normi_mpfmatrix(mpftmp[0], mpfb);
	normi_mpfmatrix(mpftmp[1], mpfa);
	mpf_div(mpftmp[0], mpftmp[0], mpftmp[1]);
	...
	printf("normal     : %f\n", etime[1]);
	printf("||a^(-1)|| strass : ");
		normi_mpfmatrix(mpftmp[1], mpfc);
		mpf_out_str(stdout, 10, 0, mpftmp[1]);
	printf("\n");
	printf("||a^(-1)||        : ");
		normi_mpfmatrix(mpftmp[1], mpfa);
		mpf_out_str(stdout, 10, 0, mpftmp[1]);
	...
	printf("||reldiff_strass||: ");
		mpf_out_str(stdout, 10, 5, mpftmp[0]);
	printf("\n");
	...
	free_mpfvector(mpfdiag_left);
	free_mpfvector(mpfdiag_right);
