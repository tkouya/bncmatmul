...
int CDDLUdecomp_square(CDDMatrix a, long int start_index, long int min_dim)
{
	...
	for(i = start_index; i < imax; i++)
	{
		...
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_square)!\n", i);
			return -1;
		}
...
int CDDLUdecomp_square_omp(CDDMatrix a, long int start_index, long int min_dim)
{
	...
	long int i, j, k, imax, jmax, itmp, dim;
	//static 
	double dtmp[BNCOMP_MAX_NUM_THREADS][DDSIZE], dtmp1[BNCOMP_MAX_NUM_THREADS][DDSIZE], dmaxii[DDSIZE];
	//static 
	cddfloat ctmp[BNCOMP_MAX_NUM_THREADS], ctmp1[BNCOMP_MAX_NUM_THREADS];
	...
	#pragma omp parallel private(thread_index)
	{
		...
		rcdd_set0(&ctmp[thread_index]);
		rcdd_set0(&ctmp1[thread_index]);
	}
	...
	for(i = start_index; i < imax; i++)
	{
		...
		if(rdd_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_square_omp)!\n", i);
			return -1;
		}
		...
		for(j = (i + 1); j < jmax; j++)
		{
			...
			rcdd_div(&ctmp[thread_index], get_cddmatrix_ij(a, j, i), get_cddmatrix_ij(a, i, i));
			set_cddmatrix_ij(a, j, i, &ctmp[thread_index]);
		}
		...
		for(j = (i + 1); j < jmax; j++)
		{
			...
			{
				//get_dmatrix_ij(a, j, k) - get_dmatrix_ij(a, j, i) * get_dmatrix_ij(a, i, k)
				rcdd_mul(&ctmp1[thread_index], get_cddmatrix_ij(a, j, i), get_cddmatrix_ij(a, i, k));
				rcdd_sub(&ctmp[thread_index], get_cddmatrix_ij(a, j, k), &ctmp1[thread_index]);
				set_cddmatrix_ij(a, j, k, &ctmp[thread_index]);
			}
		}
	}
...
int CDDLUdecomp_l21(CDDMatrix l21, CDDMatrix a, long int start_index, long int min_dim)
{
	...
	for(i = imax; i < dim; i++)
	{
		...
		if(rdd_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_l21)!\n", i);
			return -1;
		}

		for(j = start_index; j < jmax; j++)
		{
			rcdd_set(&ctmp, get_cddmatrix_ij(a, i, j));
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_dmatrix_ij(a, i, k) * get_dmatrix_ij(a, k, j);
				rcdd_mul(&ctmp1, get_cddmatrix_ij(a, i, k), get_cddmatrix_ij(a, k, j));
				rcdd_sub(&ctmp, &ctmp, &ctmp1);
			}
			rcdd_div(&ctmp1, &ctmp, get_cddmatrix_ij(a, j, j));
			rcdd_set(&ctmp, &ctmp1);
			...
			set_cddmatrix_ij(l21, i - imax, j - start_index, &ctmp);
		}
	}
...
int CDDLUdecomp_l21_omp(CDDMatrix l21, CDDMatrix a, long int start_index, long int min_dim)
{
	...
	long int i, j, k, imax, jmax, itmp, dim;
	//static 
	double dtmp[BNCOMP_MAX_NUM_THREADS][DDSIZE], dtmp1[BNCOMP_MAX_NUM_THREADS][DDSIZE], dmaxii[BNCOMP_MAX_NUM_THREADS][DDSIZE];
    //static 
	cddfloat ctmp[BNCOMP_MAX_NUM_THREADS], ctmp1[BNCOMP_MAX_NUM_THREADS];
	...
	for(i = imax; i < dim; i++)
	{
		...
		rcdd_abs_dd(dmaxii[thread_index], get_cddmatrix_ij(a, i, i));
		if(rdd_cmp_ui(dmaxii[thread_index], 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_l21_omp)!\n", i);
			//return -1;
		}

		for(j = start_index; j < jmax; j++)
		{
			rcdd_set(&ctmp[thread_index], get_cddmatrix_ij(a, i, j));
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_dmatrix_ij(a, i, k) * get_dmatrix_ij(a, k, j);
				rcdd_mul(&ctmp1[thread_index], get_cddmatrix_ij(a, i, k), get_cddmatrix_ij(a, k, j));
				rcdd_sub(&ctmp[thread_index], &ctmp[thread_index], &ctmp1[thread_index]);
			}
			rcdd_div(&ctmp1[thread_index], &ctmp[thread_index], get_cddmatrix_ij(a, j, j));
			rcdd_set(&ctmp[thread_index], &ctmp1[thread_index]);
			
			//printf("l21(i              , j       ) = (%ld, %ld), %ld, %ld\n", i, j, l21->row_dim, l21->col_dim);
			//printf("l21(i - start_index, j - imax) = (%ld, %ld), %ld, %ld\n", i - imax, j - start_index, l21->row_dim, l21->col_dim);
			//printf("(i              , j       ) = (%ld, %ld) %25.17e, %25.17e\n", i, j, dtmp, get_dmatrix_ij(a, j, j));
			set_cddmatrix_ij(a  , i              , j       , &ctmp[thread_index]);
			set_cddmatrix_ij(l21, i - imax, j - start_index, &ctmp[thread_index]);
		}
	}
...
int CDDLUdecomp_u12(CDDMatrix u12, CDDMatrix a, long int start_index, long int min_dim)
{
	...
	for(i = imax; i < dim; i++)
	{
		...
		if(rdd_cmp_ui(dmaxii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_u12)!\n", i);
			return -1;
		}

		for(j = start_index; j < jmax; j++)
		{
			rcdd_set(&ctmp, get_cddmatrix_ij(a, j, i));
			//printf("(j              , i              ) = (%ld, %ld) %25.17e\n", j, i , dtmp);
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_ddmatrix_ij(a, j, k) * get_ddmatrix_ij(a, k, i);
				rcdd_mul(&ctmp1, get_cddmatrix_ij(a, j, k), get_cddmatrix_ij(a, k, i));
				rcdd_sub(&ctmp, &ctmp, &ctmp1);
			...
			set_cddmatrix_ij(a  , j              , i       , &ctmp);
			set_cddmatrix_ij(u12, j - start_index, i - imax, &ctmp);
		}
	}
...
int CDDLUdecomp_u12_omp(CDDMatrix u12, CDDMatrix a, long int start_index, long int min_dim)
{
	...
	for(i = imax; i < dim; i++)
	{
		...
		if(rdd_cmp_ui(dmaxii[thread_index], 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_u12_omp)!\n", i);
			//return -1;
		}

		for(j = start_index; j < jmax; j++)
		{
			rcdd_set(&ctmp[thread_index], get_cddmatrix_ij(a, j, i));
			//printf("(j              , i              ) = (%ld, %ld) %25.17e\n", j, i , dtmp);
			for(k = start_index; k < j; k++)
			{
				//dtmp -= get_ddmatrix_ij(a, j, k) * get_ddmatrix_ij(a, k, i);
				rcdd_mul(&ctmp1[thread_index], get_cddmatrix_ij(a, j, k), get_cddmatrix_ij(a, k, i));
				rcdd_sub(&ctmp[thread_index], &ctmp[thread_index], &ctmp1[thread_index]);
				//printf("(j - start_index, k - start_index) = (%ld, %ld)\n", j - start_index, k - start_index);
			}
			//printf("u12(j              , i       ) = (%ld, %ld) %ld, %ld\n", j, i, u12->row_dim, u12->col_dim);
			//printf("u12(j - start_index, i - imax) = (%ld, %ld) %ld, %ld\n", j - start_index, i - imax, u12->row_dim, u12->col_dim);
			set_cddmatrix_ij(a  , j              , i       , &ctmp[thread_index]);
			set_cddmatrix_ij(u12, j - start_index, i - imax, &ctmp[thread_index]);
		}
	}
...
//	mul_ddmatrix_block(d22, l21, u12, min_dim);
	//printf("cdd_block");
	mul_cddmatrix_block(d22, l21, u12, STRASSEN_MIN_DIM);
#elif USE_STRASSEN
	//printf("cdd_strassen");
//	mul_ddmatrix_strassen(d22, l21, u12, min_dim);
	mul_cddmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);
#elif USE_WINOGRAD
//	mul_ddmatrix_strassen(d22, l21, u12, min_dim);
	//printf("cdd_winograd");
	mul_cddmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);
#elif USE_SIMPLE
	//printf("cdd_simple");
	mul_cddmatrix_simple(d22, l21, u12);
#else // USE_BLOCK
	//printf("cdd_strassen");
	mul_cddmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);
#endif // USE_BLOCK
...
#elif USE_SIMPLE
	//printf("cdd_simple");
	_bncomp_mul_cddmatrix_simple(d22, l21, u12);
#else // USE_BLOCK
	//printf("cdd_strassen");
	_bncomp_mul_cddmatrix_strassen(d22, l21, u12, STRASSEN_MIN_DIM);
#endif // USE_BLOCK
...
/************************************************************/
{
	...
	for(i = 0; i < dim; i += min_dim)
	{
		...
		//printf("D22:(row_dim, min_dim, col_dim) = (%ld, %ld, %ld)\n", row_dim, min_dim, col_dim);
		//print_ddmatrix(d22);
		//print_ddmatrix(a);
...
/************************************************************/
{
	long int i, j, k, imax, jmax, itmp, dim;
	//static 
	double tmp[BNCOMP_MAX_NUM_THREADS][DDSIZE], axii[DDSIZE];
    //static 
	cddfloat ctmp[BNCOMP_MAX_NUM_THREADS];
	int thread_index;
	...
	{
		thread_index = omp_get_thread_num();
		set0_cdd(&ctmp[thread_index]);
	}

	for(i = 0; i < dim; i++)
	{
		rcdd_abs_dd(axii, get_cddmatrix_ij(a, i, i));

		if(rdd_cmp_ui(axii, 0UL) == 0)
		{
			fprintf(stderr, "%ld : Error! (CDDLUdecomp_omp)!\n", i);
			return -1;
		}
		...
		for(j = (i + 1); j < dim; j++)
		{
			...
			for(k = (i + 1); k < dim; k++)
			{
				rcdd_mul(&ctmp[thread_index], get_cddmatrix_ij(a, j, i), get_cddmatrix_ij(a, i, k));
				rcdd_sub(&ctmp[thread_index], get_cddmatrix_ij(a, j, k), &ctmp[thread_index]);
				set_cddmatrix_ij(a, j, k, &ctmp[thread_index]);
			}
		}
	}
...
/************************************************************/
{
	long int i, j, k, imax, jmax, itmp, dim;
	//static 
	double tmp[BNCOMP_MAX_NUM_THREADS][DDSIZE], tmp1[BNCOMP_MAX_NUM_THREADS][DDSIZE], axii[DDSIZE];
    //Sstatic 
	cddfloat ctmp[BNCOMP_MAX_NUM_THREADS], ctmp1[BNCOMP_MAX_NUM_THREADS];
	int thread_index;
// SIMD : for copy & paste
	...
	__m256d dtmp256_re[BNCOMP_MAX_NUM_THREADS][DDSIZE], aji256_re[BNCOMP_MAX_NUM_THREADS][DDSIZE], ajk256_re[BNCOMP_MAX_NUM_THREADS][DDSIZE], aik256_re[BNCOMP_MAX_NUM_THREADS][DDSIZE];
	__m256d dtmp256_im[BNCOMP_MAX_NUM_THREADS][DDSIZE], aji256_im[BNCOMP_MAX_NUM_THREADS][DDSIZE], ajk256_im[BNCOMP_MAX_NUM_THREADS][DDSIZE], aik256_im[BNCOMP_MAX_NUM_THREADS][DDSIZE];
	...
	for(i = 0; i < dim; i++)
	{
		...
		for(j = (i + 1); j < dim; j++)
		{
			rcdd_abs_dd(tmp[0], get_cddmatrix_ij(a, j, i));
...
/*                     (Complex Double-double Precision)    */
/*                                                          */
/*                 ver. 0.0 2023-12-21 (Tue) Tomonori Kouya */
/*                                                          */
/************************************************************/
...
/*                                 (Quadraple Precision)    */
/*                                                          */
/*                 ver. 0.0 2023-12-12 (Thu) Tomonori Kouya */
/*                                                          */
/************************************************************/
