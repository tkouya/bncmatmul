...
/*                                 (Double Precision)       */
/*                                                          */
/*                 ver. 0.0 2004.11.03 (Wed) Tomonori Kouya */
/*                 ver. 0.1 2012-03-17 (Sat) Tomonori Kouya */
/*                 ver. 0.2 2025-02-19 (Wed) Tomonori Kouya */
/*                                                          */
/************************************************************/
#ifdef USE_SPARSE_VERSION
	#ifdef USE_OMP_VERSION
		...
		#else // USE_PRECOND
		long int _bncomp_DBiCG_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#else // USE_OMP_VERSION
		#ifndef USE_PRECOND
		long int DBiCG(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes)
		#else // USE_PRECOND
		long int DBiCG_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#endif // USE_OMP_VERSION
...
/************************************************************/
{
	...
	DVector dvec[2]; // for Preconditioning
...
/* Set initial value */
...
#ifdef USE_PRECOND
	dvec[0] = init_dvector(dim);
	dvec[1] = init_dvector(dim);
#endif // USE_PRECOND
	...
	subst_dvector(vec[7], vec[1]);
	subst_dvector(vec[8], vec[2]);
#ifdef USE_PRECOND
	if(ilu != NULL)
	{
		subst_dvector(dvec[0], vec[7]);
		solve_iLU0_drsmatrix(vec[7], ilu, dvec[0]);
		subst_dvector(dvec[1], vec[8]);
		//solve_iLU0t_cdrsmatrix(vec[2], ilu, dvec[1]);
		solve_iLU0t_drsmatrix(vec[8], ilu, dvec[1]);
}
#endif // USE_PRECOND
...
/* Main loop */
	for(times = 0; times < maxtimes; times++)
	{
		...
		if(times == 0)
		{
			...
			subst_dvector(vec[3], vec[7]);
			subst_dvector(vec[4], vec[8]);
		}
		else
		{
			beta = rho / old_rho;
			...
			add_cmul_dvector(vec[3], vec[7], beta, vec[3]);
			//add_cmul_dvector(vec[4], vec[2], beta, vec[4]);
			add_cmul_dvector(vec[4], vec[8], beta, vec[4]);
		}
		...
		mul_drsmatrixt_dvec(vec[6], a, vec[4]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
		mul_dmatrix_dvec(vec[5], a, vec[3]);
		mul_dmatrixt_dvec(vec[6], a, vec[4]);
#endif // USE_SPARSE_VERSION
		...
		sub_cmul_dvector(vec[1], vec[1], alpha, vec[5]);
		//add_cmul_dvector(vec[2], vec[2], -alpha, vec[6]);
		sub_cmul_dvector(vec[2], vec[2], alpha, vec[6]);

		//beta_num = ip_dvector(vec[1], vec[1]);

		/* Stopping Criteria */
		//dtmp = sqrt(beta_num);
        dtmp = norm2_dvector(vec[1]);
		#ifdef USE_PRECOND
...
/************************************************************/
		...
		#else // USE_PRECOND
		long int _bncomp_DCGS_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#else // USE_OMP_VERSION
		#ifndef USE_PRECOND
		long int DCGS(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes)
		#else // USE_PRECOND
		long int DCGS_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#endif // USE_OMP_VERSION
#endif // USE_SPARSE_VERSION
/************************************************************/
...
/* Set initial value */
	...
	if(ilu != NULL)
	{
		subst_dvector(dvec[0], vec[1]);
		solve_iLU0_drsmatrix(vec[1], ilu, dvec[0]);
	}
#else // USE_PRECOND
	subst_dvector(vec[1], b);
#endif // USE_PRECOND
	//subst_cdvector(vec[2], b);
	subst_dvector(vec[2], vec[1]);

	//beta_num = ip_dvector(vec[1], vec[1]);
	//init_resnorm = sqrt(beta_num);
    init_resnorm = norm2_dvector(vec[1]);
	#ifdef USE_PRECOND
	if(norm2_res_history != NULL) norm2_res_history[0] = init_resnorm;
	#endif // USE_PRECOND
...
/* Main loop */
	for(times = 0; times < maxtimes; times++)
	{
...
#else // USE_OMP_VERSION
		mul_drsmatrix_dvec(vec[8], a, vec[3]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
		mul_dmatrix_dvec(vec[8], a, vec[3]);
#endif // USE_SPARSE_VERSION
		...
		if(ilu != NULL)
		{
			subst_dvector(dvec[0], vec[8]);
			solve_iLU0_drsmatrix(vec[8], ilu, dvec[0]);
		}
#endif // USE_PRECOND
...
/************************************************************/
		...
		#else // USE_PRECOND
		long int _bncomp_DBiCGSTAB_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#else // USE_OMP_VERSION
		#ifndef USE_PRECOND
		long int DBiCGSTAB(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes)
		#else // USE_PRECOND
		long int DBiCGSTAB_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#endif // USE_OMP_VERSION
#endif // USE_SPARSE_VERSION
/************************************************************/
...
/************************************************************/
{
	...
	double dtmp, init_resnorm;
	DVector vec[10]; /* Temporary Vectors */
	DVector dvec[2]; // for Preconditioning
...
/* Main loop */
	for(times = 0; times < maxtimes; times++)
	{
		...
		if(ilu != NULL)
		{
			subst_dvector(vec[9], vec[5]);
			solve_iLU0_drsmatrix(vec[5], ilu, vec[9]);
		}
#endif // USE_PRECOND
		...
		if(ilu != NULL)
		{
			subst_dvector(vec[9], vec[8]);
			solve_iLU0_drsmatrix(vec[8], ilu, vec[9]);
		}
#endif // USE_PRECOND
...
/************************************************************/
		...
		#else // USE_PRECOND
		long int _bncomp_DGPBiCG_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#else // USE_OMP_VERSION
		#ifndef USE_PRECOND
		long int DGPBiCG(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes)
		#else // USE_PRECOND
		long int DGPBiCG_iLU0(DVector answer, DMatrix a, DVector b, double reps, double aeps, long int maxtimes, DMatrix ilu, double *norm2_res_history)
		#endif // USE_PRECOND
	#endif // USE_OMP_VERSION
#endif // USE_SPARSE_VERSION
/************************************************************/
...
/************************************************************/
{
	...
	DVector dvec[2]; // Preconditioning
...
#else // USE_OMP_VERSION
			mul_drsmatrix_dvec(vec[4], a, vec[3]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
			mul_dmatrix_dvec(vec[4], a, vec[3]);
#endif // USE_SPARSE_VERSION

#ifdef USE_PRECOND
			if(ilu != NULL)
			{
				subst_dvector(dvec[0], vec[4]);
				solve_iLU0_drsmatrix(vec[4], ilu, dvec[0]);
			}
#endif // USE_PRECOND
...
/*                 ver. 0.1 2012-03-17 (Sat) Tomonori Kouya */
/*                 ver. 0.2 2024-10-19 (Sat) Tomonori Kouya */
/*                 ver. 0.3 2025-02-19 (Wed) Tomonori Kouya */
/*                                                          */
/************************************************************/
#ifdef USE_SPARSE_VERSION
	#ifdef USE_OMP_VERSION
	...
	#else // USE_PRECOND
	long int MPFBiCG_iLU0(MPFVector answer, MPFMatrix a, MPFVector b, mpf_t reps, mpf_t aeps, long int maxtimes, MPFMatrix ilu, MPFVector norm2_res_history)
	#endif // USE_PRECOND
#endif // USE_SPARSE_VERSION
/************************************************************/
/*                                                          */
...
/************************************************************/
{
	...
	MPFVector dvec[2]; // Preconditioning
...
/* Set initial value */
...
#ifdef USE_PRECOND
	dvec[0] = init2_mpfvector(dim, prec);
	dvec[1] = init2_mpfvector(dim, prec);
#endif // USE_PRECOND
	...
	subst_mpfvector(vec[7], vec[1]);
	subst_mpfvector(vec[8], vec[2]);
#ifdef USE_PRECOND
	if(ilu != NULL)
	{
		subst_mpfvector(dvec[0], vec[1]);
		subst_mpfvector(dvec[1], vec[2]);
#ifdef USE_SPARSE_D_VERSION
		//solve_iLU0_cdrsmatrix_cmpfvec(vec[1], ilu, dvec[0]);
		//solve_iLU0t_cdrsmatrix_cmpfvec(vec[2], ilu, dvec[1]);
		solve_iLU0_drsmatrix_mpfvec(vec[7], ilu, dvec[0]);
		solve_iLU0t_drsmatrix_mpfvec(vec[8], ilu, dvec[1]);
#else // USE_SPARSE_D_VERSION
		//solve_iLU0_cmpfrsmatrix(vec[1], ilu, dvec[0]);
		//solve_iLU0t_cmpfrsmatrix(vec[2], ilu, dvec[1]);
		solve_iLU0_mpfrsmatrix(vec[7], ilu, dvec[0]);
		solve_iLU0t_mpfrsmatrix(vec[8], ilu, dvec[1]);
#endif // 
	}
#endif // USE_PRECOND
    ...
    norm2_mpfvector(init_resnorm, vec[1]);
#ifdef USE_PRECOND
...
/* Main loop */
	...
	{
		/* rho */
		ip_mpfvector(rho, vec[2], vec[7]); // vec[1]);
		...
		{
			/* p := w, pt := wt */
			subst_mpfvector(vec[3], vec[7]); //vec[1]);
			subst_mpfvector(vec[4], vec[8]); //vec[2]);
		}
		else
		{
			...
			add_cmul_mpfvector(vec[3], vec[7], beta, vec[3]);
			add_cmul_mpfvector(vec[4], vec[8], beta, vec[4]);
		}
		/* z := Ap, zt := A^T pt */
#ifdef USE_SPARSE_VERSION
#ifdef USE_OMP_VERSION
...
#else // USE_OMP_VERSION
		mul_mpfrsmatrix_mpfvec(vec[5], a, vec[3]);
		mul_mpfrsmatrixt_mpfvec(vec[6], a, vec[4]);
#endif // USE_OMP_VERSION
#elif USE_SPARSE_D_VERSION
#ifdef USE_OMP_VERSION
...
#else // USE_OMP_VERSION
		mul_drsmatrix_mpfvec(vec[5], a, vec[3]);
		mul_drsmatrixt_mpfvec(vec[6], a, vec[4]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
		mul_mpfmatrix_mpfvec(vec[5], a, vec[3]);
		mul_mpfmatrixt_mpfvec(vec[6], a, vec[4]);
#endif // USE_SPARSE_VERSION
		...
		if(ilu != NULL)
		{
			subst_mpfvector(dvec[0], vec[7]);
			subst_mpfvector(dvec[1], vec[8]);
#ifdef USE_SPARSE_D_VERSION
			solve_iLU0_drsmatrix_mpfvec(vec[7], ilu, dvec[0]);
...
#ifdef USE_PRECOND
	free_mpfvector(dvec[0]);
	free_mpfvector(dvec[1]);
#endif // USE_PRECOND
...
/************************************************************/
	...
	#else // USE_PRECOND
	long int MPFCGS_iLU0(MPFVector answer, MPFMatrix a, MPFVector b, mpf_t reps, mpf_t aeps, long int maxtimes, MPFMatrix ilu, MPFVector norm2_res_history)
	#endif // USE_PRECOND
#endif // USE_SPARSE_VERSION
/************************************************************/
/*                                                          */
...
/* Set initial value */
	...
	if(ilu != NULL)
	{
		...
		solve_iLU0_drsmatrix_mpfvec(vec[1], ilu, dvec[0]);
		// denormalize
		//cmul_cmpfvector(vec[1], ctmp, dvec[0]);
#else // USE_SPARSE_D_VERSION
		solve_iLU0_mpfrsmatrix(vec[1], ilu, dvec[0]);
#endif // USE_SPARSE_D_VERSION
	}
#endif // USE_PRECOND
	//subst_mpfvector(vec[2], b);
	subst_mpfvector(vec[2], vec[1]);
	//subst_mpfvector(vec[2], b);
...
/* Main loop */
	for(times = 0; times < maxtimes; times++)
	{
...
#else // USE_OMP_VERSION
		mul_mpfrsmatrix_mpfvec(vec[8], a, vec[3]);
#endif // USE_OMP_VERSION
#elif USE_SPARSE_D_VERSION
#ifdef USE_OMP_VERSION
		_bncomp_mul_drsmatrix_mpfvec(vec[8], a, vec[3]);
#else // USE_OMP_VERSION
		mul_drsmatrix_mpfvec(vec[8], a, vec[3]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
		mul_mpfmatrix_mpfvec(vec[8], a, vec[3]);
#endif // USE_SPARSE_VERSION
		...
		if(ilu != NULL)
		{
			...
			//cmul_cmpfvector(dvec[1], ctmp1, dvec[0]);
			//solve_iLU0_cdrsmatrix_cmpfvec(dvec[0], ilu, dvec[1]);
			solve_iLU0_drsmatrix_mpfvec(vec[8], ilu, dvec[0]);
			// denormalize
			//cmul_cmpfvector(vec[8], ctmp, dvec[0]);
#else // USE_SPARSE_D_VERSION
			solve_iLU0_mpfrsmatrix(vec[8], ilu, dvec[0]);
#endif // USE_SPARSE_D_VERSION
		}
#endif // USE_PRECOND
...
#else // USE_OMP_VERSION
		mul_mpfrsmatrix_mpfvec(vec[8], a, vec[6]);
#endif // USE_OMP_VERSION
#elif USE_SPARSE_D_VERSION
#ifdef USE_OMP_VERSION
		_bncomp_mul_drsmatrix_mpfvec(vec[8], a, vec[6]);
#else // USE_OMP_VERSION
		mul_drsmatrix_mpfvec(vec[8], a, vec[6]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
...
#ifdef USE_PRECOND
	free_mpfvector(dvec[0]); // = init2_cmpfvector(dim, prec);
	free_mpfvector(dvec[1]); // = init2_cmpfvector(dim, prec);
#endif // USE_PRECOND
...
/************************************************************/
	...
	#else // USE_PRECOND
	long int MPFBiCGSTAB_iLU0(MPFVector answer, MPFMatrix a, MPFVector b, mpf_t reps, mpf_t aeps, long int maxtimes, MPFMatrix ilu, MPFVector norm2_res_history)
	#endif // USE_PRECOND
#endif // USE_SPARSE_VERSION
/************************************************************/
/*                                                          */
...
/************************************************************/
{
	...
	mpf_t init_resnorm;
	MPFVector vec[9]; /* Temporary Vectors */
	MPFVector dvec[2]; // for Preconditioning
...
/* Main loop */
	for(times = 0; times < maxtimes; times++)
	{
		...
		if(ilu != NULL)
		{
			subst_mpfvector(dvec[0], vec[5]);
#ifdef USE_SPARSE_D_VERSION
			solve_iLU0_drsmatrix_mpfvec(vec[5], ilu, dvec[0]);
#else // USE_SPRARSE_D_VERSION
			solve_iLU0_mpfrsmatrix(vec[5], ilu, dvec[0]);
#endif // USE_SPARSE_D_VERSION
		}
#endif // USE_PRECOND
...
/************************************************************/
	...
	#else // USE_PRECOND
	long int MPFGPBiCG_iLU0(MPFVector answer, MPFMatrix a, MPFVector b, mpf_t reps, mpf_t aeps, long int maxtimes, MPFMatrix ilu, MPFVector norm2_res_history)
	#endif // USE_PRECOND
#endif // USE_SPARSE_VERSION
/************************************************************/
/*                                                          */
...
#else // USE_OMP_VERSION
			mul_mpfrsmatrix_mpfvec(vec[4], a, vec[3]);
#endif // USE_OMP_VERSION
#elif USE_SPARSE_D_VERSION
#ifdef USE_OMP_VERSION
			_bncomp_mul_drsmatrix_mpfvec(vec[4], a, vec[3]);
#else // USE_OMP_VERSION
			mul_drsmatrix_mpfvec(vec[4], a, vec[3]);
#endif // USE_OMP_VERSION
#else // USE_SPARSE_VERSION
			mul_mpfmatrix_mpfvec(vec[4], a, vec[3]);
#endif // USE_SPARSE_VERSION

#ifdef USE_PRECOND
			if(ilu != NULL)
			{
				subst_mpfvector(dvec[0], vec[4]);
#ifdef USE_SPARSE_D_VERSION
				solve_iLU0_drsmatrix_mpfvec(vec[4], ilu, dvec[0]);
#else // USE_SPARSE_D_VERSION
				solve_iLU0_mpfrsmatrix(vec[4], ilu, dvec[0]);
#endif // USE_SPARSE_D_VERSION
			}
#endif // USE_PRECOND
