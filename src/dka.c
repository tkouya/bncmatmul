...
/* center of gravity */
/* center = -a_{n-1} / (n * a_n) */
double _Complex cddka_center(CDPoly func)
{
	double _Complex ret;

	ret = get_cdpoly_i(func, func->deg - 1);
	ret /= get_cdpoly_i(func, func->deg);
	ret /= func->deg;
	ret = -ret;
...
/* radius = max (m * |a_i / a_deg|)^(1/(deg-i)) */
double cddka_radius(CDPoly func)
{
	long int i;
	double ret, tmp, num_nonzero;
	double _Complex ctmp, an;

	num_nonzero = num_nonzero_cdpoly(func);
	an = get_cdpoly_i(func, func->deg);
	ret = num_nonzero;
	for(i = func->deg - 1; i >= 0; i--)
	{
		ctmp = get_cdpoly_i(func, i) / an;
		ctmp *= num_nonzero;
		tmp = cabs(ctmp);
		tmp = pow(tmp, 1.0/(func->deg - i));
		if(ret < tmp)
...
/* Aberth's initial value */
/* (max (m*|a_i / a_deg|)^(1/(deg-i)) */
void cddka_init(double _Complex x_init[], CDPoly func)
{
	long int i, itmp;
	double rad, an, tmp, re_cinit, im_cinit;
	double _Complex cen;
	//DCmplx cinit;
	double _Complex cinit;

	rad = cddka_radius(func);
	cen = cddka_center(func);
...
//	printf("%f, %f\n", rad, cen);
	...
	for(i = 0; i < func->deg; i++)
	{
		...
		tmp = (double)(2.0 * M_PI * i / func->deg + 3.0 / (2.0 * func->deg));
		//iexp_dcmplx(cinit, tmp);
		cinit = cos(tmp) + sin(tmp) * I;
		//re_cinit = get_real_dcmplx(cinit);
		re_cinit = creal(cinit);
		//im_cinit = get_image_dcmplx(cinit);
		im_cinit = cimag(cinit);

		re_cinit = creal(cen) + rad * re_cinit;
		im_cinit = cimag(cen) + rad * im_cinit;

		//set_real_dcmplx(cinit, re_cinit);
		//set_image_dcmplx(cinit, im_cinit);
		cinit = re_cinit + im_cinit * I;
...
/* Aberth's initial value */
/* (max (m*|a_i / a_deg|)^(1/(deg-i)) */
void cddka_init_cdarray(CDArray x_init, CDPoly func)
{
	long int i, itmp;
	double rad, an, tmp, re_cinit, im_cinit;
	double _Complex cen;
...
/* DKA Method : 2nd order */
/* f(x) = 0 -> x_new := x_n - func(x) / func'(x) */
long int cddka(double _Complex ans[], double _Complex x_init[], CDPoly func, long int maxtimes, double abs_eps, double rel_eps)
{
	long int times, i, j, deg, flag;
	double absmodval, abs_x, abs_newx;
	//DCmplx modval, up_modval, low_modval, tmp;
	double _Complex modval, up_modval, low_modval, tmp;
...
//		printf("%5d: ", times);
		...
		for(i = 0; i < deg; i++)
		{
			...
			//mul_dcmplx_d(low_modval, low_modval, get_dpoly_i(func, func->deg));
			low_modval *= get_cdpoly_i(func, func->deg);
			//ceval_dpoly(up_modval, func, get_cdarray_i(x_init, i));
			up_modval = eval_cdpoly(func, x_init[i]); // get_cdarray_i(x_init, i));

			//div_dcmplx(modval, up_modval, low_modval);
			modval = up_modval / low_modval;
			//sub_dcmplx(tmp, get_cdarray_i(x_init, i), modval);
			tmp = x_init[i] - modval;
			//set_cdarray_i(ans, i, tmp);
			ans[i] = tmp;
//			printf("%25.17e, ", get_real_dcmplx(get_cfarray_i(ans, i)));
			...
			/* check convergence */
			//absmodval = abs_dcmplx(modval);
			absmodval = cabs(modval);
			//abs_x = abs_dcmplx(get_cdarray_i(x_init, i));
			abs_newx = cabs(ans[i]); //abs_dcmplx(get_cdarray_i(ans, i));
			if( absmodval > (abs_x + abs_newx) * rel_eps + abs_eps )
				flag = 1;
