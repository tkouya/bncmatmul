...
extern "C" {
void _bncomp_mul_ddmatrix_ddvec(DDVector, DDMatrix, DDVector);
void _bncomp_mul_tdmatrix_tdvec(TDVector, TDMatrix, TDVector);
void _bncomp_mul_qdmatrix_qdvec(QDVector, QDMatrix, QDVector);
void _bncomp_mul_dsmatrix_dsvec(DSVector, DSMatrix, DSVector);
void _bncomp_mul_tsmatrix_tsvec(TSVector, TSMatrix, TSVector);
void _bncomp_mul_qsmatrix_qsvec(QSVector, QSMatrix, QSVector);
}
...
{                                                                                    \
    ...
    *t_cpu = 0.0;                                                                    \
    if (run_cpu) {                                                                   \
        _bncomp_mul_##P##matrix_##P##vec(cs_cpu, A_cpu, b_cpu);   /* warm-up */              \
        *t_cpu = measure_min_cpu(opts.reps, [&]() {                                  \
            _bncomp_mul_##P##matrix_##P##vec(cs_cpu, A_cpu, b_cpu);                          \
        });                                                                          \
    }                                                                                \
                                                                                     \
    /* ---- GPU side ---- */                                                         \
    ...
    for (int p = 0; p < N_PRECS; ++p) {
        ...
        for (long N : opts.sizes) {
            ...
            if (cpu_ran) {
                ...
                emit_csv("matvec", bare, N, 0, tcpu, tgpu, "omp", rel_err);
            } else {
                printf("  %-4s %-9ld   %14s   %14.6f   %9s   %s\n",
                       PRECS[p].name, N, "(skipped)", tgpu, "-", "(no CPU ref)");
                emit_csv("matvec", bare, N, 0, 0.0, tgpu, "none", -1.0);
            }
        }
        PRECS[p].end();
    }
