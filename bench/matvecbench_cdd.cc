...
int matvecloop_cdd(long int start_dim, long int end_dim, long int dim_step, long int maxsec, long int opt_dim_flag)
{
	long int dim;
	...
	CDDMatrix mat_a;
    CDDVector vec_x, vec_y; // vec_y := mat_a * vec_x 
	CDDVector vec_y_long;
	dd_real tmp, tmp1, relerr[7];
#ifdef USE_MBLAS
    dd_complex ctmp, one = (dd_real)(1.0), zero = (dd_real)(0.0);
	dd_complex *mat_a_m, *vec_x_m, *vec_y_m;
#endif // def USE_MBLAS
	double *ddtmp, *ddtmp2;
#ifdef USE_OZ
	...
	for(dim = start_dim; dim <= end_dim; dim += dim_step)
	{
	    ...
	    vec_x = init_cddvector(dim);
		vec_y = init_cddvector(dim);
		vec_y_long = init_cddvector(dim);

	// mpack
#ifdef USE_MBLAS
		mat_a_m = new dd_complex[dim * dim];
		vec_x_m = new dd_complex[dim];
		vec_y_m = new dd_complex[dim];
#endif // ifdef USE_MBLAS
	    rdd_srand((unsigned long)(dim * 106));
		for(i = 0; i < dim; i++)
		{
			for(j = 0; j < dim; j++)
			{
...
//				rdd_sqrt(tmp.x, tmp.x);
				...
				// tmp := (ru - 0.5) * exp(rn)
				rdd_urand(tmp.x); // ru in [0, 1]
				rdd_sub_d(tmp.x, tmp.x, 0.5); // (ru - 0.5)
				rdd_nrand(tmp1.x); rdd_exp_mpfr(tmp1.x, tmp1.x); // exp(rn)
				rdd_mul(tmp.x, tmp.x, tmp1.x);
#ifdef USE_MBLAS
				rdd_set(ctmp.real().x, tmp.x);
#endif // USE_MBLAS
				rdd_urand(tmp.x); // ru in [0, 1]
				rdd_sub_d(tmp.x, tmp.x, 0.5); // (ru - 0.5)
				rdd_nrand(tmp1.x);rdd_exp_mpfr(tmp1.x, tmp1.x); // exp(rn)
				rdd_mul(tmp.x, tmp.x, tmp1.x);
#ifdef USE_MBLAS
                rdd_set(ctmp.imag().x, tmp.x);
				//mpc_set_fr_fr(ctmp, tmp, tmp, get_bnc_default_rounding_mode());
				set_cddmatrix_ij_dd_complex(mat_a, i, j, ctmp);
				//set_cddmatrix_ij(da, i, j, tmp.x);
				//cout << "tmp = " << tmp << endl;
				//printf("tmp_x = %25.17e\n", tmp.x[0]);
				// mpack
...
#ifdef USE_MBLAS
            rdd_set(ctmp.imag().x, tmp.x);
			set_cddvector_i_dd_complex(vec_x, i, ctmp);
...
//			for(i = 0; i < iteration; i++) mul_ddmatrix_block(vec_y, mat_a, vec_x, _BNC_DEFAULT_MIN_DIM_STRASSEN);
			for(i = 0; i < iteration; i++) _bncomp_mul_cddmatrix_cddvec(vec_y, mat_a, vec_x);
#else // USE_OMP
			for(i = 0; i < iteration; i++) mul_cddmatrix_cddvec(vec_y, mat_a, vec_x);
#endif // USE_OMP
		...
		for(i = 0; i < dim; i++)
		{
			set_cddvector_i_dd_complex(vec_y, i, vec_y_m[i]);
		}
		//printf("||C|| = "); normf_cddmatrix(tmp.x, vec_y); cout << tmp.x[0]; printf("\n");
#else // USE_MBLAS
		//printf("||C|| = "); normf_cddmatrix(tmp.x, vec_y); cout << tmp.x[0]; printf("\n");
#endif // USE_MBLAS
	...
	// get relative errors
#ifdef USE_OMP
		_bncomp_mul_cddmatrix_cddvec(vec_y_long, mat_a, vec_x);
#else // USE_OMP
		mul_cddmatrix_cddvec(vec_y_long, mat_a, vec_x);
#endif // USE_OMP
		relerr3_cddvector(relerr[0].x, relerr[1].x, relerr[2].x, relerr[3].x, relerr[4].x, relerr[5].x, relerr[6].x, vec_y, vec_y_long, 0);
/*
...
*/
		...
		printf("[%5ld bits] %5ld x %5ld: %10.3g sec ", (long int)106, dim, dim, etime);
		...
		free_cddvector(vec_x);
		free_cddvector(vec_y);
		free_cddvector(vec_y_long);
...
//	for(idim = 32; idim <= 16384; idim *= 2)
	...
	//for(idim = 1024; idim < 32768; idim *= 2)
	{
		...
		end_flag = matvecloop_cdd(idim - 1, idim + 1, 1, maxsec, 0);

//		end_flag = matmulloop_dd(idim - 1, idim + 1, 1, maxsec, 0);
		idim_half = 3 * idim / 2;

		if(idim_half >= min_dim)
			end_flag = matvecloop_cdd(idim_half - 1, idim_half + 1, 1, maxsec, 0);

//		if((idim <= opt_dim) && (opt_dim <= idim * 2))
//			end_flag = matmulloop_cdd(opt_dim - 5, opt_dim + 5, 1, maxsec, 1, min_dim);
