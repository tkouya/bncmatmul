...
QDVector init_qdvector(long int dimension)
{
	...
	ret = (QDVector)malloc(sizeof(qdvector)); // Fix it!: 2024-04-26(Fri) T.Kouya
	if(ret == NULL)
		return ret;
...
void add2_qdvector(QDVector c, QDVector a)
{
    ...
    for(index = 0; index < c->real_dim; index += _BNC_D_WIDTH)
    {
        ...
        _bncavx512_rqd_add(tmp8, in_ret, in_a_val);
...
void sub2_qdvector(QDVector c, QDVector a)
{
    ...
    for(index = 0; index < c->real_dim; index += _BNC_D_WIDTH)
    {
        ...
        _bncavx512_rqd_sub(tmp8, in_ret, in_a_val);
...
void sub_cmul_qdvector(QDVector c, QDVector a, double val[QDSIZE], QDVector b)
{
	...
	if((a->dim != b->dim) || (c->dim != a->dim) || (c->dim != b->dim))
	{
		fprintf(stderr, "ERROR: sub_cmul_qdvector\n");
		return;
	}
#if defined(__AVX2__) && !defined(__AVX512F__) // __AVX2__
	__m256d a4[QDSIZE], b4[QDSIZE], c4[QDSIZE], val4[QDSIZE];
