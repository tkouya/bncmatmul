...
#elif defined(__ARM_NEON) || defined(__aarch64__) // NEON
...
#elif defined(__ARM_NEON) || defined(__aarch64__) // NEON
    float64x2_t tmp[TDSIZE];
    for(i = 0; i < a->real_dim; i += _BNC_D_WIDTH)
    {
        tmp[0] = vnegq_f64(vld1q_f64(&(a->element[0][i])));
        tmp[1] = vnegq_f64(vld1q_f64(&(a->element[1][i])));
        tmp[2] = vnegq_f64(vld1q_f64(&(a->element[2][i])));
        vst1q_f64(&(c->element[0][i]), tmp[0]);
        vst1q_f64(&(c->element[1][i]), tmp[1]);
        vst1q_f64(&(c->element[2][i]), tmp[2]);
    }
#else // others
	double tmp[TDSIZE];
...
void normf_tdmatrix(double ret[TDSIZE], TDMatrix mat)
{
    ...
    float64x2_t cij[TDSIZE], aik[TDSIZE], bkj[TDSIZE], tmp[TDSIZE];
    ...
    for(i = 0; i < ret->real_row_dim; i += 1)
    {
        for(j = 0; j < ret->real_col_dim; j += _BNC_D_WIDTH)
        {
            ...
            for(k = 0; k < real_mid_dim; k += 1)
            {
                ...
                _bncneon_rtd_mul(tmp, aik, bkj);
                _bncneon_rtd_add(cij, cij, tmp);
            }
            vst1q_f64(&(ret->element[0][i*ret->real_col_dim + j]), cij[0]);
            vst1q_f64(&(ret->element[1][i*ret->real_col_dim + j]), cij[1]);
            vst1q_f64(&(ret->element[2][i*ret->real_col_dim + j]), cij[2]);
        }
    }
#else // others
	double mat1[TDSIZE];
...
//#endif // BNC_USE_GCC
    ...
    float64x2_t acc[TDSIZE], aij[TDSIZE], vbj[TDSIZE], tmp[TDSIZE];
    double sum[TDSIZE];
    for(i = 0; i < v->dim; i += 1)
    {
        _bncneon_set0_td(acc);
        for(j = 0; j < real_col_dim; j += _BNC_D_WIDTH)
        {
            aij[0] = vld1q_f64(&(a->element[0][i*real_col_dim + j]));
            aij[1] = vld1q_f64(&(a->element[1][i*real_col_dim + j]));
            aij[2] = vld1q_f64(&(a->element[2][i*real_col_dim + j]));

            vbj[0] = vld1q_f64(&(vb->element[0][j]));
            vbj[1] = vld1q_f64(&(vb->element[1][j]));
            vbj[2] = vld1q_f64(&(vb->element[2][j]));

            _bncneon_rtd_mul(tmp, aij, vbj);
            _bncneon_rtd_add(acc, acc, tmp);
        }
        _bncneon_rtd_sum128d(sum, acc);
        v->element[0][i] = sum[0];
        v->element[1][i] = sum[1];
        v->element[2][i] = sum[2];
    }
    ...
    long real_row_dim = a->real_row_dim;
    long real_col_dim = a->real_col_dim;
    float64x2_t acc[TDSIZE], aij[TDSIZE], vbi[TDSIZE], tmp[TDSIZE];
    double sum[TDSIZE];
    for(j = 0; j < v->dim; j += 1)
    {
        _bncneon_set0_td(acc);
        for(i = 0; i < real_row_dim; i += 1)
        {
            ...
            aij[1] = vld1q_f64(&(a->element[1][i*real_col_dim + j]));
            aij[2] = vld1q_f64(&(a->element[2][i*real_col_dim + j]));
            vbi[0] = vdupq_n_f64(vb->element[0][i]);
            vbi[1] = vdupq_n_f64(vb->element[1][i]);
            vbi[2] = vdupq_n_f64(vb->element[2][i]);
            _bncneon_rtd_mul(tmp, aij, vbi);
            _bncneon_rtd_add(acc, acc, tmp);
        }
        _bncneon_rtd_sum128d(sum, acc);
        v->element[0][j] = sum[0];
