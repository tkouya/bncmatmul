...
void _bncomp_neg_ddmatrix_partial(DDMatrix ret, long int ret_index[4], DDMatrix mat_a, long int mat_a_index[4])
{
	...
	for(i = 0; i < imax; i++)
	{
		...
		for(j = 0; j < jmax; j++)
		{
			...
			set_ddmatrix_ij(ret, ret_i[thread_index], ret_j[thread_index], -get_ddmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
#else // __cplusplus
			rdd_neg(tmp, get_ddmatrix_ij(mat_a, a_i[thread_index], a_j[thread_index]));
...
void _bncomp_mul_ddmatrix_block(DDMatrix ret, DDMatrix mat_a, DDMatrix mat_b, long int min_dim)
{
	...
	small_tmp_mat = (DDMatrix *)calloc(sizeof(DDMatrix), num_div_mid);
...
*/
	...
	for(i = 0; i < max_num_div; i++)
	{
		mat_a_index[i][0] = i * min_dim;
		mat_a_index[i][1] = (i + 1) * min_dim;
		mat_a_index[i][2] = i * min_dim;
		mat_a_index[i][3] = (i + 1) * min_dim;

		mat_b_index[i][0] = i * min_dim;
		mat_b_index[i][1] = (i + 1) * min_dim;
		mat_b_index[i][2] = i * min_dim;
		mat_b_index[i][3] = (i + 1) * min_dim;
	}
	...
	for(i = 0; i < num_div_row; i++)
	{
		...
		*/
		...
		for(j = 0; j < num_div_col; j++)
		{
			...
			mat_b_index[j][2] = j * min_dim;
			mat_b_index[j][3] = (j + 1) * min_dim;
...
#endif // __cplusplus
	DDMatrix mpfc, mpfa, mpfb, mpfc_normal, mpfc_block;
...
//	dim = 128;
	if(argc <= 1)
	{
		fprintf(stderr, "Usage: %s [dim] [#thread]\n", argv[0]);
		return 0;
	}
	dim = atol(argv[1]);
	if(dim <= 0)
...
//	printf("OpenMP #Threads = %ld\n", omp_get_num_threads());
	...
	mpfc_block = init_ddmatrix(dim, dim);
...
//	mul_ddmatrix_strassen(mpfc, mpfa, mpfb, 32);
	...
	sub_ddmatrix(mpfb, mpfc_normal, mpfc_block);
	...
	normi_ddmatrix(&ddtmp[1], mpfb);
	normi_ddmatrix(&ddtmp[2], mpfc_normal);
#else // __cplusplus
	normi_ddmatrix(ddtmp[0], mpfa);
	normi_ddmatrix(ddtmp[1], mpfb);
	normi_ddmatrix(ddtmp[2], mpfc_normal);
