...
//	for(i = 0; i < vec->dim; i++)
    for(i = 0; i < vec->avx2_dim; i += 4) // AVX2
	{
#ifdef __cplusplus
		vec->element[i] = 0.0;
		vec->element[i + 1] = 0.0;
		...
		vec->element[i + 3] = 0.0;
#else // __cplusplus
		SET0_DDVECTOR_I(vec, i);
		SET0_DDVECTOR_I(vec, i + 1);
		SET0_DDVECTOR_I(vec, i + 2);
        SET0_DDVECTOR_I(vec, i + 3);
#endif // __cplusplus
	}
}
...
DDVector init_ddvector(long int dim)
{
	...
	ret->dim = dim;
    ret->avx2_dim = (dim / 4) * 4;
    if((dim % 4) >= 1) ret->avx2_dim += 4;

#ifdef __cplusplus
//	ret->element = (dd_real *)calloc(dim, sizeof(dd_real));
	ret->element = (dd_real *)calloc(avx2_dim, sizeof(dd_real));
#else // __cplusplus
//	ret->element = (double *)calloc(dim, sizeof(double) * DDSIZE);
	ret->element = (double *)calloc(avx2_dim, sizeof(double) * DDSIZE);
#endif // __cplusplus
	if(ret->element == NULL)
	{
...
//	for(index = 0; index < dim; index++)
	for(index = 0; index < avx2_dim; index += 4)
	{
#ifdef __cplusplus
		ret->element[index] = 0.0;
		ret->element[index + 1] = 0.0;
		ret->element[index + 2] = 0.0;
		ret->element[index + 3] = 0.0;
#else // __cplusplus
		*(ret->element + index * DDSIZE)     = 0.0;
		*(ret->element + index * DDSIZE + 1) = 0.0;
		*(ret->element + (index + 1) * DDSIZE)     = 0.0;
		*(ret->element + (index + 1) * DDSIZE + 1) = 0.0;
		*(ret->element + (index + 2) * DDSIZE)     = 0.0;
		*(ret->element + (index + 2) * DDSIZE + 1) = 0.0;
		*(ret->element + (index + 3) * DDSIZE)     = 0.0;
		*(ret->element + (index + 3) * DDSIZE + 1) = 0.0;
#endif // __cplusplus
	}
...
//	for(i = 0; i < c->dim; i++)
	for(i = 0; i < c->avx2_dim; i += 4)
	{
#ifdef __cplusplus
		c->element[i    ] = a->element[i    ] + b->element[i    ];
		c->element[i + 1] = a->element[i + 1] + b->element[i + 1];
		c->element[i + 2] = a->element[i + 2] + b->element[i + 2];
		c->element[i + 3] = a->element[i + 3] + b->element[i + 3];
#else // __cplusplus
		rdd_add(tmp, get_ddvector_i(a, i),  get_ddvector_i(b, i));
		set_ddvector_i(c, i, tmp);
		rdd_add(tmp, get_ddvector_i(a, i + 1),  get_ddvector_i(b, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_add(tmp, get_ddvector_i(a, i + 2),  get_ddvector_i(b, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_add(tmp, get_ddvector_i(a, i + 3),  get_ddvector_i(b, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif //  __cplusplus
	}
}
/* c += a */
...
//	for(i = 0; i < c->dim; i++)
	for(i = 0; i < c->avx2_dim; i += 4)
	{
		...
		c->element[i + 1] += a->element[i + 1];
		c->element[i + 2] += a->element[i + 2];
		c->element[i + 3] += a->element[i + 3];
#else // __cplusplus
		rdd_add(tmp, get_ddvector_i(c, i), get_ddvector_i(a, i));
		set_ddvector_i(c, i, tmp);
		rdd_add(tmp, get_ddvector_i(c, i + 1), get_ddvector_i(a, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_add(tmp, get_ddvector_i(c, i + 2), get_ddvector_i(a, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_add(tmp, get_ddvector_i(c, i + 3), get_ddvector_i(a, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif //  __cplusplus
	}
...
void sub_ddvector(DDVector c, DDVector a, DDVector b)
{
    ...
    for(i = 0; i < c->avx2_dim; i += 4) // AVX2
	{
		...
		set_ddvector_i(c, i, tmp);
		rdd_sub(tmp, get_ddvector_i(a, i + 1), get_ddvector_i(b, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_sub(tmp, get_ddvector_i(a, i + 2), get_ddvector_i(b, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_sub(tmp, get_ddvector_i(a, i + 3), get_ddvector_i(b, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif // __cplusplus
	}
...
//	for(i = 0; i < c->dim; i++)
	for(i = 0; i < c->avx2_dim; i += 4)
	{
		...
		set_ddvector_i(c, i, tmp);
		rdd_sub(tmp, get_ddvector_i(c, i + 1), get_ddvector_i(a, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_sub(tmp, get_ddvector_i(c, i + 2), get_ddvector_i(a, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_sub(tmp, get_ddvector_i(c, i + 3), get_ddvector_i(a, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif // __cplusplus
	}
...
//	for(i = 0; i < c->dim; i++)
	for(i = 0; i < c->avx2_dim; i += 4)
    {
		...
		c->element[i + 1] = val * a->element[i + 1];
		c->element[i + 2] = val * a->element[i + 2];
		c->element[i + 3] = val * a->element[i + 3];
#else // __cplusplus
		rdd_mul(tmp, val, get_ddvector_i(a, i));
		set_ddvector_i(c, i, tmp);
		rdd_mul(tmp, val, get_ddvector_i(a, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_mul(tmp, val, get_ddvector_i(a, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_mul(tmp, val, get_ddvector_i(a, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif // __cplusplus
	}
...
#endif // __cplusplus
{
    ...
    for(i = 0; i < c->avx2_dim; i += 4)
	{
		...
		set_ddvector_i(c, i, tmp);
		rdd_mul(tmp, val, get_ddvector_i(c, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_mul(tmp, val, get_ddvector_i(c, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_mul(tmp, val, get_ddvector_i(c, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif // __cplusplus
	}
...
//	for(i = 0; i < c->dim; i++)
	for(i = 0; i < c->avx2_dim; i += 4)
    {
		...
		c->element[i + 1] = a->element[i + 1] + val * b->element[i + 1];
		c->element[i + 2] = a->element[i + 2] + val * b->element[i + 2];
		c->element[i + 3] = a->element[i + 3] + val * b->element[i + 3];
#else // __cplusplus
		rdd_mul(tmp, val, get_ddvector_i(b, i));
		rdd_add(tmp, tmp, get_ddvector_i(a, i));
		set_ddvector_i(c, i, tmp);

		rdd_mul(tmp, val, get_ddvector_i(b, i + 1));
		rdd_add(tmp, tmp, get_ddvector_i(a, i + 1));
		set_ddvector_i(c, i + 1, tmp);

		rdd_mul(tmp, val, get_ddvector_i(b, i + 2));
		rdd_add(tmp, tmp, get_ddvector_i(a, i + 2));
		set_ddvector_i(c, i + 2, tmp);

		rdd_mul(tmp, val, get_ddvector_i(b, i + 3));
		rdd_add(tmp, tmp, get_ddvector_i(a, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif // __cplusplus
	}
}
...
#endif // __cplusplus
//	for(i = 0; i < a->dim; i++)
	for(i = 0; i < a->avx2_dim; i += 4)
    {
#ifdef __cplusplus
		*ret += a->element[i] * b->element[i];
		*ret += a->element[i + 1] * b->element[i + 1];
		*ret += a->element[i + 2] * b->element[i + 2];
		*ret += a->element[i + 3] * b->element[i + 3];
#else // __cplusplus
		rdd_mul(tmp, get_ddvector_i(a, i), get_ddvector_i(b, i));
		rdd_add(ret, ret, tmp);
		rdd_mul(tmp, get_ddvector_i(a, i + 1), get_ddvector_i(b, i + 1));
		rdd_add(ret, ret, tmp);
		rdd_mul(tmp, get_ddvector_i(a, i + 2), get_ddvector_i(b, i + 2));
		rdd_add(ret, ret, tmp);
		rdd_mul(tmp, get_ddvector_i(a, i + 3), get_ddvector_i(b, i + 3));
		rdd_add(ret, ret, tmp);
#endif // __cplusplus
	}
...
//	for(i = 0; i < a->dim; i++)
	for(i = 0; i < a->avx2_dim; i += 4)
	{
		...
		set_ddvector_i(c, i + 1, get_ddvector_i(a, i + 1));
		set_ddvector_i(c, i + 2, get_ddvector_i(a, i + 2));
		set_ddvector_i(c, i + 3, get_ddvector_i(a, i + 3));
#endif // __cplusplus
	}
}
...
//	for(i = 0; i < a->dim; i++)
	for(i = 0; i < a->avx2_dim; i += 4)
	{
		...
		set_ddvector_i(c, i, tmp);
		rdd_neg(tmp, get_ddvector_i(a, i + 1));
		set_ddvector_i(c, i + 1, tmp);
		rdd_neg(tmp, get_ddvector_i(a, i + 2));
		set_ddvector_i(c, i + 2, tmp);
		rdd_neg(tmp, get_ddvector_i(a, i + 3));
		set_ddvector_i(c, i + 3, tmp);
#endif // __cplusplus
	}
}
...
#ifdef __cplusplus
		*ret += abs(a->element[i]);
		*ret += abs(a->element[i + 1]);
		*ret += abs(a->element[i + 2]);
		*ret += abs(a->element[i + 3]);
#else // __cplusplus
		rdd_abs(tmp, get_ddvector_i(a, i));
		rdd_add(ret, ret, tmp);
		rdd_abs(tmp, get_ddvector_i(a, i + 1));
		rdd_add(ret, ret, tmp);
		rdd_abs(tmp, get_ddvector_i(a, i + 2));
		rdd_add(ret, ret, tmp);
		rdd_abs(tmp, get_ddvector_i(a, i + 3));
		rdd_add(ret, ret, tmp);
#endif // __cplusplus
	}
...
#endif // __cplusplus
//	for(i = 1; i < a->dim; i++)
	for(i = 1; i < a->avx2_dim; i += 4)
	{
#ifdef __cplusplus
		tmp = abs(a->element[i]);
		if(*ret < tmp) *ret = tmp;
		tmp = abs(a->element[i + 1]);
		if(*ret < tmp) *ret = tmp;
		tmp = abs(a->element[i + 2]);
		if(*ret < tmp) *ret = tmp;
		tmp = abs(a->element[i + 3]);
		if(*ret < tmp) *ret = tmp;
#else // __cplusplus
		rdd_abs(tmp, get_ddvector_i(a, i));
...
#endif // __cplusplus
//	dim = vec->dim;
    avx2_dim = vec->avx2_dim;

//	for(i = 0; i < dim ; i++)
	for(i = 0; i < avx2_dim ; i += 4)
	{
#ifdef __cplusplus
		//*ret += vec->element[i] * vec->element[i];
		tmp += sqr(vec->element[i]);
		tmp += sqr(vec->element[i + 1]);
		tmp += sqr(vec->element[i + 2]);
		tmp += sqr(vec->element[i + 3]);
#else // __cplusplus
		c_dd_sqr(GET_DDVECTOR_I(vec, i), tmp);
		c_dd_add(tmp, ret, ret);
		c_dd_sqr(GET_DDVECTOR_I(vec, i + 1), tmp);
		c_dd_add(tmp, ret, ret);
		c_dd_sqr(GET_DDVECTOR_I(vec, i + 2), tmp);
		c_dd_add(tmp, ret, ret);
		c_dd_sqr(GET_DDVECTOR_I(vec, i + 3), tmp);
		c_dd_add(tmp, ret, ret);
#endif // __cplusplus
	}
...
// Use set_tdvector_i as functions
#ifdef USE_TDLINEAR_FUNCTIONS
#ifndef __cplusplus
// set element
//void set_tdvector_i(DDVector vec, long int index, dd_real value)
void set_tdvector_i(TDVector vec, long int index, double value[TDSIZE])
{
	if((index < 0) || (index >= vec->dim))
	{
		fprintf(stderr, "ERROR: illegal element index! (index = %ld)\n", index);
...
//	*(vec->element + index * TDSIZE)     = value[0];
//	*(vec->element + index * TDSIZE + 1) = value[1];
//	*(vec->element + index * TDSIZE + 2) = value[2];
//	*(vec->element + index * TDSIZE + 3) = value[3];
	SET_TDVECTOR_I(vec, index, value);
}
#endif // __cplusplus
#endif // USE_TDLINEAR_FUNCTIONS
...
TDVector init_tdvector(long int dim)
{
	...
	// callocation
	ret = (tdvector *)malloc(sizeof(tdvector));
	if(ret == NULL)
	{
...
//	ret->element = new qd_real[dim];
#else // __cplusplus
	ret->element = (double *)calloc(dim, sizeof(double) * TDSIZE);
#endif // __cplusplus
	if(ret->element == NULL)
	{
		fprintf(stderr, "ERROR: cannot allocate TDVector(dim = %ld)\n", dim);
		free(ret);
		return NULL;
	}
	...
	for(index = 0; index < dim; index++)
	{
		...
		//ret->element[index] = sqrt(ret->element[index]);
#else // __cplusplus
		*(ret->element + index * TDSIZE)     = 0.0;
		*(ret->element + index * TDSIZE + 1) = 0.0;
		*(ret->element + index * TDSIZE + 2) = 0.0;
#endif // __cplusplus
	}
...
void print_tdvector(TDVector vec)
{
	...
	for(index = 0; index < vec->dim; index++)
	{
		...
		std::cout << (vec->element[index]).to_string(62) << "\n";
#else // __cplusplus
		//c_td_write((vec->element + index * TDSIZE));
...
/* c = val * a */
#ifdef __cplusplus
void cmul_tdvector(TDVector c, qd_real val, TDVector a)
#else // __cplusplus
void cmul_tdvector(TDVector c, double val[TDSIZE], TDVector a)
#endif // __cplusplus
{
...
/* c *= val */
#ifdef __cplusplus
void cmul2_tdvector(TDVector c, qd_real val)
#else // __cplusplus
void cmul2_tdvector(TDVector c, double val[TDSIZE])
#endif // __cplusplus
{
...
/* c = a + val * b */
#ifdef __cplusplus
void add_cmul_tdvector(TDVector c, TDVector a, qd_real val, TDVector b)
#else // __cplusplus
void add_cmul_tdvector(TDVector c, TDVector a, double val[TDSIZE], TDVector b)
#endif // __cplusplus
{
...
/* (a, b) */
#ifdef __cplusplus
void ip_tdvector(qd_real *ret, TDVector a, TDVector b)
#else // __cplusplus
...
/* ||a||_1 */
#ifdef __cplusplus
//void norm1_tdvector(qd_real &ret, TDVector a)
void norm1_tdvector(qd_real *ret, TDVector a)
#else // __cplusplus
...
/* ||a||_infty */
#ifdef __cplusplus
//void normi_tdvector(qd_real &ret, TDVector a)
void normi_tdvector(qd_real *ret, TDVector a)
#else // __cplusplus
...
// Euclid norm
#ifdef __cplusplus
//void norm2_tdvector(qd_real &ret, TDVector vec)
void norm2_tdvector(qd_real *ret, TDVector vec)
#else // __cplusplus
...
#endif // __cplusplus
{
	...
	*ret = (qd_real)0.0;
	...
	c_td_copy_d((double)0.0, tmp);
	c_td_copy_d((double)0.0, ret);
#endif // __cplusplus
	...
	for(i = 0; i < dim ; i++)
	{
		...
		tmp += qd_real::accurate_mul(vec->element[i], vec->element[i]);
#else // __cplusplus
		c_td_sqr(GET_TDVECTOR_I(vec, i), tmp);
		c_td_add(tmp, ret, ret);
#endif // __cplusplus
	}
...
//		for(j = 0; j < mat->col_dim; j++)
		for(j = 0; j < mat->avx2_col_dim; j += 4)
		{
#ifdef __cplusplus
			mat->element[i * mat->col_dim + j] = 0.0;
			mat->element[i * mat->col_dim + j + 1] = 0.0;
			mat->element[i * mat->col_dim + j + 2] = 0.0;
			mat->element[i * mat->col_dim + j + 3] = 0.0;
#else // __cplusplus
			SET0_DDMATRIX_IJ(mat, i, j);
			SET0_DDMATRIX_IJ(mat, i, j + 1);
			SET0_DDMATRIX_IJ(mat, i, j + 2);
			SET0_DDMATRIX_IJ(mat, i, j + 3);
#endif // __cplusplus
		}
	}
}
...
DDMatrix init_ddmatrix(long int row_dim, long int col_dim)
{
	...
	long int avx2_row_index, avx2_col_index;
	DDMatrix ret = NULL;
	...
	ret->row_dim = row_dim;
	ret->col_dim = col_dim;
	avx2_row_dim = (row_dim / 4) + (((row_dim % 4) >= 1) ? 1 : 0) * 4;
	avx2_col_dim = (com_dim / 4) + (((col_dim % 4) >= 1) ? 1 : 0) * 4;
#ifdef __cplusplus
//	ret->element = (dd_real *)calloc(row_dim * col_dim, sizeof(dd_real));
	ret->element = (dd_real *)calloc(avx2_row_dim * avx2_col_dim, sizeof(dd_real));
#else // __cplusplus
//	ret->element = (double *)calloc(row_dim * col_dim, sizeof(double) * DDSIZE);
	ret->element = (double *)calloc(avx2_row_dim * avx2_col_dim, sizeof(double) * DDSIZE);
#endif // __cplusplus
	if(ret->element == NULL)
	{
    ...
    ret->avx2_row_dim = avx2_row_dim;
    ret->avx2_col_dim = avx2_col_dim;

	// set all zeros 
//	for(row_index = 0; row_index < row_dim; row_index++)
	for(row_index = 0; row_index < avx2_row_dim; row_index++)
	{
//		for(col_index = 0; col_index < col_dim; col_index++)
		for(col_index = 0; col_index < avx2_col_dim; col_index += 4)
        {
#ifdef __cplusplus
			ret->element[row_index * col_dim + col_index] = (dd_real)0.0;
			ret->element[row_index * col_dim + col_index + 1] = (dd_real)0.0;
			ret->element[row_index * col_dim + col_index + 2] = (dd_real)0.0;
			ret->element[row_index * col_dim + col_index + 3] = (dd_real)0.0;
#else // __cplusplus
			//*(ret->element + index * DDSIZE)     = 0.0;
			//*(ret->element + index * DDSIZE + 1) = 0.0;
			SET0_DDMATRIX_IJ(ret, row_index, col_index);
			SET0_DDMATRIX_IJ(ret, row_index, col_index + 1);
			SET0_DDMATRIX_IJ(ret, row_index, col_index + 2);
			SET0_DDMATRIX_IJ(ret, row_index, col_index + 3);
#endif // __cplusplus
		}
	}
...
TDMatrix init_tdmatrix(long int row_dim, long int col_dim)
{
	...
	if(ret == NULL)
	{
		fprintf(stderr, "ERROR: cannot allocate one tdmatrix\n");
		return ret;
	}

	ret->row_dim = row_dim;
	ret->col_dim = col_dim;
	ret->element = (double *)calloc(row_dim * col_dim, sizeof(double) * TDSIZE);
	if(ret->element == NULL)
	{
		fprintf(stderr, "ERROR: cannot allocate qdmatrix(row_dim, col_dim = %ld, %ld)\n", row_dim, col_dim);
		free(ret);
	...
	for(row_index = 0; row_index < row_dim; row_index++)
	{
		for(col_index = 0; col_index < col_dim; col_index++)
		{
			//*(ret->element + index * DDSIZE)     = 0.0;
			//*(ret->element + index * DDSIZE + 1) = 0.0;
			SET0_TDMATRIX_IJ(ret, row_index, col_index);
		}
	}
...
#ifdef __cplusplus
			index = i * row_dim + j;
			qd_a_ij.x[0] = *(get_tdmatrix_ij(a, i, j));
			qd_a_ij.x[1] = *(get_tdmatrix_ij(a, i, j) + 1);
			qd_a_ij.x[2] = *(get_tdmatrix_ij(a, i, j) + 2);
			qd_a_ij.x[3] = 0.0;
			qd_b_ij.x[0] = *(get_tdmatrix_ij(b, i, j));
			qd_b_ij.x[1] = *(get_tdmatrix_ij(b, i, j) + 1);
			qd_b_ij.x[2] = *(get_tdmatrix_ij(b, i, j) + 2);
			qd_b_ij.x[3] = 0.0;
			c->element[index] = qd_a - qd_b;
#else // __cplusplus
//			rqd_sub(tmp, get_qdmatrix_ij(a, i, j), get_qdmatrix_ij(b, i, j));
			qd_a_ij[0] = *(get_tdmatrix_ij(a, i, j));
			qd_a_ij[1] = *(get_tdmatrix_ij(a, i, j) + 1);
			qd_a_ij[2] = *(get_tdmatrix_ij(a, i, j) + 2);
			qd_a_ij[3] = 0.0;
			qd_b_ij[0] = *(get_tdmatrix_ij(b, i, j));
			qd_b_ij[1] = *(get_tdmatrix_ij(b, i, j) + 1);
			qd_b_ij[2] = *(get_tdmatrix_ij(b, i, j) + 2);
			qd_b_ij[3] = 0.0;
...
void transpose_tdmatrix(TDMatrix c, TDMatrix a)
{
	...
	if((c->row_dim != a->col_dim) || (c->col_dim != a->row_dim))
	{
		fprintf(stderr, "ERROR: transpose_tdmatrix\n");
		return;
	}
...
#ifdef __cplusplus
			index = i * row_dim + j;
			qd_c_ij.x[0] = *(get_tdmatrix_ij(a, i, j));
			qd_c_ij.x[1] = *(get_tdmatrix_ij(a, i, j) + 1);
			qd_c_ij.x[2] = *(get_tdmatrix_ij(a, i, j) + 2);
			qd_c_ij.x[3] = 0.0;
			c->element[index] = qd_c_ij;
#else // __cplusplus
//			rqd_sub(tmp, get_qdmatrix_ij(a, i, j), get_qdmatrix_ij(b, i, j));
			qd_c_ij[0] = *(get_tdmatrix_ij(a, i, j));
			qd_c_ij[1] = *(get_tdmatrix_ij(a, i, j) + 1);
			qd_c_ij[2] = *(get_tdmatrix_ij(a, i, j) + 2);
			qd_c_ij[3] = 0.0;
...
//	printf("start mul_tdmatrix(%ld, %ld)\n", ret->row_dim, ret->col_dim);
	...
	for(i = 0; i < row_dim; i++)
	{
		//printf("i = %ld, col_dim = %ld\n", i, col_dim);
		for(j = 0; j < col_dim; j++)
		{
			//printf("(%ld, %ld) ", i, j);
			set0_td(GET_TDMATRIX_IJ(ret, i, j));
			//printf("(%ld, %ld) ", i, j);
			for(k = 0; k < mid_dim; k++)
			{
				c_td_mul(GET_TDMATRIX_IJ(a, i, k), GET_TDMATRIX_IJ(b, k, j), tmp);
				c_td_add(tmp, GET_TDMATRIX_IJ(ret, i, j), GET_TDMATRIX_IJ(ret, i, j));
				//c_td_mul(GET_TDMATRIX_IJ(a, i, k), GET_TDMATRIX_IJ(b, k, j), tmp);
...
void normf_tdmatrix(double ret[TDSIZE], TDMatrix mat)
{
	...
	for(i = 0; i < dim ; i++)
	{
		c_td_sqr(GET_QDVECTOR_I(mat, i), tmp);
		c_td_add(tmp, ret, ret);
	}
...
void relerr_element_tdvector(double max_relerr[TDSIZE], double min_relerr[TDSIZE], double norm_relerr[TDSIZE], TDVector approx_vec, TDVector true_vec, int norm_type)
{
	...
	for(i = 0; i < approx_vec->dim; i++)
	{
		rtd_abs(abs_diff_vec, get_qdvector_i(diff_vec, i));
		rtd_abs(abs_true_vec, get_qdvector_i(true_vec, i));
