//******************************************************************************
// template_linear.h :Template for Multiple precision linear computation
//                                targetting double, dd_real, qd_real and mpreal
...
#define __TEMPLATE_TK_LINEAR_MP_VERSION__ "2021-07-15"
...
/* a = a^(-1) */
/* square matrix only */
template <typename T> void inverse_matrix(T a[], int dim)
{
	int i, j, k;
	T tmp, aii;
	...
	for(i = 0; i < dim; i++)
	{
		if(a[i * dim + i] == (T)0)
		{
			fprintf(stderr, "ERROR in inverse_matrix: Pivot(%d, %d) is zero.\n", i, i);
			return;
		}
...
void set_array(double ret_array[], mpfr::mpreal org_array[], int dim)
{
	...
	for(int i = 0; i < dim; i++)
	{
		...
		ret_array[i] = mpfr_get_d(mpfr_ptr(org_array[i]), MPFR_RNDN);
	}
}
...
// relerr: return relative error of val
void qd_relerr(qd_real &relerr, qd_real approx, mpfr::mpreal true_val)
{
	mpfr::mpreal tmp_approx, tmp_relerr;

	//mpfr_set_qd(tmp_approx.mpfr_ptr(), approx.x, MPFR_RNDN);
	mpfr_set_qd(mpfr_ptr(tmp_approx), approx.x, MPFR_RNDN);

	if(true_val != 0)
		tmp_relerr = abs((tmp_approx - true_val) / true_val);
	else
		tmp_relerr = abs(tmp_approx - true_val);

	mpfr_get_qd(relerr.x, mpfr_ptr(tmp_relerr), MPFR_RNDN);
	//mpfr_get_qd(relerr.x, tmp_relerr.mpfr_ptr(), MPFR_RNDN);
}
...
void set_array(dd_real ret_array[], mpfr::mpreal org_array[], int dim)
{
	...
	for(int i = 0; i < dim; i++)
	{
		mpfr_get_dd(ret_array[i].x, mpfr_ptr(org_array[i]), MPFR_RNDN);
...
void set_array(qd_real ret_array[], mpfr::mpreal org_array[], int dim)
{
	...
	for(int i = 0; i < dim; i++)
	{
		...
		mpfr_get_qd(ret_array[i].x, mpfr_ptr(org_array[i]), MPFR_RNDN);
	}
}
...
void set_array(mpfr::mpreal ret_array[], dd_real org_array[], int dim)
{
	...
	for(int i = 0; i < dim; i++)
	{
		mpfr_set_dd(mpfr_ptr(ret_array[i]), org_array[i].x, MPFR_RNDN);
...
void set_array(mpfr::mpreal ret_array[], qd_real org_array[], int dim)
{
	...
	for(int i = 0; i < dim; i++)
	{
		...
		mpfr_set_qd(mpfr_ptr(ret_array[i]), org_array[i].x, MPFR_RNDN);
	}
}
...
// 2021-06-23(Wed)
// row-major -> col-major
template <typename T> void row2col(T ret_mat[], T org_mat[], int dim)
{
	for(int i = 0; i < dim; i++)
	{
		for(int j = 0; j < dim; j++)
		{
			ret_mat[COL_ZERO_IJ(i, j, dim, dim)] = org_mat[ROW_ZERO_IJ(i, j, dim, dim)];
		}
	}
}

// 2021-06-23(Wed)
// get min and max relative errors
template <typename T> T get_minmax_relerr(T *relmax, T *relmin, T approx[], T true_val[], int dim)
{
	int i;
	T relerr, zero = (T)0;
	T dif_norm2, true_norm2;

	relerr = approx[0] - true_val[0];
	if(true_val[0] != zero)
		relerr /= true_val[0];
	if(relerr < zero)
		relerr = -relerr;
	...
	*relmax = relerr;
	*relmin = relerr;
	dif_norm2 = (approx[0] - true_val[0]) * (approx[0] - true_val[0]);
	true_norm2 = true_val[0] * true_val[0];

	for(i = 1; i < dim; i++)
	{
		relerr = approx[i] - true_val[i];
		if(true_val[i] != zero)
			relerr /= true_val[i];
		if(relerr < zero)
			relerr = -relerr;
		if(*relmax < relerr) *relmax = relerr;
		if(*relmin > relerr) *relmin = relerr;
		dif_norm2 += (approx[i] - true_val[i]) * (approx[i] - true_val[i]);
		true_norm2 += true_val[i] * true_val[i];
	}

	if(true_norm2 != (T)0)
		dif_norm2 /= true_norm2;
...
template <typename T> void read_test_linear_eq(T A[], T true_x[], T b[], long int dim, const char *fname_A, const char *fname_true_x, const char *fname_b)
{
	FILE *fp;
	int i, j, ret_scan;
	...
	if(!fp)
	{
		std::cerr << "cannot open \" "<< fname_A << " \" to read matrix A!" << std::endl;
	}
	else{
		std::cout << fname_A << " was successfully read..." << std::endl;
		do {
			ret_scan = fscanf(fp, "%d, %d, %s", &i, &j, str_num);
			A[ZERO_INDEX(i, j, dim)] = (T)str_num;
		} while(!feof(fp));
	...
	if(!fp)
	{
		std::cerr << "cannot open \" "<< fname_true_x << " \" to read true x!" << std::endl;
	}
	else{
		std::cout << fname_true_x << " was successfully read..." << std::endl;
		do {
	...
	if(!fp)
	{
		std::cerr << "cannot open \" "<< fname_b << " \" to read b!" << std::endl;
	}
	else{
		std::cout << fname_b << " was successfully read..." << std::endl;
		do {
			ret_scan = fscanf(fp, "%d, %s", &i, str_num);
			b[i] = (T)str_num;
		} while(!feof(fp));
